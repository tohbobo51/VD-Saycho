...
local function AddTab(name, icon)
    ...
    btn.Font="GothamMedium"; btn.TextSize=12
...
local function CreateToggle(parent, labelText, state, callback, order)
    ...
    lbl.TextColor3=Color3.new(1,1,1); lbl.Font="GothamMedium"; lbl.TextSize=12
...
local function AddDropdownFeature(parent, title, options_table, order)
    ...
    label.TextColor3=Color3.new(1,1,1); label.Font="GothamMedium"; label.TextSize=12
...
local function CreateSlider(parent, labelText, minVal, maxVal, defaultVal, callback, order)
    ...
    valLbl.TextColor3=COLOR.ORANGE; valLbl.Font="GothamBold"; valLbl.TextSize=12
    valLbl.BackgroundTransparency=1; valLbl.TextXAlignment="Right"
...
local function makeTag(plName, role)
    ...
    roleLbl.TextColor3=Color3.new(1,1,1); roleLbl.TextXAlignment="Center"
...
local function applyESP(pl)
    ...
    if States.EspTracer then
        -- draw line via Drawing (kalau executor support)
        pcall(function()
            if not c:FindFirstChild("ESP_Tracer") then
                local hrp=c:FindFirstChild("HumanoidRootPart")
                if hrp then
                    local myHRP=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                    if myHRP then
                        ...
                        line.From=Camera:WorldToViewportPoint(myHRP.Position)
                        line.To=Camera:WorldToViewportPoint(hrp.Position)
                        -- store reference
                        local tag=Instance.new("StringValue"); tag.Name="ESP_Tracer"; tag.Parent=c
...
local function startSpectate(target)
    ...
    if not target then Camera.CameraType=Enum.CameraType.Custom; return end
    States.SpectateTarget=target
    Camera.CameraType=Enum.CameraType.Scriptable
    specConn=RunService.Heartbeat:Connect(function()
        if not target or not target.Character then
            Camera.CameraType=Enum.CameraType.Custom; specConn:Disconnect(); return
        end
...
sLbl.Font="GothamBold"; sLbl.TextSize=12; sLbl.TextColor3=COLOR.SURVIVOR
sLbl.BackgroundTransparency=1; sLbl.TextXAlignment="Left"
local kLbl=Instance.new("TextLabel",legendRow)
kLbl.Size=UDim2.new(0.5,0,1,0); kLbl.Position=UDim2.new(0.5,0,0,0)
kLbl.Text="🔴 Killer  "; kLbl.Font="GothamBold"; kLbl.TextSize=12
kLbl.TextColor3=COLOR.KILLER; kLbl.BackgroundTransparency=1; kLbl.TextXAlignment="Right"
...
tpKillerBtn.Font="GothamBold"; tpKillerBtn.TextSize=12; tpKillerBtn.TextXAlignment="Left"
...
hopBtn.Font="GothamBold"; hopBtn.TextSize=12; hopBtn.TextXAlignment="Left"
...
rejoinBtn.Font="GothamBold"; rejoinBtn.TextSize=12; rejoinBtn.TextXAlignment="Left"
...
setH.TextColor3=COLOR.ORANGE; setH.Font="GothamBold"; setH.TextSize=14
setH.BackgroundTransparency=1; setH.TextXAlignment="Left"; setH.LayoutOrder=0
...
WM.Font="GothamBold"; WM.TextSize=11; WM.TextXAlignment="Left"; WM.ZIndex=5
