...
-- Heal diri sendiri (sekali klik)
SurTab:Button({
    Title       = "💊 Heal Diri Sendiri",
    Description = "HealEvent [HRP kita, true]",
    Callback    = function()
        local ok, err = pcall(function()
            local char = LocalPlayer.Character
            ...
            local rem = getHealRemote()
            if not rem then error("Remote Heal tidak ditemukan") end
...
-- Auto Heal diri sendiri
local autoHealSelfEnabled = false
SurTab:Toggle({
    Title       = "Auto Heal Diri  (loop tiap 1.5 detik)",
    Value       = false,
    Callback    = function(v)
            ...
            task.spawn(function()
                while autoHealSelfEnabled do
                    pcall(function()
                        ...
                        local hrp  = char and char:FindFirstChild("HumanoidRootPart")
                        local rem  = getHealRemote()
                        if hrp and rem then rem:FireServer(hrp, true) end
                    end)
                    task.wait(1.5)
                end
            end)
...
do
    ...
    SurTab:Button({
        ...
        Description = "HealEvent [HRP target, false]",
        Callback    = function()
            ...
            local ok, err = pcall(function()
                ...
                if not hrp then error("HRP target tidak ada") end
                local rem = getHealRemote()
...
local GriefTab = Window:CreateTab("Griefing", "skull")
    ...
    Callback = function()
        ...
        if gens then
            for _, gen in pairs(gens:GetChildren()) do
                -- Kirim sinyal rusak ke semua generator di map
                game:GetService("ReplicatedStorage").Remotes.Generator.BreakGenEvent:FireServer(gen)
            end
            WindUI:Notify({ Title = "Griefing", Desc = "Semua Generator Rusak!", Duration = 3 })
        end
    end
})
...
GriefTab:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({ Title = "Griefing", Desc = "Semua Pallet Hancur!", Duration = 3 })
    end
})
