...
-- FIX 1: cek Drawing lebih robust (Delta pakai pcall, bukan rawget)
local function hasDrawing()
    local ok, _ = pcall(function() return Drawing.new end)
    return ok and Drawing ~= nil
end

local function drawFOVCircle(radius)
    -- Hentikan update loop lama
    ...
    if not hasDrawing() then
        warn("[VD] Drawing API tidak tersedia di executor ini")
        return
    end
    ...
    pcall(function()
        ...
        fovCircle.Color     = Color3.fromRGB(168, 85, 247)  -- ungu, sinkron tema
        fovCircle.Thickness = 2        -- FIX 2: lebih tebal, kelihatan di mobile
        fovCircle.Filled    = false
        fovCircle.NumSides  = 64

        -- FIX 3: update posisi tiap frame (aman saat rotate layar)
...
-- ─────────────────────────────────────────────────────────────
AimTab:Section({ Title = "Silent Aim", Icon = "ghost" })

AimTab:Toggle({
    Title       = "Enable Silent Aim",
    Description = "Otomatis redirect tembakan ke target terdekat (tidak ubah kamera)",
    Value       = false,
    Callback    = function(v)
...
AimTab:Toggle({
    Title       = "Team Check  (skip sesama tim)",
    Description = "Silent Aim tidak akan redirect ke teammate",
    Value       = true,
    Callback    = function(v) Config.SilentAim.TeamCheck = v end
})

AimTab:Section({ Title = "Silent Aim Target", Icon = "user" })
...
AimTab:Dropdown({
    ...
    Value    = "Body",
    Callback = function(v)
        Config.SilentAim.Bone = v:lower()
    end
})

AimTab:Section({ Title = "Silent Aim Range & FOV", Icon = "sliders-horizontal" })
