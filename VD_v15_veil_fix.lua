...
-- Dijalankan di background - tidak block GUI
-- Veil remote hanya ada kalau sedang main sebagai Veil
task.spawn(function()
    task.wait(2) -- tunggu game load dulu
    pcall(function()
        local Remotes = RS:FindFirstChild("Remotes")
        if not Remotes then return end
        local veilFolder = Remotes:FindFirstChild("Killers")
            and Remotes.Killers:FindFirstChild("Veil")
        if not veilFolder then
            warn("[VD] Veil remote tidak ada (normal jika bukan main Veil)")
            return
        end
        R_VeilVFX       = veilFolder:FindFirstChild("vfx")
        R_VeilUpdateWep = veilFolder:FindFirstChild("updatewep")
        R_VeilSpear     = veilFolder:FindFirstChild("Spearthrow")
        print("[VD] Veil remotes loaded:", R_VeilVFX ~= nil, R_VeilUpdateWep ~= nil, R_VeilSpear ~= nil)
    end)
end)
