...
-- ============================================================
-- FPS COUNTER  (draggable, compact — dari VD v17)
-- ============================================================
local function setFPSCounter(on)
    if fpsGuiFrame and fpsGuiFrame.Parent then fpsGuiFrame:Destroy(); fpsGuiFrame = nil end
    if not on then return end

    local sg = Instance.new("ScreenGui"); sg.Name = "VD_FPS_Counter"
    sg.ResetOnSpawn = false; sg.DisplayOrder = 999
    sg.Parent = LocalPlayer:FindFirstChild("PlayerGui") or game:GetService("CoreGui")

    local frame = Instance.new("Frame", sg)
    frame.Size                   = UDim2.new(0, 185, 0, 28)
    ...
    st.Color = Color3.fromRGB(232, 137, 12); st.Thickness = 1; st.Transparency = 0.4

    local lbl = Instance.new("TextLabel", frame)
    lbl.Size = UDim2.new(1, -10, 1, 0); lbl.Position = UDim2.new(0, 8, 0, 0)
    lbl.Text = "VD  |  -- fps  |  -- ms"
    lbl.TextColor3 = Color3.fromRGB(232, 137, 12); lbl.Font = Enum.Font.GothamBold
    lbl.TextSize = 11; lbl.BackgroundTransparency = 1
    ...
    game:GetService("RunService").RenderStepped:Connect(function()
        ...
        if tick() - lastT >= 1 then
            lbl.Text = string.format("VD  |  %d fps  |  %d ms", frames, pingMs)
            frames = 0; lastT = tick()
        end
    end)
end
