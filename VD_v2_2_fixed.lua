...
-- (duplicate HttpService removed)

-- (duplicate player var removed)
...
-- INIT LANGSUNG: isi cache sekarang supaya radar tidak tunggu 3 detik pertama
local function NEX_ScanCache()
    local map = workspace:FindFirstChild("Map")
    if not map then return end
    local gens, hooks, pallets, gates = {}, {}, {}, {}
    for _, obj in ipairs(map:GetDescendants()) do
        if obj:IsA("BasePart") then
            if (obj.Name:find("Generator") or obj.Name:find("generator")) and not obj.Name:find("Point") then
                table.insert(gens, { part = obj, model = obj.Parent })
            elseif obj.Name == "Hook" or obj.Name == "HookPoint" or obj.Name == "HookHitbox" then
                table.insert(hooks, { part = obj, model = obj.Parent })
            elseif obj.Name == "Pallet" or obj.Name == "PalletStatic" then
                table.insert(pallets, { part = obj, model = obj.Parent })
            elseif obj.Name == "Gate" or obj.Name == "ExitGate" then
...
pcall(NEX_ScanCache)   -- init segera

-- ── Radar ScreenGui fallback (untuk mobile yang DrawingAvailable = false) ──
local RadarGui = nil
local radarDotPool = {}   -- pool frame bulat untuk titik radar
local RADAR_POOL_SIZE = 30

local function ensureRadarGui()
    if RadarGui and RadarGui.Parent then return end
    RadarGui = Instance.new("ScreenGui")
    RadarGui.Name = "VD_Radar"
    RadarGui.ResetOnSpawn = false
    RadarGui.DisplayOrder = 950
    RadarGui.IgnoreGuiInset = true
    RadarGui.Parent = LocalPlayer:FindFirstChild("PlayerGui") or game:GetService("CoreGui")

    -- Frame background radar
    ...
    stroke.Color = Color3.fromRGB(255,65,65); stroke.Thickness = 1.5

    -- Crosshair lines
    local h = Instance.new("Frame", bg); h.Name = "CrossH"
    h.BackgroundColor3 = Color3.fromRGB(40,40,40); h.BorderSizePixel=0
    h.AnchorPoint = Vector2.new(0.5,0.5)
    h.Position = UDim2.new(0.5,0,0.5,0)
    h.Size = UDim2.new(0.85,0,0,1)

    local v = Instance.new("Frame", bg); v.Name = "CrossV"
    v.BackgroundColor3 = Color3.fromRGB(40,40,40); v.BorderSizePixel=0
    v.AnchorPoint = Vector2.new(0.5,0.5)
    v.Position = UDim2.new(0.5,0,0.5,0)
    v.Size = UDim2.new(0,1,0.85,0)

    -- Center dot (kita)
    local center = Instance.new("Frame", bg); center.Name = "CenterDot"
    center.BackgroundColor3 = Color3.fromRGB(0,255,0); center.BorderSizePixel=0
    ...
    for i = 1, RADAR_POOL_SIZE do
        ...
        lbl.TextSize = 9
        lbl.Text = ""
        table.insert(radarDotPool, dot)
    end
end

local function GuiRadar_step()
    if not VD.RADAR_Enabled or DrawingAvailable then return end   -- pakai Drawing kalau tersedia
    pcall(ensureRadarGui)
    if not RadarGui then return end
    local bg = RadarGui:FindFirstChild("RadarBG")
    if not bg then return end

    local sz = VD.RADAR_Size or 160
    local vp = workspace.CurrentCamera.ViewportSize
    bg.Size     = UDim2.new(0, sz, 0, sz)
    bg.Position = UDim2.new(0, vp.X - sz - 20, 0, 20)
    bg.Visible  = true
    ...
    local scale  = (sz/2 - 8) / 150
    local maxR   = 0.42   -- maks jarak dari pusat (dalam skala 0-0.5)

    local function worldToRadarScale(px, pz)
        ...
        local dx, dy = rx2*scale/sz, rz2*scale/sz
        local len = math.sqrt(dx*dx+dy*dy)
        if len > maxR then dx=dx/len*maxR; dy=dy/len*maxR end
        return UDim2.new(0.5+dx, 0, 0.5+dy, 0)
    end

    local dotIdx = 1
    local function placeDot(pos3, color, label)
        if dotIdx > #radarDotPool then return end
        local rp = worldToRadarScale(pos3.X, pos3.Z)
        ...
        if lbl then lbl.Text = label or "" end
        dotIdx = dotIdx + 1
    end
    ...
    if VD.RADAR_Killer then
        for _, pl in ipairs(Players:GetPlayers()) do
            if pl ~= LocalPlayer and IsKiller(pl) and pl.Character then
                ...
                if r then placeDot(r.Position, Color3.fromRGB(255,65,65), "K") end
            end
        end
    end
    -- Survivors
    if VD.RADAR_Survivor then
        for _, pl in ipairs(Players:GetPlayers()) do
            if pl ~= LocalPlayer and IsSurvivor(pl) and pl.Character then
                local r = pl.Character:FindFirstChild("HumanoidRootPart")
                if r then placeDot(r.Position, Color3.fromRGB(65,220,130), "S") end
            end
        end
    end
    -- Generators
    if VD.RADAR_Generator then
        for _, gen in ipairs(NEX_Cache.Generators) do
            if gen.part then placeDot(gen.part.Position, Color3.fromRGB(255,180,50), "G") end
        end
    end
    -- Pallets
    if VD.RADAR_Pallet then
        for _, p in ipairs(NEX_Cache.Pallets) do
            if p.part then placeDot(p.part.Position, Color3.fromRGB(220,180,100), "P") end
        end
    end

    -- Sembunyikan sisa dot
    for i = dotIdx, #radarDotPool do radarDotPool[i].Visible = false end
end
...
local function AimbotUpdate(cam)
    ...
    -- Mobile always-active: tidak butuh RMB / touch hold
    local mobileAlways = VD.AIM_MobileAlways or false
    if VD.AIM_UseRMB and not AimState.AimHolding and not mobileAlways then
        AimState.AimTarget = nil; return
    end
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    -- Mobile: Touch tap juga aktifkan aimbot (kalau pakai mode touch)
    if input.UserInputType == Enum.UserInputType.Touch and VD.AIM_MobileAlways then
        AimState.AimHolding = true
    end
end)
UserInputService.InputEnded:Connect(function(input)
    ...
    -- Mobile: lepas touch → matikan hold (kecuali mobile-always aktif)
    if input.UserInputType == Enum.UserInputType.Touch and not VD.AIM_MobileAlways then
        AimState.AimHolding = false
    end
end)
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    pcall(GuiRadar_step)   -- Fallback radar (ScreenGui) untuk mobile
end)
-- Toggle values
        ...
        if obj and obj.Parent and obj.Name ~= "Lobby" then
            ...
            if targetPart then
                ...
                if isPlayer then
                    ...
                    -- Object case (no HP)
                    -- Cek apakah ini Generator → tampilkan progress
                    local genProgress = nil
                    if obj.Name == "Generator" then
                        -- Cari NumberValue "GeneratorProgress" atau "Progress" di dalam model
                        local pVal = obj:FindFirstChild("GeneratorProgress")
                            or obj:FindFirstChild("Progress")
                            or obj:FindFirstChild("Percentage")
                        if not pVal then
                            -- Cari rekursif satu level
                            for _, child in ipairs(obj:GetChildren()) do
                                if child:IsA("NumberValue") or child:IsA("IntValue") then
                                    pVal = child; break
                                end
                                -- Cari di PointLight: kalau hijau = selesai
                            end
                        end
                        if pVal then
                            genProgress = math.floor(math.clamp(pVal.Value, 0, 100))
                        else
                            -- Fallback: tebak dari warna PointLight
                            local hitbox = obj:FindFirstChild("HitBox")
                            local pl = hitbox and hitbox:FindFirstChildOfClass("PointLight")
                            if pl then
                                if pl.Color == Color3.fromRGB(126,255,126) then
                                    genProgress = 100  -- selesai (hijau)
                                elseif pl.Color == Color3.fromRGB(255,255,126) then
                                    genProgress = 50   -- sedang dikerjakan (kuning)
                                else
                                    genProgress = 0    -- belum
                                end
                            end
                    ...
                    if genProgress ~= nil then
                        local bar = string.rep("█", math.floor(genProgress/10))
                             .. string.rep("░", 10 - math.floor(genProgress/10))
                        data.hpLabel.Text    = bar .. " " .. genProgress .. "%"
                        data.hpLabel.Visible = true
                        -- Warna bar: hijau kalau selesai, kuning kalau jalan
                        if genProgress >= 100 then
                            data.hpLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
                        elseif genProgress > 0 then
                            data.hpLabel.TextColor3 = Color3.fromRGB(255, 220, 50)
                        else
                            data.hpLabel.TextColor3 = data.color
                        end
                    else
                        data.hpLabel.Text    = ""
                        data.hpLabel.Visible = false
...
-- Auto Heal
-- ════ SELF HEAL (Godmode approach, tapi TIDAK kebal) ════════════
-- Cara kerja:
--   • Pantau Humanoid.Health setiap Heartbeat
--   • Kalau HP drop di bawah threshold → set balik ke MaxHealth (revive/full HP)
--   • BEDA dari GodMode: GodMode set HP max TERUS-TERUSAN
--     Self Heal hanya trigger saat HP drop, lalu dibiarkan turun lagi secara normal
--     → tetap bisa kena damage, tapi otomatis bangkit/pulih
-- Threshold default 1 = hanya aktif saat knocked (HP=0) = revive saja
-- Kalau threshold diset lebih tinggi (mis 50) = auto heal saat HP < 50

local selfHealEnabled   = false
local selfHealThreshold = 1    -- default: revive saja (HP ≤ 1 = knocked)
...
local function startSelfHeal()
    ...
    selfHealConn = RunService.Heartbeat:Connect(function()
        ...
        -- Cek apakah HP di bawah threshold
        if hum.Health <= selfHealThreshold then
            -- Sama persis dengan GodMode tapi hanya 1x trigger, bukan loop max
            hum.Health = hum.MaxHealth

            -- Paksa humanoid state bangun (kalau knocked)
            pcall(function()
                hum:ChangeState(Enum.HumanoidStateType.GettingUp)
            end)

            -- Juga fire remote HealEvent sebagai backup
            pcall(function()
...
SurTab:Toggle({
    Title = "✅ Self Heal / Auto Revive (Godmode Style)",
    Desc  = "HP drop → langsung full lagi. Bisa kena damage tapi tidak mati/knocked",
    Value = false,
    Callback = function(v)
        selfHealEnabled = v
...
SurTab:Slider({
    ...
    Desc  = "1 = hanya revive saat knocked | 50 = heal kalau HP < 50",
...
AimTab:Toggle({
    Title       = "📱 Mobile Mode  (selalu aktif, tidak butuh RMB/Touch hold)",
    Description = "Aktifkan di mobile — aimbot jalan terus tanpa perlu tahan apapun",
    Value       = false,
    Callback    = function(v)
        VD.AIM_MobileAlways = v
        -- Kalau mobile mode aktif, langsung set holding = true supaya langsung jalan
        if v then
            AimState.AimHolding = true
            WindUI:Notify({ Title="Aimbot", Content="Mobile Mode aktif! Aimbot selalu ON.", Duration=2, Icon="crosshair" })
        else
            AimState.AimHolding = false
        end
    end
...
-- ============================================================
-- ============================================================
-- FLING TAB  — Flee + Fling (semua dalam 1 tab)
-- ============================================================
FlingTab:Section({ Title = "Flee Killer", Icon = "wind" })

FlingTab:Toggle({
    Title       = "Auto Flee Killer  (TP menjauh otomatis)",
    Description = "Saat killer terlalu dekat → teleport ke posisi terjauh dari killer",
    Value       = false,
    Callback    = function(v) VD.AUTO_TeleAway = v end
})
...
FlingTab:Slider({
    Title    = "Flee Trigger Distance  (studs)",
    Value    = { Min = 10, Max = 80, Default = 40 },
    Step     = 5,
    Callback = function(v) VD.AUTO_TeleAwayDist = v end
})

FlingTab:Section({ Title = "Fling", Icon = "zap" })

FlingTab:Button({
    Title       = "⚡ Fling Killer Terdekat",
    Description = "Fling ke killer yang paling dekat",
    Callback    = function()
        local ok, err = pcall(NEX_FlingNearest)
        WindUI:Notify({
            Title   = "Fling",
            Content = ok and "Fling fired!" or "Gagal: "..tostring(err),
            Duration = 2, Icon = ok and "zap" or "alert-circle"
        })
    end
})
...
FlingTab:Button({
    ...
    Description = "Fling ke semua player di server",
    Callback    = function()
        local ok, err = pcall(NEX_FlingAll)
        WindUI:Notify({
            Title   = "Fling",
            Content = ok and "Fling All fired!" or "Gagal: "..tostring(err),
            Duration = 2, Icon = ok and "zap" or "alert-circle"
        })
    end
})
