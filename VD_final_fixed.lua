...
-- ── ESP COLOR DROPDOWN (ColorPicker diganti Dropdown) ────────────
EspTab:Section({ Title = "ESP Color Settings", Icon = "palette" })

-- Helper: nama warna → Color3
local function nameToColor(name)
    local map = {
        ...
        Ungu      = Color3.fromRGB(168, 85, 247),
        Orange    = Color3.fromRGB(255, 165, 0),
        Pink      = Color3.fromRGB(255, 105, 180),
    }
    return map[name] or Color3.fromRGB(255, 255, 255)
end

local colorOptions = {"Biru","Merah","Hijau","Kuning","Putih","Cyan","Ungu","Orange","Pink"}
            ...
            for obj, data in pairs(espObjects) do
                ...
                if pl then
                    ...
                    if not isKiller then
                        if data.highlight then
                            data.highlight.FillColor    = COLOR_SURVIVOR
                            data.highlight.OutlineColor = COLOR_SURVIVOR
                        end
                        if data.nameLabel then data.nameLabel.TextColor3 = COLOR_SURVIVOR end
                        if data.hpLabel   then data.hpLabel.TextColor3   = COLOR_SURVIVOR end
                        if data.distLabel then data.distLabel.TextColor3 = COLOR_SURVIVOR end
                    end
                end
            end
        end)
        ...
        pcall(function()
            ...
            for obj, data in pairs(espObjects) do
                ...
                if pl then
                    ...
                    if isKiller then
                        if data.highlight then
                            data.highlight.FillColor    = COLOR_MURDERER
                            data.highlight.OutlineColor = COLOR_MURDERER
                        end
                        if data.nameLabel then data.nameLabel.TextColor3 = COLOR_MURDERER end
                        if data.hpLabel   then data.hpLabel.TextColor3   = COLOR_MURDERER end
                        if data.distLabel then data.distLabel.TextColor3 = COLOR_MURDERER end
                    end
                end
            end
        end)
...
EspTab:Dropdown({
    ...
    Value   = "Putih",
    Callback = function(v)
        pcall(function() COLOR_GENERATOR = nameToColor(v) end)
    end
})
...
EspTab:Dropdown({
    ...
    Value   = "Hijau",
    Callback = function(v)
        pcall(function() COLOR_GENERATOR_DONE = nameToColor(v) end)
    end
})
...
EspTab:Dropdown({
    ...
    Value   = "Putih",
    Callback = function(v)
        pcall(function() COLOR_GATE = nameToColor(v) end)
    end
})
...
EspTab:Dropdown({
    ...
    Value   = "Kuning",
    Callback = function(v)
        pcall(function() COLOR_PALLET = nameToColor(v) end)
    end
})
...
EspTab:Dropdown({
    ...
    Value   = "Merah",
    Callback = function(v)
        pcall(function() COLOR_HOOK = nameToColor(v) end)
    end
})
...
EspTab:Dropdown({
    ...
    Value   = "Orange",
    Callback = function(v)
        pcall(function() COLOR_WINDOW = nameToColor(v) end)
    end
})
...
GriefTab:Dropdown({
    ...
    Values  = {"Ungu","Biru","Cyan","Hijau","Kuning","Orange","Merah","Pink","Putih"},
    Value   = "Ungu",
    Callback = function(v)
        pcall(function()
            local map = {
                Ungu=Color3.fromRGB(200,150,255), Biru=Color3.fromRGB(0,0,255),
                Cyan=Color3.fromRGB(0,255,255),   Hijau=Color3.fromRGB(0,255,0),
                Kuning=Color3.fromRGB(255,255,0), Orange=Color3.fromRGB(255,165,0),
                Merah=Color3.fromRGB(255,0,0),    Pink=Color3.fromRGB(255,105,180),
                Putih=Color3.fromRGB(255,255,255),
            }
            UnhookRing.Color = map[v] or Color3.fromRGB(200,150,255)
        end)
    end
})
