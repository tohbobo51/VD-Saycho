...
-- ── VEIL KILLER REMOTES (v18) ──────────────────────────────
local R_VeilVFX        = safeRemote("Killers", "Veil", "vfx")
local R_VeilUpdateWep  = safeRemote("Killers", "Veil", "updatewep")
local R_VeilSpear      = safeRemote("Killers", "Veil", "Spearthrow")
...
local function OpenColorPicker(defaultColor, onChanged, swatchRef)
    ...
    closeX.Activated:Connect(function() pop:Destroy() end)
...
SubLabel.Text = "Violence District  v18.0"; SubLabel.Position = UDim2.new(0, 82, 0, 16)
...
-- Silent Aim Spear: tembak Spearthrow ke arah survivor terdekat
local veilSpearConn = nil
local function startVeilSilentAim()
    if veilSpearConn then veilSpearConn:Disconnect() end
    veilSpearConn = task.spawn(function()
        while States.VeilSilentAim do
            pcall(function()
                ...
                if target and target.Character then
                    ...
                    if tHRP then
                        -- Hitung arah unit vector ke target
                        local dir = (tHRP.Position - myHRP.Position).Unit
                        -- Kirim Spearthrow: args = {1, true, false} + arah + jarak
                        R_VeilVFX and pcall(function() R_VeilVFX:FireServer(1, true, false) end)
                        R_VeilSpear:FireServer(Vector3.new(dir.X, dir.Y, dir.Z), dist)
                    end
                end
            end)
            task.wait(States.VeilSpearDelay)
        end
    end)
end
local function stopVeilSilentAim()
    States.VeilSilentAim = false
    -- veilSpearConn adalah task.spawn thread, akan berhenti sendiri lewat States check
end

-- Kill Aura: BasicAttack terus ke survivor terdekat dalam radius
local veilAuraConn = nil
local function startVeilKillAura()
    if veilAuraConn then task.cancel(veilAuraConn) end
    veilAuraConn = task.spawn(function()
        while States.VeilKillAura do
            pcall(function()
                if not R_BasicAttack then return end
                local myChar = LocalPlayer.Character; if not myChar then return end
                local myHRP = myChar:FindFirstChild("HumanoidRootPart"); if not myHRP then return end
                local target, dist = getNearestSurvivor(myHRP.Position, States.VeilAuraRadius)
                if target then
                    R_BasicAttack:FireServer(true)
                end
            end)
            task.wait(0.3)
        end
...
print("[VD v18.0] Loaded — Saycho | Veil: Silent Aim Spear + Kill Aura + VFX Spam")
