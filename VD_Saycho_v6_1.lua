...
║   VERSION : 6.1 — Full Fix + Gameplay Adjusted       ║
║   BY      : Saycho                                   ║
╠══════════════════════════════════════════════════════╣
║  FIX v6.1 :                                          ║
║  [1] AutoHeal   — cooldown 4s (was: setiap frame)    ║
║  [2] AntiChase  — cooldown 2s (was: setiap frame)    ║
║  [3] AutoGen    — interval 0.1s + cek jarak 10 studs ║
║  [4] FastVault  — loop heartbeat 0.2s (was: 1x fire) ║
║  [5] AutoRepair — cooldown 1s (was: setiap frame)    ║
║  [6] GetGen     — multi-path fallback (robust)       ║
║  [7] attackTimer— dipisah FastAttack vs AutoLunge    ║
╚══════════════════════════════════════════════════════╝
]]
...
local Remote = {
    ...
    -- PLACEHOLDER — isi setelah dapat dari RemoteSpy:
    -- Lever        = Remotes:WaitForChild("Lever"):WaitForChild("???"),
    -- Hook         = Remotes:WaitForChild("Hook"):WaitForChild("???"),
    -- Unhook       = Remotes:WaitForChild("Hook"):WaitForChild("Unhook"),
    -- Pallet       = Remotes:WaitForChild("Pallet"):WaitForChild("???"),
    -- Door         = Remotes:WaitForChild("Door"):WaitForChild("OpenDoor"),
}
...
local function SafeFire(remote, ...)
    ...
    if not ok then warn("[VD v6.1] Remote error: " .. tostring(err)) end
end
...
-- ============================================================
-- TIMER COOLDOWNS (FIX: semua fitur punya CD masing-masing)
-- ============================================================
local Timers = {
    heal    = 0,   CD_heal    = 4.0,  -- Auto Heal: 4 detik
    chase   = 0,   CD_chase   = 2.0,  -- Anti Chase: 2 detik
    repair  = 0,   CD_repair  = 0.1,  -- Auto Generator: 0.1 detik (cepat tapi ada jarak check)
    vault   = 0,   CD_vault   = 0.2,  -- Fast Vault: 0.2 detik
    attack  = 0,   CD_attack  = 0.3,  -- Fast Attack: 0.3 detik
    lunge   = 0,   CD_lunge   = 0.5,  -- Auto Lunge: 0.5 detik (terpisah dari attack)
    object  = 0,   CD_object  = 1.0,  -- Auto Repair Object: 1 detik
}
...
-- ============================================================
-- UI HELPER: SETTING CHIPS (LayoutOrder integer *10)
-- ============================================================
...
-- ============================================================
-- ESP SYSTEM (conn:Disconnect — bukan :Destroy)
-- ============================================================
local function RemoveESP(player)
    if ESPObjects[player] then
        local objs = ESPObjects[player]
        if objs[1] and objs[1].Parent then objs[1]:Destroy() end  -- Highlight
        if objs[2] and objs[2].Parent then objs[2]:Destroy() end  -- BillboardGui
        if objs[3] then objs[3]:Disconnect() end                   -- Heartbeat conn
...
ArrowUp.Activated:Connect(function()
...
ArrowDown.Activated:Connect(function()
    local cf = Camera.CFrame.LookVector
    SetLockedDir(-Vector3.new(cf.X, 0, cf.Z).Unit)
...
ArrowCenter.Activated:Connect(function()
...
-- ============================================================
-- GET GENERATORS (FIX: multi-path fallback)
-- ============================================================
local function GetGenerators()
    local list = {}
    -- Coba berbagai path yang mungkin ada di setiap map VD
    local candidates = {
    ...
    for _, folder in ipairs(candidates) do
        if folder then
            for _, child in ipairs(folder:GetChildren()) do
                -- Ambil semua yang namanya mengandung "Generator" atau "Gen"
                if child.Name:lower():find("generator") or child.Name:lower():find("genpoint") then
                    table.insert(list, child)
                end
            end
            if #list > 0 then break end  -- Berhenti di path pertama yang berhasil
        end
    end
    return list
end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if States.NoStamina and hum then
        for _, vname in ipairs({"Stamina","stamina","SP","StaminaValue"}) do
            ...
            if val and val:IsA("NumberValue") then
                val.Value = 100  -- FIX: tidak pakai val.MaxValue (tidak ada)
            end
        end
    end

    -- ── [3] Direction Lock / Moonwalk ───────────────────────
    -- FIX: CFrame.new(pos, target) langsung — valid dan benar
    ...
    if (States.AutoGenerator or States.FastGen)
        ...
        pcall(function()
            ...
            for _, gen in ipairs(gens) do
                -- FIX: cek jarak dulu, harus ≤ 10 studs dari generator
    ...
    -- ── [8] Fast Vault — 0.2 detik loop ────────────────────
    -- FIX: sekarang loop, bukan cuma 1x fire
    if States.FastVault and Timers.vault >= Timers.CD_vault then
        Timers.vault = 0
        SafeFire(Remote.FastVault, LocalPlayer)
    end

    -- ── [9] Auto Repair Objects — cooldown 1 detik ─────────
...
AddDropdownFeature(SurPage, "⚙️  Generator Rush", {
    { Name = "Auto Generator Rush",
      Callback = function(s) States.AutoGenerator = s end },
    { Name = "Fast Generator (Speed 1.5x)",
      Callback = function(s) States.FastGen = s end },
}, 20)

AddDropdownFeature(SurPage, "🔧  Auto Lever", {
    { Name = "Auto Lever  [Butuh Remote Lever]",
      Callback = function(s) States.AutoLever = s end },
    { Name = "Fast Lever  [Butuh Remote Lever]",
      Callback = function(s) end },
}, 40)

AddDropdownFeature(SurPage, "💊  Heal", {
    { Name = "Auto Heal Self",
      Callback = function(s) States.AutoHeal = s end },
    { Name = "Auto Heal Others  [Butuh Remote]",
      Callback = function(s) end },
}, 60)

AddDropdownFeature(SurPage, "🏃  Movement", {
    { Name = "Fast Vault",
      Callback = function(s) States.FastVault = s end },
    { Name = "No Cooldown",
      Callback = function(s) States.NoCD = s end },
    { Name = "Direction Lock (Moonwalk)",
      Callback = function(s)
    ...
    { Name = "Anti Chase",
      Callback = function(s) States.AntiChase = s end },
}, 80)

AddDropdownFeature(SurPage, "🛠️  Repair", {
    { Name = "Auto Repair Objects",
      Callback = function(s) States.AutoRepair = s end },
}, 100)
...
AddDropdownFeature(KilPage, "⚔️  Attack", {
    { Name = "Fast Attack",
      Callback = function(s) States.FastAttack = s end },
    { Name = "Auto Lunge",
      Callback = function(s) States.AutoLunge = s end },
    { Name = "Power Deactivate (1x fire)",
      Callback = function(s)
...
AddDropdownFeature(KilPage, "👁️  Tracking", {
    { Name = "Survivor Aura  [Butuh Remote]",
      Callback = function(s) end },
    { Name = "Snap Target (Beta)  [Butuh Remote]",
      Callback = function(s) end },
}, 40)
...
WaterText.Text                   = " 🪐 Lynxx VD v6.1  |  Saycho"
...
-- ============================================================
print("✅ VD Script v6.1 Loaded | By Saycho")
print("   Fix: AutoHeal CD 4s | AntiChase CD 2s | Gen jarak check")
print("   Fix: FastVault loop | AutoRepair CD 1s | lunge timer terpisah")
