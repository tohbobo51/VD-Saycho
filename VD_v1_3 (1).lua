...
-- ── Fungsi unhook utama ──────────────────────────────────────────
local function tryFireUnhook(hook)
    -- Coba berbagai path remote yang mungkin dipakai VD
    local remotes = {
        function() ReplicatedStorage.Remotes.Carry.HookEvent:FireServer(hook) end,
        function() ReplicatedStorage.Remotes.Unhook:FireServer(hook) end,
        function()
            local r = ReplicatedStorage.Remotes.Carry:FindFirstChild("Unhook")
            if r then r:FireServer(hook) end
        end,
    }
    for _, fn in ipairs(remotes) do
        pcall(fn)
    end
end
            ...
            if hrp and map then
                ...
                if hookFolder then
                    for _, hook in pairs(hookFolder:GetChildren()) do
                        ...
                        -- Ambil posisi hook (BasePart atau HookPoint)
                        local hookPart = hook:IsA("BasePart") and hook
                        ...
                        if hookPart then
                            ...
                            if dist <= _G.UnhookRange then
                                -- Cek ada survivor yang tergantung
                                local hasVictim = false
                                for _, pl in ipairs(Players:GetPlayers()) do
                                    if pl ~= LocalPlayer and pl.Character then
                                        local plHRP = pl.Character:FindFirstChild("HumanoidRootPart")
                                        if plHRP and (plHRP.Position - hookPart.Position).Magnitude < 5 then
                                ...
                                if hasVictim then
                                    ...
                                    WindUI:Notify({
                                        Title    = "Auto Unhook",
                                        Content  = "Hook ditemukan & remote dikirim!",
                                        Duration = 1.5,
                                        Icon     = "anchor"
                                    })
                                end
...
GriefTab:Button({
    ...
    Callback    = function()
        local count = 0
        -- Cari semua PalletPointSlide di seluruh workspace secara rekursif
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj.Name == "PalletPointSlide" then
                pcall(function()
                    ReplicatedStorage
                        ...
                        :WaitForChild("Pallet")
                        :WaitForChild("PalletDropEvent")
                        :FireServer(obj)
                    count = count + 1
                end)
                task.wait(0.05) -- kecil delay agar tidak flood server sekaligus
            end
        end
        WindUI:Notify({
            Title    = "Grief",
            Content  = count .. " Pallet dijatuhkan!",
            Duration = 3,
            Icon     = "zap"
        })
    end
