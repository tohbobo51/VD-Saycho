...
-- ════ SELF HEAL — Godmode approach, TIDAK kebal ════════════════
-- HP drop ke threshold → set balik max + bangun dari knocked
-- Beda GodMode: GodMode TERUS set max (kebal total)
-- Self Heal: hanya set max saat HP drop, lalu normal lagi → bisa kena damage
...
local function startSelfHeal()
    ...
    selfHealConn = RunService.Heartbeat:Connect(function()
        ...
        if hum.Health <= selfHealThreshold then
            -- Set HP penuh (sama seperti GodMode, tapi hanya 1x saat drop)
            hum.Health = hum.MaxHealth
            -- Bangun dari knocked state
            pcall(function()
                hum:ChangeState(Enum.HumanoidStateType.GettingUp)
            end)
            -- Backup: fire HealEvent remote sekalian
            pcall(function()
