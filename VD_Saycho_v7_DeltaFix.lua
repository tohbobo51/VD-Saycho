...
║   VERSION : 7.0 — DELTA EXECUTOR FIX                    ║
║   BY      : Saycho                                       ║
╠══════════════════════════════════════════════════════════╣
║  FIX v7.0 (Delta Compatible):                           ║
║  1. ScreenGui parent → PlayerGui (CoreGui diblokir       ║
║     Delta secara silent, GUI tidak muncul)               ║
║  2. VirtualInputManager → full pcall guard               ║
║  3. SC_PressSpace pakai keypress fallback                ║
╚══════════════════════════════════════════════════════════╝
]]
...
-- CLEANUP
-- ============================================================
-- Bersihkan GUI lama dari semua kemungkinan parent
for _, parent in ipairs({CoreGui, PlayerGui}) do
    local old = parent:FindFirstChild("VD_Saycho_Official")
    if old then old:Destroy() end
end

-- Delta sering silent-block CoreGui → pakai PlayerGui sebagai parent utama
local ScreenGui
...
-- ============================================================
-- REMOTE TABLE — KOSONG DULU, diisi task.spawn di bawah
-- SafeFire sudah handle nil, jadi aman
-- ============================================================
local Remote = {}
...
local function SafeFire(remote, ...)
    ...
    if not ok then warn("[VD] Remote error: " .. tostring(err)) end
end
...
DragZone.Size = UDim2.new(1, -50, 0, 50); DragZone.Position = UDim2.new(0,0,0,0)
DragZone.BackgroundTransparency = 1; DragZone.Text = ""; DragZone.ZIndex = 1
MakeDraggable(MainFrame, DragZone)

local TopGrip = Instance.new("Frame", MainFrame)
TopGrip.Size = UDim2.new(0,60,0,4); TopGrip.Position = UDim2.new(0.5,-30,0,7)
TopGrip.BackgroundColor3 = Color3.fromRGB(255,140,0); TopGrip.BorderSizePixel = 0
TopGrip.ZIndex = 2; Instance.new("UICorner", TopGrip)

local LynxxTitle = Instance.new("TextLabel", MainFrame)
LynxxTitle.Text = "Lynxx 🪐"; LynxxTitle.Position = UDim2.new(0,15,0,16)
LynxxTitle.Size = UDim2.new(0,90,0,22); LynxxTitle.TextColor3 = Color3.fromRGB(255,140,0)
LynxxTitle.Font = "GothamBold"; LynxxTitle.TextSize = 16
LynxxTitle.BackgroundTransparency = 1; LynxxTitle.TextXAlignment = "Left"; LynxxTitle.ZIndex = 3

local Sep = Instance.new("Frame", MainFrame)
Sep.Size = UDim2.new(0,1,0,18); Sep.Position = UDim2.new(0,110,0,20)
Sep.BackgroundColor3 = Color3.fromRGB(60,60,60); Sep.BorderSizePixel = 0; Sep.ZIndex = 3

local GameTitle = Instance.new("TextLabel", MainFrame)
GameTitle.Text = "Violence District"; GameTitle.Position = UDim2.new(0,118,0,16)
...
GameTitle.BackgroundTransparency = 1; GameTitle.TextXAlignment = "Left"; GameTitle.ZIndex = 3

local MinBtn = Instance.new("TextButton", MainFrame)
MinBtn.Size = UDim2.new(0,32,0,32); MinBtn.Position = UDim2.new(1,-44,0,9)
MinBtn.BackgroundColor3 = Color3.fromRGB(35,35,35); MinBtn.Text = "—"
MinBtn.TextColor3 = Color3.new(1,1,1); MinBtn.Font = "GothamBold"
MinBtn.TextSize = 16; MinBtn.BorderSizePixel = 0; MinBtn.ZIndex = 5
...
ResizeBtn.Size = UDim2.new(0,18,0,18); ResizeBtn.Position = UDim2.new(1,-22,1,-22)
ResizeBtn.BackgroundColor3 = Color3.fromRGB(50,50,50); ResizeBtn.Text = "⊡"
ResizeBtn.TextColor3 = Color3.fromRGB(90,90,90); ResizeBtn.TextSize = 11
ResizeBtn.BorderSizePixel = 0; ResizeBtn.ZIndex = 5
...
Sidebar.ZIndex = 4; Instance.new("UICorner", Sidebar)
...
Container.BackgroundTransparency = 1; Container.ZIndex = 4
...
local function CreatePage(name)
    ...
    pg.Visible = false; pg.ScrollBarThickness = 3
    pg.ScrollBarImageColor3 = Color3.fromRGB(255,140,0)
    pg.ScrollBarImageTransparency = 0.4; pg.BorderSizePixel = 0
...
local function AddTab(name)
    ...
    btn.Text = "   "..name; btn.TextColor3 = Color3.fromRGB(120,120,120)
    btn.Font = "GothamMedium"; btn.TextSize = 13
    btn.TextXAlignment = "Left"; btn.BorderSizePixel = 0; btn.ZIndex = 5
    local ind = Instance.new("Frame", btn)
    ind.Size = UDim2.new(0,3,0,18); ind.Position = UDim2.new(0,6,0.5,-9)
    ind.BackgroundColor3 = Color3.fromRGB(255,140,0); ind.Visible = false
    ind.BorderSizePixel = 0; Instance.new("UICorner", ind)
    btn.MouseButton1Click:Connect(function()
        for _,p in pairs(Pages) do p.Visible = false end
        for _,t in ipairs(TabButtons) do
            t.btn.TextColor3 = Color3.fromRGB(120,120,120); t.ind.Visible = false
        end
...
SurPage.Visible = true; surBtn.TextColor3 = Color3.new(1,1,1); surInd.Visible = true
...
local function SectionHeader(parent, text, order)
    ...
    lbl.TextColor3 = Color3.fromRGB(255,140,0); lbl.Font = "GothamBold"
...
local function CreateToggle(parent, labelText, defaultState, callback, order)
    ...
    row.Size = UDim2.new(1,-10,0,44); row.BackgroundColor3 = Color3.fromRGB(25,25,25)
    row.BorderSizePixel = 0; row.LayoutOrder = order; Instance.new("UICorner", row)
    local lbl = Instance.new("TextLabel", row)
    lbl.Text = "  "..labelText; lbl.Size = UDim2.new(1,-62,1,0)
    ...
    track.BackgroundColor3 = defaultState and Color3.fromRGB(255,140,0) or Color3.fromRGB(50,50,50)
...
local function AddDropdownFeature(parent, title, options_table, order)
    ...
    frame.Size = UDim2.new(1,-10,0,44); frame.BackgroundColor3 = Color3.fromRGB(25,25,25)
    frame.Text = ""; frame.BorderSizePixel = 0; frame.LayoutOrder = order
    Instance.new("UICorner", frame)
    local label = Instance.new("TextLabel", frame)
    label.Text = "  "..title; label.Size = UDim2.new(1,-38,1,0)
    ...
    arrow.Text = "▼  "; arrow.Size = UDim2.new(0,32,1,0); arrow.Position = UDim2.new(1,-32,0,0)
    arrow.TextColor3 = Color3.fromRGB(255,140,0); arrow.BackgroundTransparency = 1
    arrow.TextXAlignment = "Right"; arrow.Font = "GothamBold"; arrow.TextSize = 13
    local isOpen = false
    local listFrame = Instance.new("Frame", parent)
    listFrame.Size = UDim2.new(1,-10,0,0); listFrame.BackgroundColor3 = Color3.fromRGB(28,28,28)
    listFrame.ClipsDescendants = true; listFrame.BorderSizePixel = 0
    listFrame.LayoutOrder = order+1; Instance.new("UICorner", listFrame)
    ...
    for _, opt in ipairs(options_table) do
        ...
        b.Size = UDim2.new(1,0,0,34); b.BackgroundColor3 = Color3.fromRGB(33,33,33)
        b.Text = "  "..opt.Name; b.TextColor3 = Color3.fromRGB(175,175,175)
        b.Font = "Gotham"; b.TextSize = 12; b.TextXAlignment = "Left"; b.BorderSizePixel = 0
        local stateOn = false
        b.MouseButton1Click:Connect(function()
            stateOn = not stateOn
            TweenService:Create(b, TweenInfo.new(0.12), {
                BackgroundColor3 = stateOn and Color3.fromRGB(55,38,10) or Color3.fromRGB(33,33,33)
            }):Play()
            b.TextColor3 = stateOn and Color3.fromRGB(255,140,0) or Color3.fromRGB(175,175,175)
...
local function CreateSettingDropdown(parent, title, icon, options_list, onSelect, order)
    ...
    titleRow.LayoutOrder = base
    local tLbl = Instance.new("TextLabel", titleRow)
    tLbl.Text = icon.."  "..title; tLbl.Size = UDim2.new(1,0,1,0)
    tLbl.TextColor3 = Color3.fromRGB(180,180,180); tLbl.Font = "GothamMedium"
    tLbl.TextSize = 12; tLbl.BackgroundTransparency = 1; tLbl.TextXAlignment = "Left"
    local optRow = Instance.new("Frame", parent)
    optRow.Size = UDim2.new(1,-10,0,36); optRow.BackgroundTransparency = 1
    optRow.LayoutOrder = base+5
    ...
    for i, opt in ipairs(options_list) do
        ...
        b.BackgroundColor3 = opt.default and Color3.fromRGB(255,140,0) or Color3.fromRGB(30,30,30)
        b.Text = opt.label
        b.TextColor3 = opt.default and Color3.new(1,1,1) or Color3.fromRGB(155,155,155)
        ...
        b.MouseButton1Click:Connect(function()
            ...
            TweenService:Create(b, TweenInfo.new(0.15), {BackgroundColor3=Color3.fromRGB(255,140,0)}):Play()
    ...
    div.BorderSizePixel = 0; div.LayoutOrder = base+9
    return optRow
end
...
local function RemoveESP(player)
    if ESPObjects[player] then
        ...
        if o[1] and o[1].Parent then o[1]:Destroy() end
        if o[2] and o[2].Parent then o[2]:Destroy() end
        if o[3] then o[3]:Disconnect() end
...
local function CreateESP(player)
    ...
    local function applyToChar(char)
        ...
        hl.FillColor=Color3.fromRGB(0,100,255); hl.FillTransparency=0.55
        hl.OutlineColor=Color3.fromRGB(80,160,255); hl.OutlineTransparency=0
        hl.Adornee=char; hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop
        hl.Enabled=States.EspEnabled; hl.Parent=char
        local bb = Instance.new("BillboardGui")
        bb.Size=UDim2.new(0,130,0,55); bb.StudsOffset=Vector3.new(0,3.2,0)
        bb.AlwaysOnTop=true; bb.MaxDistance=500; bb.Enabled=States.EspEnabled; bb.Parent=hrp
        local function lbl(bb2, txt, col, py, ts)
            local l = Instance.new("TextLabel", bb2)
            l.Text=txt; l.Size=UDim2.new(1,0,0,ts+4); l.Position=UDim2.new(0,0,0,py)
            l.TextColor3=col; l.Font="GothamBold"; l.TextSize=ts
            l.BackgroundTransparency=1; l.TextStrokeTransparency=0
            l.TextStrokeColor3=Color3.new(0,0,0); return l
        end
        local nameLbl = lbl(bb, player.Name, Color3.fromRGB(80,180,255), 0, 13)
        local hpLbl   = lbl(bb, "[ 100 HP ]", Color3.fromRGB(255,80,80), 19, 12)
        local distLbl = lbl(bb, "[ 0 MM ]",   Color3.fromRGB(200,200,200), 36, 11)
        local conn = RunService.Heartbeat:Connect(function()
            if not States.EspEnabled then hl.Enabled=false; bb.Enabled=false; return end
            hl.Enabled=true; bb.Enabled=true
            if hum and hum.Parent then hpLbl.Text="[ "..math.floor(hum.Health).." HP ]" end
            local m = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if hrp and hrp.Parent and m then
                distLbl.Text="[ "..math.floor((hrp.Position-m.Position).Magnitude).." MM ]"
            end
        end)
...
Players.PlayerAdded:Connect(function(p) if States.EspEnabled then CreateESP(p) end end)
...
DirLockFrame.Size=UDim2.new(0,110,0,110); DirLockFrame.Position=UDim2.new(1,-130,0.5,-55)
DirLockFrame.BackgroundTransparency=1; DirLockFrame.Visible=false; DirLockFrame.ZIndex=8
MakeDraggable(DirLockFrame, DirLockFrame)

local function MakeArrowBtn(sym, px, py)
    local b = Instance.new("TextButton", DirLockFrame)
    b.Size=UDim2.new(0,38,0,38); b.Position=UDim2.new(0,px,0,py)
    b.BackgroundColor3=Color3.fromRGB(20,20,20); b.BackgroundTransparency=0.25
    b.Text=sym; b.TextColor3=Color3.new(1,1,1); b.Font="GothamBold"
    b.TextSize=20; b.BorderSizePixel=0; b.ZIndex=9
    Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); return b
end

local ArrowUp=MakeArrowBtn("▲",36,0); local ArrowDown=MakeArrowBtn("▼",36,72)
local ArrowLeft=MakeArrowBtn("◄",0,36); local ArrowRight=MakeArrowBtn("►",72,36)

local ArrowCenter = Instance.new("TextButton", DirLockFrame)
ArrowCenter.Size=UDim2.new(0,28,0,28); ArrowCenter.Position=UDim2.new(0,41,0,41)
ArrowCenter.BackgroundColor3=Color3.fromRGB(255,140,0); ArrowCenter.BackgroundTransparency=0.3
ArrowCenter.Text="✕"; ArrowCenter.TextColor3=Color3.new(1,1,1); ArrowCenter.Font="GothamBold"
ArrowCenter.TextSize=14; ArrowCenter.BorderSizePixel=0; ArrowCenter.ZIndex=9
Instance.new("UICorner",ArrowCenter).CornerRadius=UDim.new(1,0)

local function SetLockedDir(vec)
    LockedLookVector=vec
    for _,b in ipairs({ArrowUp,ArrowDown,ArrowLeft,ArrowRight}) do
        b.BackgroundTransparency=0.25; b.TextColor3=Color3.new(1,1,1)
    end
end

ArrowUp.MouseButton1Click:Connect(function()
    local c=Camera.CFrame.LookVector; SetLockedDir(Vector3.new(c.X,0,c.Z).Unit)
    ArrowUp.BackgroundColor3=Color3.fromRGB(255,140,0); ArrowUp.BackgroundTransparency=0
end)
ArrowDown.MouseButton1Click:Connect(function()
    local c=Camera.CFrame.LookVector; SetLockedDir(-Vector3.new(c.X,0,c.Z).Unit)
    ArrowDown.BackgroundColor3=Color3.fromRGB(255,140,0); ArrowDown.BackgroundTransparency=0
end)
ArrowLeft.MouseButton1Click:Connect(function()
    local r=Camera.CFrame.RightVector; SetLockedDir(-Vector3.new(r.X,0,r.Z).Unit)
    ArrowLeft.BackgroundColor3=Color3.fromRGB(255,140,0); ArrowLeft.BackgroundTransparency=0
end)
ArrowRight.MouseButton1Click:Connect(function()
    local r=Camera.CFrame.RightVector; SetLockedDir(Vector3.new(r.X,0,r.Z).Unit)
    ArrowRight.BackgroundColor3=Color3.fromRGB(255,140,0); ArrowRight.BackgroundTransparency=0
end)
...
local function GetGenerators()
    ...
    for _,folder in ipairs(cands) do
        if folder then
...
RunService.Heartbeat:Connect(function(dt)
    ...
    for k in pairs(Timers) do
        if type(Timers[k])=="number" and not k:find("CD_") then Timers[k]=Timers[k]+dt end
    end
    if States.AutoHeal and hum and hum.Health<hum.MaxHealth and Timers.heal>=Timers.CD_heal then
        Timers.heal=0; SafeFire(Remote.HealReset, LocalPlayer)
    end
    if States.NoStamina and hum then
        for _,vn in ipairs({"Stamina","stamina","SP","StaminaValue"}) do
            local v=char:FindFirstChild(vn) or hum:FindFirstChild(vn)
            if v and v:IsA("NumberValue") then v.Value=100 end
        end
    end
    if States.DirLockActive and LockedLookVector and hrp then
        if hum then hum.AutoRotate=false end
        local p=hrp.Position; hrp.CFrame=CFrame.new(p, p+LockedLookVector)
    elseif not States.DirLockActive and hum then hum.AutoRotate=true end
    if States.AntiChase and hrp and Timers.chase>=Timers.CD_chase then
        Timers.chase=0; SafeFire(Remote.Chase, char, false)
    end
    if States.FastAttack and Timers.attack>=Timers.CD_attack then
        Timers.attack=0; SafeFire(Remote.BasicAttack, false)
    end
    if States.AutoLunge and Timers.lunge>=Timers.CD_lunge then
        Timers.lunge=0; SafeFire(Remote.Lunge, false)
    end
    if (States.AutoGenerator or States.FastGen) and Timers.repair>=Timers.CD_repair and hrp then
        Timers.repair=0
        pcall(function()
            for _,gen in ipairs(GetGenerators()) do
                local gp=gen:IsA("Model")
                    and (gen.PrimaryPart and gen.PrimaryPart.Position or gen:GetPivot().Position)
                    or gen.Position
                if (gp-hrp.Position).Magnitude<=10 then
                    SafeFire(Remote.RepairEvent, gen, false, States.FastGen and 1.5 or nil)
                end
            end
        end)
    end
    if States.FastVault and Timers.vault>=Timers.CD_vault then
        Timers.vault=0; SafeFire(Remote.FastVault, LocalPlayer)
    end
    if States.AutoRepair and Timers.object>=Timers.CD_object then
        Timers.object=0; SafeFire(Remote.EnableCollision)
    end
end)
...
AddDropdownFeature(SurPage, "🏃  Movement", {
    ...
    {Name="Direction Lock (Moonwalk)", Callback=function(s)
        States.DirLockActive=s; DirLockFrame.Visible=s
        if not s then
            LockedLookVector=nil
...
CreateToggle(EspPage, "Enable ESP (Highlight + Info)", false, function(s) ToggleESP(s) end, 20)
...
CreateToggle(PlrPage, "No Stamina (Full Stamina)", false, function(s) States.NoStamina=s end, 20)
...
CreateSettingDropdown(SetPage, "Icon Size", "📐", {
    ...
    local s={small=35,medium=55,large=80}
...
WaterMark.Size=UDim2.new(0,230,0,26); WaterMark.Position=UDim2.new(0,10,1,-36)
WaterMark.BackgroundColor3=Color3.fromRGB(18,18,18); WaterMark.BackgroundTransparency=0.25
WaterMark.BorderSizePixel=0; WaterMark.ZIndex=5
Instance.new("UICorner",WaterMark).CornerRadius=UDim.new(0,7)
local WaterText = Instance.new("TextLabel", WaterMark)
WaterText.Text=" 🪐 Lynxx VD v6.3 + SC  |  Saycho"
WaterText.Size=UDim2.new(1,0,1,0); WaterText.TextColor3=Color3.fromRGB(255,140,0)
WaterText.Font="GothamBold"; WaterText.TextSize=11
WaterText.BackgroundTransparency=1; WaterText.TextXAlignment="Left"; WaterText.ZIndex=6
...
local function SC_PressSpace()
    if VirtualInputManager then
        ...
        -- Fallback: simulasi keypress pakai cara lain yang didukung Delta
        pcall(function()
            local inputObj = {
...
local function SC_OnVisible(Check, Line, Goal)
    if LocalPlayer.Team and LocalPlayer.Team.Name~="Survivors" then
        if SC_Conn then SC_Conn:Disconnect(); SC_Conn=nil end; return
    end
    if Check.Visible then
        if SC_Conn then SC_Conn:Disconnect() end
        SC_Conn = RunService.Heartbeat:Connect(SC_MakeHB(Line, Goal))
    else
        if SC_Conn then SC_Conn:Disconnect(); SC_Conn=nil end
    end
end

local function SC_Setup(gui)
    local Check=gui:WaitForChild("Check",10); if not Check then return end
    local Line=Check:WaitForChild("Line",10); local Goal=Check:WaitForChild("Goal",10)
    if not Line or not Goal then return end
    Check:GetPropertyChangedSignal("Visible"):Connect(function() SC_OnVisible(Check,Line,Goal) end)
    print("[SkillCheck] ✅ Ready!")
end

local ex=PlayerGui:FindFirstChild("SkillCheckPromptGui")
if ex then task.spawn(SC_Setup, ex) end
PlayerGui.ChildAdded:Connect(function(c)
    if c.Name=="SkillCheckPromptGui" then task.spawn(SC_Setup, c) end
end)

-- ============================================================
-- REMOTE LOADER — dijalankan di background SETELAH GUI muncul
-- GUI sudah tampil duluan, remote load belakangan
-- SafeFire() aman dengan nil, tidak akan error
-- ============================================================
print("✅ VD v6.3 GUI Loaded | Remote loading di background...")
...
task.spawn(function()
    local ok, err = pcall(function()
        ...
        local function w(parent, name)
            local r = parent:WaitForChild(name, 8)
            if not r then warn("[VD] Remote tidak ada: "..name) end
            return r
        end

        Remote.EmoteHandler    = w(Remotes, "EmoteHandler")
        Remote.Chase           = (function() local f=w(Remotes,"Chase"); return f and w(f,"Runevent") end)()
        Remote.HealReset       = (function() local f=w(Remotes,"Healing"); return f and w(f,"Reset") end)()
        Remote.UpdateLook      = (function() local f=w(Remotes,"Game"); return f and w(f,"UpdateCharacterLook") end)()
        Remote.BasicAttack     = (function() local f=w(Remotes,"Attacks"); return f and w(f,"BasicAttack") end)()
        Remote.Lunge           = (function() local f=w(Remotes,"Attacks"); return f and w(f,"Lunge") end)()
        Remote.FastVault       = (function() local f=w(Remotes,"Window"); return f and w(f,"fastvault") end)()
        Remote.EnableCollision = (function() local f=w(Remotes,"Collision"); return f and w(f,"EnableCollision") end)()
        Remote.RepairEvent     = (function() local f=w(Remotes,"Generator"); return f and w(f,"RepairEvent") end)()
        Remote.PowerDeact      = (function()
            local a=w(Remotes,"Killers"); local b=a and w(a,"Killer")
            return b and w(b,"PowerDoneDeactivating")
        end)()
    end)
    if not ok then warn("[VD] LoadRemotes error: "..tostring(err)) end
    print("[VD] ✅ Remotes loaded! Semua fitur aktif.")
end)
