...
local function IsKiller(pl)  return pl and pl.Team and pl.Team.Name == "Killer" end
local function IsSurvivor(pl)return pl and pl.Team and pl.Team.Name == "Survivors" end

local NEX_Cache = { Generators={}, Gates={}, Hooks={}, Pallets={}, Windows={}, ClosestHook=nil, ExitPos=nil }

local function NEX_ScanMap()
    local map = Workspace:FindFirstChild("Map")
    if not map then return end
    local gens,gates,hooks,pallets,windows = {},{},{},{},{}
    local exitPos = nil
    ...
    for _,obj in ipairs(map:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if part then
                local n = obj.Name
                if n=="Generator" then table.insert(gens,{model=obj,part=part})
                elseif n=="Gate" then table.insert(gates,{model=obj,part=part})
                elseif n=="Hook" then table.insert(hooks,{model=obj,part=part})
                elseif n=="Palletwrong" or n:lower():find("pallet") then table.insert(pallets,{model=obj,part=part})
                elseif n=="Window" then table.insert(windows,{model=obj,part=part}) end
            end
        end
    end
    NEX_Cache.Generators=gens; NEX_Cache.Gates=gates; NEX_Cache.Hooks=hooks
    NEX_Cache.Pallets=pallets; NEX_Cache.Windows=windows; NEX_Cache.ExitPos=exitPos
    print("[VD v2.0] ScanMap: Generators="..#gens.." Hooks="..#hooks)
end
...
-- Scan map saat load & tiap respawn
task.spawn(function() task.wait(3); pcall(NEX_ScanMap) end)
LocalPlayer.CharacterAdded:Connect(function() task.wait(3); pcall(NEX_ScanMap) end)
-- Rescan tiap 30 detik (map bisa berubah)
task.spawn(function()
    while true do task.wait(30); pcall(NEX_ScanMap) end
end)
                ...
                while autoHealEnabled do
                    ...
                    if root then
                        ...
                        if closestTarget then
                            local args = {
                                "neutral",  -- argumen benar dari RemoteSpy (bukan "success")
                                0,          -- argumen benar dari RemoteSpy (bukan 1)
...
-- ============================================================
-- NEW FEATURES v2.0 — LOGIC FUNCTIONS
-- ============================================================

-- ── HELPER: cari musuh/survivor terdekat ─────────────────────
local function getNearestEnemy(fromPos, radius, targetTeam)
    local best, bestD = nil, radius or math.huge
    for _,pl in ipairs(Players:GetPlayers()) do
        if pl ~= LocalPlayer and pl.Character then
            local isTarget = targetTeam=="Killer" and IsKiller(pl) or targetTeam=="Survivor" and IsSurvivor(pl) or true
            if isTarget then
                local hrp = pl.Character:FindFirstChild("HumanoidRootPart")
                if hrp then
...
local function NEX_AutoAttack()
    if not _G.VD_AutoAttack or GetRole()~="Killer" then return end
    local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    _aaT = (_aaT or 0) + 0.016
    if _aaT < 0.4 then return end; _aaT = 0
    for _,pl in ipairs(Players:GetPlayers()) do
        if pl~=LocalPlayer and IsSurvivor(pl) and pl.Character then
            ...
            if tr and th and th.Health>0 and (tr.Position-root.Position).Magnitude<=(_G.VD_AutoAttackRange or 12) then
                pcall(function()
                    local b = ReplicatedStorage:FindFirstChild("Remotes") and
                              ReplicatedStorage.Remotes:FindFirstChild("Attacks") and
                              ReplicatedStorage.Remotes.Attacks:FindFirstChild("BasicAttack")
    ...
    for _,pl in ipairs(Players:GetPlayers()) do
        if pl~=LocalPlayer and IsSurvivor(pl) and pl.Character then
            ...
            if r and h and h.Health>0 then
                if not _hitboxOriginals[pl] then _hitboxOriginals[pl]=r.Size end
                local sz=_G.VD_HitboxSize or 15
                r.Size=Vector3.new(sz,sz,sz); r.CanCollide=false; r.Transparency=0.7
            end
        end
    end
end
...
local function NEX_DoubleTap()
    if not _G.VD_DoubleTap or GetRole()~="Killer" then return end
    _dtT=(_dtT or 0)+0.016
    if _dtT<0.5 then return end; _dtT=0
    pcall(function()
        local ba=ReplicatedStorage:FindFirstChild("Remotes") and
                 ReplicatedStorage.Remotes:FindFirstChild("Attacks") and
                 ReplicatedStorage.Remotes.Attacks:FindFirstChild("BasicAttack")
        if ba then ba:FireServer(false); task.wait(0.05); ba:FireServer(false) end
    end)
end

-- ── INFINITE LUNGE (Killer) ──────────────────────────────────
local function NEX_InfiniteLunge()
    if not _G.VD_InfLunge or GetRole()~="Killer" then return end
    local root=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if root then
        pcall(function() root.Velocity=root.CFrame.LookVector*100+Vector3.new(0,10,0) end)
    end
end

-- ── AUTO HOOK (Killer) ───────────────────────────────────────
local _isAutoHooking=false
local function NEX_AutoHook()
    if not _G.VD_AutoHook or GetRole()~="Killer" or _isAutoHooking then return end
    local root=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    -- Cari survivor yang knock (HP <= 25%)
        ...
        if pl~=LocalPlayer and IsSurvivor(pl) and pl.Character then
            ...
            if tr and hum then
                ...
                if pct<=0.25 and pct>0 then
                    ...
                    if d<tDist then tDist=d; target=tr end
                end
            end
        end
    end
    ...
    for _,h in ipairs(NEX_Cache.Hooks) do
        if h.part then
            local d=(h.part.Position-target.Position).Magnitude
            if d<hDist then hDist=d; closestHook=h end
        end
    end
    if not closestHook then return end
    _isAutoHooking=true
    task.spawn(function()
        pcall(function()
            root.CFrame=CFrame.new(target.Position+Vector3.new(0,3,0),target.Position)
            task.wait(0.25)
            local vim=game:GetService("VirtualInputManager")
            pcall(function() vim:SendKeyEvent(true,Enum.KeyCode.Space,false,game); task.wait(0.05); vim:SendKeyEvent(false,Enum.KeyCode.Space,false,game) end)
            task.wait(0.6)
            if root and root.Parent then
                root.CFrame=CFrame.new(closestHook.part.Position+Vector3.new(0,3,0))
                task.wait(0.25)
                pcall(function()
                    local evt=ReplicatedStorage:FindFirstChild("Remotes") and
                              ReplicatedStorage.Remotes:FindFirstChild("Carry") and
                              ReplicatedStorage.Remotes.Carry:FindFirstChild("HookEvent")
                    if evt then
                        local hp=closestHook.model and (closestHook.model:FindFirstChild("HookPoint") or closestHook.model:FindFirstChild("HookHitbox")) or closestHook.part
                        evt:FireServer(hp)
                    end
                end)
            end
        end)
        task.wait(1); _isAutoHooking=false
    end)
end
...
pcall(function()
    ...
    local fl=r and r:FindFirstChild("Items") and r.Items:FindFirstChild("Flashlight")
    ...
    if ok and mt and setreadonly then
        pcall(function()
            ...
            mt.__namecall=newcclosure(function(self,...)
                if not checkcaller() and _G.VD_AntiBlind and self==gb then
                    if getnamecallmethod()=="FireServer" and GetRole()=="Killer" then return nil end
                end
                return old(self,...)
            end)
            setreadonly(mt,true)
...
pcall(function()
    ...
    if ok and mt and setreadonly then
        pcall(function()
            ...
            local old=mt.__namecall
            mt.__namecall=newcclosure(function(self,...)
                if _G.VD_NoPalletStun and (self==stun or self==stunDrop) then return nil end
                return old(self,...)
            end)
            setreadonly(mt,true)
        end)
    ...
    if ok and mt and setreadonly then
        pcall(function()
            ...
            mt.__newindex=newcclosure(function(t,k,v)
                if not checkcaller() and _G.VD_NoSlowdown and GetRole()=="Killer" then
...
-- ── DESTROY PALLETS (Killer) ─────────────────────────────────
local _dpT=0
local function NEX_DestroyAllPallets()
    if not _G.VD_DestroyPallets or GetRole()~="Killer" then return end
    _dpT=(_dpT or 0)+0.016
    if _dpT<1.5 then return end; _dpT=0
    pcall(function()
        local j=ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("Pallet") and ReplicatedStorage.Remotes.Pallet:FindFirstChild("Jason")
        if not j then return end
        local dg=j:FindFirstChild("Destroy-Global"); local d=j:FindFirstChild("Destroy")
        if dg then pcall(function() dg:FireServer() end) end
        if d then for _,pallet in ipairs(NEX_Cache.Pallets) do if pallet.model then task.spawn(function() pcall(function() d:FireServer(pallet.model) end) end) end end end
    end)
end
...
local function NEX_FullGenBreak()
    if not _G.VD_FullGenBreak or GetRole()~="Killer" then return end
    _gbT=(_gbT or 0)+0.016
    if _gbT<0.8 then return end; _gbT=0
    pcall(function()
        local be=ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("Generator") and ReplicatedStorage.Remotes.Generator:FindFirstChild("BreakGenEvent")
...
-- ── BEAT KILLER (auto kill all survivors) ────────────────────
local _bkTarget=nil
local _bkT=0
local function NEX_BeatKiller()
    if not _G.VD_BeatKiller or GetRole()~="Killer" then _bkTarget=nil; return end
    local root=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    _bkT=(_bkT or 0)+0.016
    if _bkT<0.05 then return end; _bkT=0
    -- Validate/pick target
    if _bkTarget and _bkTarget.Character then
        local th=_bkTarget.Character:FindFirstChildOfClass("Humanoid")
        if not (th and th.Health>0) then _bkTarget=nil end
    else _bkTarget=nil end
    if not _bkTarget then
    ...
    if not (_bkTarget and _bkTarget.Character) then return end
    local tr=_bkTarget.Character:FindFirstChild("HumanoidRootPart")
    if not tr then _bkTarget=nil; return end
    ...
    pcall(function() root.CFrame=CFrame.new(tr.Position+dir*3+Vector3.new(0,1,0),tr.Position) end)
    pcall(function()
        local ba=ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("Attacks") and ReplicatedStorage.Remotes.Attacks:FindFirstChild("BasicAttack")
        if ba then ba:FireServer(false) end
    end)
end

-- ── FLEE KILLER / AUTO TELE AWAY (Survivor) ──────────────────
local _fleeLast=0
local function NEX_TeleAway()
    if not _G.VD_TeleAway or GetRole()~="Survivor" then return end
    if tick()-_fleeLast<3 then return end
    local root=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    ...
    if killerDist>(_G.VD_TeleAwayDist or 40) then return end
    _fleeLast=tick()
    local bestSpot,bestD=nil,0
    for _,gate in ipairs(NEX_Cache.Gates) do
        if gate.part and killerPos then
            local d=(gate.part.Position-killerPos).Magnitude
            if d>bestD then bestD=d; bestSpot=gate.part.Position end
        end
    end
    if not bestSpot then
        for _,gen in ipairs(NEX_Cache.Generators) do
            if gen.part and killerPos then
                ...
                if d>bestD then bestD=d; bestSpot=gen.part.Position end
            end
        end
    end
...
-- ── BEAT SURVIVOR (escape game auto) ─────────────────────────
local function NEX_BeatSurvivor()
    if not _G.VD_BeatSurvivor or GetRole()~="Survivor" then return end
    local root=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    -- Teleport ke exit/gate
    if NEX_Cache.ExitPos then
        NEX_TeleportToPosition(NEX_Cache.ExitPos)
    elseif #NEX_Cache.Gates>0 and NEX_Cache.Gates[1].part then
        NEX_TeleportToPosition(NEX_Cache.Gates[1].part.Position)
    end
end
...
local function NEX_FlingNearest()
    ...
    if not (best and best.Character) then return end
    ...
    task.spawn(function()
        for _=1,10 do
            pcall(function()
                ...
                root.Velocity=Vector3.new(_G.VD_FlingStrength or 10000,(_G.VD_FlingStrength or 10000)/2,_G.VD_FlingStrength or 10000)
                root.RotVelocity=Vector3.new(9999,9999,9999)
            end)
            task.wait()
        end
        pcall(function() root.CFrame=origCF; root.Velocity=Vector3.zero; root.RotVelocity=Vector3.zero end)
    end)
end
...
-- ── IMPROVED AUTOPARRY (v2.0 — working) ──────────────────────
local _parryLastTime=0
local _parryLastWarn=0
local function NEX_AutoParry()
    if not _G.VD_AutoParry or GetRole()~="Survivor" then return end
    if tick()-_parryLastTime<(_G.VD_ParryDelay or 0.5) then return end
    local root=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    for _,pl in ipairs(Players:GetPlayers()) do
        if pl~=LocalPlayer and IsKiller(pl) and pl.Character then
            ...
            if kr then
                ...
                if dist<=(_G.VD_ParryRange or 15) then
                    local sensitivity=_G.VD_ParrySensitivity or 45
                    ...
                    rayP.FilterDescendantsInstances={LocalPlayer.Character,pl.Character,workspace.CurrentCamera}
                    local hit=workspace:Raycast(root.Position,kr.Position-root.Position,rayP)
                    if angle<=sensitivity and not hit then
                        pcall(function()
                            ...
                            -- Coba semua nama dagger yang mungkin
                            local dagger=items:FindFirstChild("Parrying Dagger") or items:FindFirstChild("Parry") or items:FindFirstChild("Dagger") or items:FindFirstChild("parry")
                            if not dagger then
                                -- Cari rekursif
                                for _,v in ipairs(items:GetDescendants()) do
                                    if v:IsA("RemoteEvent") and (v.Name:lower():find("parry") or v.Name:lower():find("block")) then
                                        dagger={_parryEvt=v}; break
                                    end
                                end
                            end
                            if not dagger then
                                if tick()-_parryLastWarn>2 then warn("[VD AutoParry] Dagger remote tidak ditemukan"); _parryLastWarn=tick() end
                                return
                            end
                            local parryEvt = (dagger._parryEvt) or dagger:FindFirstChild("parry") or dagger:FindFirstChild("Parry") or dagger:FindFirstChildWhichIsA("RemoteEvent")
                            if parryEvt then
                                -- Hadapkan ke killer dulu
                                local lookTarget=Vector3.new(kr.Position.X,root.Position.Y,kr.Position.Z)
...
-- ── IMPROVED AIMBOT (v2.0 — smooth + prediction + vis check) ─
local _aimConn=nil
local _aimFOVConn=nil
local _aimFOVFrame=nil
local _aimHolding=false

local function _buildAimFOV(radius)
    if _aimFOVConn then _aimFOVConn:Disconnect(); _aimFOVConn=nil end
    if _aimFOVFrame then pcall(function() _aimFOVFrame:Destroy() end); _aimFOVFrame=nil end
    if not radius or radius<=0 then return end
    -- Frame-based circle (works on all executors including mobile)
    local pg=LocalPlayer:FindFirstChild("PlayerGui") or game:GetService("CoreGui")
    local sg=Instance.new("ScreenGui"); sg.Name="VD_AimFOV"; sg.ResetOnSpawn=false; sg.DisplayOrder=997; sg.IgnoreGuiInset=true; sg.Parent=pg
    local f=Instance.new("Frame",sg)
    f.AnchorPoint=Vector2.new(0.5,0.5)
    f.BackgroundTransparency=1
    f.Size=UDim2.new(0,radius*2,0,radius*2)
    f.Position=UDim2.new(0.5,0,0.5,0)
    Instance.new("UICorner",f).CornerRadius=UDim.new(1,0)
    local stk=Instance.new("UIStroke",f)
    stk.Color=Color3.fromRGB(168,85,247); stk.Thickness=2; stk.Transparency=0.1
    _aimFOVFrame=sg
    -- Update radius tiap frame kalau resize
    _aimFOVConn=RunService.RenderStepped:Connect(function()
        if not (_aimFOVFrame and _aimFOVFrame.Parent) then return end
        local r=_G.VD_AimFOV or radius
...
local function _getAimTarget()
    ...
    for _,pl in ipairs(Players:GetPlayers()) do
        if pl~=LocalPlayer and pl.Character then
            local isEnemy=(GetRole()=="Survivor" and IsKiller(pl)) or (GetRole()=="Killer" and IsSurvivor(pl))
            if isEnemy then
                local tr=pl.Character:FindFirstChild("HumanoidRootPart")
                if tr and (tr.Position-root.Position).Magnitude<=(_G.VD_AimRange or 200) then
                    local pos,onScreen=cam:WorldToViewportPoint(tr.Position)
                    if onScreen then
                        local vp=cam.ViewportSize
                        local d=math.sqrt((pos.X-vp.X/2)^2+(pos.Y-vp.Y/2)^2)
                        if d<(_G.VD_AimFOV or 120) and d<bestD then bestD=d; best=pl end
                    end
                end
            end
        end
...
local function startAimbot_v2()
    if _aimConn then _aimConn:Disconnect() end
    _buildAimFOV(_G.VD_AimFOV or 120)
    _aimConn=RunService.Heartbeat:Connect(function()
        if not _G.VD_AimEnabled then return end
        if _G.VD_AimUseHold and not _aimHolding then return end
        local cam=workspace.CurrentCamera; if not cam then return end
        local target=_getAimTarget()
        if not target or not target.Character then return end
        local part=(_G.VD_AimBone=="head") and target.Character:FindFirstChild("Head") or target.Character:FindFirstChild("HumanoidRootPart")
        if not part then return end
        -- Prediction
        local pred=part.Position
        if _G.VD_AimPredict then
            local vel=part.AssemblyLinearVelocity or Vector3.zero
            pred=pred+vel*0.1
        end
        -- Smooth lerp
        local smooth=_G.VD_AimSmooth or 0.35
        cam.CFrame=cam.CFrame:Lerp(CFrame.new(cam.CFrame.Position,pred),smooth)
    end)
end

local function stopAimbot_v2()
    if _aimConn then _aimConn:Disconnect(); _aimConn=nil end
    if _aimFOVConn then _aimFOVConn:Disconnect(); _aimFOVConn=nil end
    if _aimFOVFrame then pcall(function() _aimFOVFrame:Destroy() end); _aimFOVFrame=nil end
end

-- Touch hold untuk aimbot mobile
local _aimHoldBtn=nil
local function buildAimHoldBtn()
    if _aimHoldBtn and _aimHoldBtn.Parent then return end
    local pg=LocalPlayer:FindFirstChild("PlayerGui") or game:GetService("CoreGui")
    local sg=Instance.new("ScreenGui"); sg.Name="VD_AimHoldBtn"; sg.ResetOnSpawn=false; sg.DisplayOrder=996; sg.IgnoreGuiInset=true; sg.Parent=pg
    local btn=Instance.new("TextButton",sg)
    btn.Size=UDim2.new(0,70,0,70); btn.Position=UDim2.new(1,-90,1,-160)
    btn.BackgroundColor3=Color3.fromRGB(200,55,55); btn.BackgroundTransparency=0.2
    btn.Text="🎯\nAIM"; btn.TextSize=13; btn.Font=Enum.Font.GothamBold; btn.TextColor3=Color3.new(1,1,1)
    btn.BorderSizePixel=0; btn.ZIndex=20
    Instance.new("UICorner",btn).CornerRadius=UDim.new(1,0)
    local stk=Instance.new("UIStroke",btn); stk.Color=Color3.fromRGB(255,100,100); stk.Thickness=2
    btn.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.Touch then
            _aimHolding=true; btn.BackgroundColor3=Color3.fromRGB(50,200,80); stk.Color=Color3.fromRGB(50,230,80)
        end
    end)
    btn.InputEnded:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.Touch then
            _aimHolding=false; btn.BackgroundColor3=Color3.fromRGB(200,55,55); stk.Color=Color3.fromRGB(255,100,100)
        end
    end)
    _aimHoldBtn=sg
end
...
local _spearAimConn=nil
local function startSpearAimbot()
    if _spearAimConn then _spearAimConn:Disconnect() end
    _spearAimConn=RunService.Heartbeat:Connect(function()
        if not _G.VD_SpearAimbot or GetRole()~="Killer" then return end
        ...
        if not (best and best.Character) then return end
        local tr=best.Character:FindFirstChild("HumanoidRootPart"); if not tr then return end
        -- Gravity drop compensation
        local startPos=root.Position+Vector3.new(0,2,0)
        local gravity=50; local speed=100
        local time=bestD/speed
...
local function stopSpearAimbot()
    if _spearAimConn then _spearAimConn:Disconnect(); _spearAimConn=nil end
end

-- ── RADAR (Mobile-friendly Frame-based) ──────────────────────
local _radarGui=nil
local _radarDots={}
local _radarObjDots={}
...
local function buildRadar()
    if _radarGui and _radarGui.Parent then _radarGui:Destroy() end
    _radarDots={}; _radarObjDots={}
    local pg=LocalPlayer:FindFirstChild("PlayerGui") or game:GetService("CoreGui")
    local sg=Instance.new("ScreenGui"); sg.Name="VD_Radar"; sg.ResetOnSpawn=false; sg.DisplayOrder=995; sg.IgnoreGuiInset=true; sg.Parent=pg
    local RS=_G.VD_RadarSize or 130
    local rf=Instance.new("Frame",sg)
    rf.Size=UDim2.new(0,RS,0,RS); rf.Position=UDim2.new(0,20,0,120)
    rf.BackgroundColor3=Color3.fromRGB(15,15,15); rf.BackgroundTransparency=0.3; rf.BorderSizePixel=0; rf.ClipsDescendants=true
    local rc=Instance.new("UICorner",rf); rc.Name="RCorner"; rc.CornerRadius=UDim.new(0,5)
    local rStk=Instance.new("UIStroke",rf); rStk.Color=Color3.fromRGB(168,85,247); rStk.Thickness=2
    -- Crosshair lines
    local ch=Instance.new("Frame",rf); ch.Size=UDim2.new(1,-20,0,1); ch.Position=UDim2.new(0,10,0.5,0); ch.BackgroundColor3=Color3.fromRGB(45,45,45); ch.BorderSizePixel=0
    local cv=Instance.new("Frame",rf); cv.Size=UDim2.new(0,1,1,-20); cv.Position=UDim2.new(0.5,0,0,10); cv.BackgroundColor3=Color3.fromRGB(45,45,45); cv.BorderSizePixel=0
    -- Center dot (kamu)
    local cdot=Instance.new("Frame",rf); cdot.Size=UDim2.new(0,10,0,10); cdot.Position=UDim2.new(0.5,-5,0.5,-5); cdot.BackgroundColor3=Color3.fromRGB(0,255,100); cdot.BorderSizePixel=0; cdot.ZIndex=5
    Instance.new("UICorner",cdot).CornerRadius=UDim.new(1,0)
    -- Player dots (20)
    for i=1,20 do
        local d=Instance.new("Frame",rf); d.Size=UDim2.new(0,8,0,8); d.BackgroundColor3=Color3.fromRGB(255,65,65); d.BorderSizePixel=0; d.ZIndex=4; d.Visible=false
        Instance.new("UICorner",d).CornerRadius=UDim.new(1,0)
        _radarDots[i]=d
    end
    -- Generator dots (15)
    for i=1,15 do
        local d=Instance.new("Frame",rf); d.Size=UDim2.new(0,6,0,6); d.BackgroundColor3=Color3.fromRGB(255,180,50); d.BorderSizePixel=0; d.ZIndex=3; d.Visible=false
        Instance.new("UICorner",d).CornerRadius=UDim.new(1,0)
        _radarObjDots[i]=d
    end
    _radarGui=sg
    -- Drag
    local drag,ds,sp=false,nil,nil
    rf.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then drag=true;ds=i.Position;sp=rf.Position end end)
    UserInputService.InputChanged:Connect(function(i) if drag and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then local d=i.Position-ds; rf.Position=UDim2.new(sp.X.Scale,sp.X.Offset+d.X,sp.Y.Scale,sp.Y.Offset+d.Y) end end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then drag=false end end)

    if _radarConn then _radarConn:Disconnect() end
    _radarConn=RunService.RenderStepped:Connect(function()
        if not (_radarGui and _radarGui.Parent) then return end
        if not _G.VD_RadarEnabled then rf.Visible=false; return end
        rf.Visible=true
        -- Resize
        local RS2=_G.VD_RadarSize or 130
        rf.Size=UDim2.new(0,RS2,0,RS2)
        rc.CornerRadius=_G.VD_RadarCircle and UDim.new(1,0) or UDim.new(0,5)
        ...
        local half=RS2/2; local scale=(half-10)/150; local maxD=half-8
        local function w2r(px,pz)
            local rx=px-myRoot.Position.X; local rz=pz-myRoot.Position.Z
        ...
        local idx=1
        for _,pl in ipairs(Players:GetPlayers()) do
            if pl~=LocalPlayer and pl.Character and idx<=#_radarDots then
                local pr=pl.Character:FindFirstChild("HumanoidRootPart")
                local isK=IsKiller(pl); local isS=IsSurvivor(pl)
                if pr and ((isK and _G.VD_RadarKiller) or (isS and _G.VD_RadarSurvivor)) then
                    local p=w2r(pr.Position.X,pr.Position.Z)
                    if p then
                        local d=_radarDots[idx]
        ...
        for i=idx,#_radarDots do _radarDots[i].Visible=false end
        local objIdx=1
        if _G.VD_RadarGenerator then
            for _,gen in ipairs(NEX_Cache.Generators) do
                if gen.part and objIdx<=#_radarObjDots then
                    local p=w2r(gen.part.Position.X,gen.part.Position.Z)
                    if p then
                        local d=_radarObjDots[objIdx]
        ...
        for i=objIdx,#_radarObjDots do _radarObjDots[i].Visible=false end
    end)
end

local function destroyRadar()
    _G.VD_RadarEnabled=false
...
-- ── GENERATOR ESP WITH PROGRESS % ────────────────────────────
local _genEspConn=nil
local _genEspLabels={}

local function updateGenESP()
    -- Bersihkan label lama yang sudah invalid
    for gen,data in pairs(_genEspLabels) do
        if not (gen.part and gen.part.Parent) then
            pcall(function() if data.bb then data.bb:Destroy() end end)
            _genEspLabels[gen]=nil
        end
    end
    if not _G.VD_GenESP then
        for _,data in pairs(_genEspLabels) do pcall(function() if data.bb then data.bb:Destroy() end end) end
        _genEspLabels={}; return
    end
    for _,gen in ipairs(NEX_Cache.Generators) do
        if gen.part and gen.part.Parent then
            if not _genEspLabels[gen] then
                local bb=Instance.new("BillboardGui")
                bb.Size=UDim2.new(0,120,0,40); bb.StudsOffset=Vector3.new(0,4,0)
                bb.AlwaysOnTop=true; bb.Adornee=gen.part; bb.Parent=gen.part
                local lbl=Instance.new("TextLabel",bb)
                lbl.Size=UDim2.new(1,0,1,0); lbl.BackgroundTransparency=1
                lbl.TextStrokeTransparency=0; lbl.TextStrokeColor3=Color3.new(0,0,0)
                lbl.Font=Enum.Font.GothamBold; lbl.TextSize=13; lbl.TextColor3=Color3.fromRGB(255,200,50)
                _genEspLabels[gen]={bb=bb,lbl=lbl}
            end
            local data=_genEspLabels[gen]
            -- Cari progress dari attribute atau value
            local pct=0
            pcall(function()
                local attr=gen.part:GetAttribute("Progress") or gen.model:GetAttribute("Progress")
                if attr then pct=math.clamp(attr,0,100); return end
                -- Cari NumberValue bernama Progress/Charge
                for _,v in ipairs(gen.model:GetDescendants()) do
                    if v:IsA("NumberValue") and (v.Name:lower():find("progress") or v.Name:lower():find("charge") or v.Name:lower():find("repair")) then
                        pct=math.clamp(v.Value,0,100); return
                    end
                end
            end)
            local dist=0
            local root=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if root then dist=math.floor((gen.part.Position-root.Position).Magnitude) end
            data.lbl.Text=string.format("⚡ Gen  %d%%\n📍 %dm",math.floor(pct),dist)
            data.lbl.TextColor3=pct>=100 and Color3.fromRGB(100,255,100) or (pct>=50 and Color3.fromRGB(255,200,50) or Color3.fromRGB(255,100,100))
        end
    end
end

-- Main heartbeat untuk semua fitur baru
RunService.Heartbeat:Connect(function(dt)
    ...
    pcall(NEX_BeatKiller)
    pcall(NEX_TeleAway)
    pcall(NEX_AutoParry)
end)

-- Gen ESP update tiap 1 detik
task.spawn(function()
    while true do
        pcall(updateGenESP)
        task.wait(1)
...
-- Defaults
_G.VD_AutoAttack=false; _G.VD_AutoAttackRange=12
_G.VD_HitboxEnabled=false; _G.VD_HitboxSize=15
_G.VD_DoubleTap=false; _G.VD_InfLunge=false
_G.VD_AutoHook=false; _G.VD_AntiBlind=false
_G.VD_NoPalletStun=false; _G.VD_NoSlowdown=false
_G.VD_DestroyPallets=false; _G.VD_FullGenBreak=false
_G.VD_BeatKiller=false; _G.VD_BeatSurvivor=false
_G.VD_TeleAway=false; _G.VD_TeleAwayDist=40
_G.VD_FlingStrength=10000
_G.VD_AutoParry=false; _G.VD_ParryDelay=0.5; _G.VD_ParryRange=15; _G.VD_ParrySensitivity=45
_G.VD_AimEnabled=false; _G.VD_AimFOV=120; _G.VD_AimRange=200; _G.VD_AimSmooth=0.35; _G.VD_AimPredict=true; _G.VD_AimBone="head"; _G.VD_AimUseHold=true
_G.VD_SpearAimbot=false
_G.VD_RadarEnabled=false; _G.VD_RadarSize=130; _G.VD_RadarCircle=false; _G.VD_RadarKiller=true; _G.VD_RadarSurvivor=true; _G.VD_RadarGenerator=true
_G.VD_GenESP=false

-- ============================================================
-- UI UNTUK FITUR BARU v2.0
-- ============================================================

-- ─── KILLER TAB (tambahan) ───────────────────────────────────
local killerTab2=killerTab   -- gunakan tab yang sama

killerTab2:Section({ Title="Combat Killer", Icon="swords" })
killerTab2:Toggle({ Title="Auto Attack", Description="Otomatis BasicAttack ke survivor dalam jangkauan", Value=false, Callback=function(v) _G.VD_AutoAttack=v end })
killerTab2:Slider({ Title="Auto Attack Range (studs)", Value={Min=5,Max=30,Default=12}, Step=1, Callback=function(v) _G.VD_AutoAttackRange=v end })
killerTab2:Toggle({ Title="Hitbox Expand", Description="Perbesar hitbox survivor (lebih gampang kena)", Value=false, Callback=function(v) _G.VD_HitboxEnabled=v end })
killerTab2:Slider({ Title="Ukuran Hitbox", Value={Min=5,Max=30,Default=15}, Step=1, Callback=function(v) _G.VD_HitboxSize=v end })
killerTab2:Toggle({ Title="Double Tap", Description="Kirim 2x BasicAttack tiap 0.5 detik", Value=false, Callback=function(v) _G.VD_DoubleTap=v end })
killerTab2:Toggle({ Title="Infinite Lunge", Description="Terus-menerus lunge ke depan", Value=false, Callback=function(v) _G.VD_InfLunge=v end })

killerTab2:Section({ Title="Utility Killer", Icon="wrench" })
killerTab2:Toggle({ Title="Auto Hook", Description="Otomatis hook survivor yang knock", Value=false, Callback=function(v) _G.VD_AutoHook=v end })
killerTab2:Toggle({ Title="Anti Blind (Flashlight)", Description="Block efek blind dari flashlight survivor", Value=false, Callback=function(v) _G.VD_AntiBlind=v end })
killerTab2:Toggle({ Title="No Pallet Stun", Description="Tidak kena stun saat kena palet", Value=false, Callback=function(v) _G.VD_NoPalletStun=v end })
killerTab2:Toggle({ Title="No Slowdown", Description="WalkSpeed tidak bisa dikurangi efek apapun", Value=false, Callback=function(v) _G.VD_NoSlowdown=v end })

killerTab2:Section({ Title="Map Control (Killer)", Icon="map-pin" })
killerTab2:Toggle({ Title="Destroy All Pallets", Description="Hancurkan semua palet di map tiap 1.5 detik", Value=false, Callback=function(v) _G.VD_DestroyPallets=v end })
killerTab2:Toggle({ Title="Full Gen Break", Description="Rusak semua generator tiap 0.8 detik", Value=false, Callback=function(v) _G.VD_FullGenBreak=v end })
killerTab2:Toggle({ Title="Beat Killer (Auto Win)", Description="Otomatis kejar dan habisi semua survivor", Value=false, Callback=function(v) _G.VD_BeatKiller=v end })
killerTab2:Toggle({ Title="Spear Aimbot (Veil)", Description="Kamera otomatis aim ke survivor untuk lempar tombak", Value=false, Callback=function(v)
    _G.VD_SpearAimbot=v
    if v then startSpearAimbot() else stopSpearAimbot() end
end })

-- ─── SURVIVOR TAB (tambahan) ─────────────────────────────────
SurTab:Section({ Title="Combat Survivor", Icon="shield" })
SurTab:Toggle({ Title="Auto Parry (v2.0 — Fixed)", Description="Parry otomatis saat killer mendekat (butuh Parrying Dagger)", Value=false, Callback=function(v)
    _G.VD_AutoParry=v
end })
SurTab:Slider({ Title="Parry Range (studs)", Value={Min=5,Max=25,Default=15}, Step=1, Callback=function(v) _G.VD_ParryRange=v end })
SurTab:Slider({ Title="Parry Sensitivity (derajat)", Value={Min=15,Max=90,Default=45}, Step=5, Callback=function(v) _G.VD_ParrySensitivity=v end })

SurTab:Section({ Title="Escape & Griefing", Icon="zap" })
SurTab:Toggle({ Title="Flee Killer (Auto TeleAway)", Description="Otomatis teleport jauh saat killer mendekat (cooldown 3 detik)", Value=false, Callback=function(v) _G.VD_TeleAway=v end })
SurTab:Slider({ Title="TeleAway Trigger Distance", Value={Min=10,Max=80,Default=40}, Step=5, Callback=function(v) _G.VD_TeleAwayDist=v end })
SurTab:Toggle({ Title="Beat Survivor (Auto Exit)", Description="Teleport ke pintu keluar otomatis", Value=false, Callback=function(v)
    _G.VD_BeatSurvivor=v
    if v then task.spawn(function() while _G.VD_BeatSurvivor do pcall(NEX_BeatSurvivor); task.wait(2) end end) end
end })

-- ─── GRIEFING / FLING TAB ────────────────────────────────────
local GriefTab2=GriefTab
GriefTab2:Section({ Title="Fling", Icon="zap" })
GriefTab2:Toggle({ Title="Fling Enabled", Description="Aktifkan sistem fling", Value=false, Callback=function(v) _G.VD_FlingEnabled=v end })
GriefTab2:Slider({ Title="Fling Strength", Value={Min=1000,Max=50000,Default=10000}, Step=500, Callback=function(v) _G.VD_FlingStrength=v end })
GriefTab2:Button({ Title="Fling Nearest Player", Description="Fling player terdekat sekarang", Callback=function()
    if not _G.VD_FlingEnabled then WindUI:Notify({Title="Fling",Content="Aktifkan Fling Enabled dulu!",Duration=2,Icon="alert-circle"}); return end
    task.spawn(NEX_FlingNearest)
end })
GriefTab2:Button({ Title="Fling All Players", Description="Fling semua player sekarang", Callback=function()
    if not _G.VD_FlingEnabled then WindUI:Notify({Title="Fling",Content="Aktifkan Fling Enabled dulu!",Duration=2,Icon="alert-circle"}); return end
    task.spawn(NEX_FlingAll)
end })

-- ─── AIMBOT TAB BARU ─────────────────────────────────────────
local AimTabNew=Window:Tab({ Title="Aimbot", Icon="crosshair" })
AimTabNew:Section({ Title="Aimbot v2.0 (Smooth + Prediction)", Icon="target" })
AimTabNew:Toggle({ Title="Enable Aimbot", Value=false, Callback=function(v)
    _G.VD_AimEnabled=v
    if v then startAimbot_v2(); buildAimHoldBtn()
    else stopAimbot_v2() end
end })
AimTabNew:Toggle({ Title="Hold to Aim (Mobile Button)", Description="Muncul tombol 🎯 di layar, tahan untuk aktifkan aim", Value=true, Callback=function(v)
    _G.VD_AimUseHold=v
    if v then buildAimHoldBtn()
    elseif _aimHoldBtn then pcall(function() _aimHoldBtn:Destroy() end); _aimHoldBtn=nil end
end })
AimTabNew:Toggle({ Title="Prediction (follow velocity)", Value=true, Callback=function(v) _G.VD_AimPredict=v end })
AimTabNew:Slider({ Title="FOV Circle Radius", Value={Min=30,Max=400,Default=120}, Step=10, Callback=function(v)
    _G.VD_AimFOV=v
    if _G.VD_AimEnabled then _buildAimFOV(v) end
end })
AimTabNew:Slider({ Title="Smooth (0.1=lambat, 1.0=snap)", Value={Min=1,Max=10,Default=4}, Step=1, Callback=function(v) _G.VD_AimSmooth=v/10 end })
AimTabNew:Slider({ Title="Max Range (studs)", Value={Min=50,Max=500,Default=200}, Step=25, Callback=function(v) _G.VD_AimRange=v end })

-- ─── RADAR TAB BARU ──────────────────────────────────────────
local RadarTab=Window:Tab({ Title="Radar", Icon="radio" })
RadarTab:Section({ Title="Mini Radar", Icon="map" })
RadarTab:Toggle({ Title="Enable Radar", Value=false, Callback=function(v)
    _G.VD_RadarEnabled=v
    if v then buildRadar() else destroyRadar() end
end })
RadarTab:Slider({ Title="Ukuran Radar", Value={Min=80,Max=250,Default=130}, Step=10, Callback=function(v) _G.VD_RadarSize=v end })
RadarTab:Toggle({ Title="Bentuk Lingkaran", Value=false, Callback=function(v) _G.VD_RadarCircle=v end })
RadarTab:Section({ Title="Radar Filters", Icon="filter" })
RadarTab:Toggle({ Title="Tampilkan Killer", Value=true, Callback=function(v) _G.VD_RadarKiller=v end })
RadarTab:Toggle({ Title="Tampilkan Survivor", Value=true, Callback=function(v) _G.VD_RadarSurvivor=v end })
RadarTab:Toggle({ Title="Tampilkan Generator", Value=true, Callback=function(v) _G.VD_RadarGenerator=v end })

-- ─── GENERATOR ESP WITH PROGRESS ─────────────────────────────
-- Tambah ke ESP existing atau GriefTab
GriefTab:Section({ Title="Generator ESP (dengan %)", Icon="gauge" })
GriefTab:Toggle({ Title="Generator ESP + Progress %", Description="Tampilkan label % repair di setiap generator", Value=false, Callback=function(v)
    _G.VD_GenESP=v
    if not v then for _,data in pairs(_genEspLabels) do pcall(function() if data.bb then data.bb:Destroy() end end) end; _genEspLabels={} end
end })
