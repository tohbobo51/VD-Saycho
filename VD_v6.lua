...
-- ── Color Picker ESP ─────────────────────────────────────────
EspTab:Section({ Title = "Warna ESP", Icon = "palette" })
...
EspTab:ColorPicker({
    ...
    Callback = function(c)
        COLOR_SURVIVOR = c
        -- Update semua ESP yang sudah ada
        for obj, data in pairs(espObjects) do
            if data.highlight and not obj:FindFirstChildWhichIsA("Weapon") then
                pcall(function() data.highlight.FillColor = c end)
            end
        end
    end
})
...
-- ============================================================
-- SURVIVOR REMOTES  — Fast Vault (auto toggle), Stop Emote, Heal
-- ============================================================
...
local vaultTouchConns    = {}   -- koneksi Touched per VaultTrigger

-- Remote refs (di-cache setelah pertama kali dipakai)
local remWindow = nil
local function getWindowRemotes()
    if remWindow then return remWindow end
    ...
    if ok then remWindow = r end
    return ok and r or nil
end

-- Cari semua VaultTrigger di map (BasePart / Part)
...
-- Putuskan semua koneksi vault
local function disconnectVaultConns()
    for _, c in ipairs(vaultTouchConns) do pcall(function() c:Disconnect() end) end
    vaultTouchConns = {}
end

-- Hook satu VaultTrigger ke fast vault
local function hookVaultTrigger(trigger)
    local conn = trigger.Touched:Connect(function(part)
        if not (fastVaultEnabled or instantVaultEnabled) then return end
        -- Pastikan yang menyentuh adalah karakter kita
        local char = LocalPlayer.Character
        if not char then return end
        local hit = false
        for _, bp in ipairs(char:GetDescendants()) do
            if bp == part then hit = true; break end
        end
        if not hit and char:FindFirstChild(part.Name) == nil then
            -- cek parent chain
            local p = part.Parent
            while p do
                if p == char then hit = true; break end
                p = p.Parent
            end
        end
        ...
        if instantVaultEnabled then
            -- Kirim VaultEvent (start) + VaultCompleteEvent (selesai) seketika
            pcall(function()
                rems.vaultEvent:FireServer(trigger, true)
                task.wait(0.03)
                rems.vaultComplete:FireServer(trigger, false)
            end)
        elseif fastVaultEnabled then
            -- Hanya fast vault biasa
            pcall(function()
...
-- Aktifkan: hook semua trigger yang ada + listen trigger baru
local vaultMapConn = nil
local function startVaultHook()
    disconnectVaultConns()
    for _, t in ipairs(getAllVaultTriggers()) do hookVaultTrigger(t) end

    -- Kalau ada VaultTrigger baru ditambahkan (map loading)
    local map = workspace:FindFirstChild("Map")
    if map then
...
SurTab:Toggle({
    ...
    Description = "ON = setiap kali karakter touch VaultTrigger → fastvault langsung fire",
    Value       = false,
    Callback    = function(v)
        fastVaultEnabled = v
        if v then
            instantVaultEnabled = false   -- matikan instant kalau fast aktif
            startVaultHook()
            WindUI:Notify({ Title="Vault", Content="Fast Vault aktif!", Duration=2, Icon="wind" })
        else
            if not instantVaultEnabled then stopVaultHook() end
            WindUI:Notify({ Title="Vault", Content="Fast Vault mati.", Duration=1.5, Icon="wind" })
        end
    end
})

SurTab:Toggle({
    Title       = "Instant Vault  (skip animasi penuh)",
    Description = "ON = VaultEvent + VaultCompleteEvent dikirim sekaligus saat touch jendela",
    Value       = false,
    Callback    = function(v)
        instantVaultEnabled = v
        if v then
            fastVaultEnabled = false   -- matikan fast kalau instant aktif
            startVaultHook()
            WindUI:Notify({ Title="Vault", Content="Instant Vault aktif!", Duration=2, Icon="zap" })
        else
            if not fastVaultEnabled then stopVaultHook() end
            WindUI:Notify({ Title="Vault", Content="Instant Vault mati.", Duration=1.5, Icon="zap" })
        end
    end
})
...
-- Heal diri sendiri
SurTab:Button({
    Title       = "💊 Heal Diri Sendiri",
    Description = "FireServer → Healing/HealEvent [HRP kita, true]",
    Callback    = function()
        local ok, err = pcall(function()
            local char = LocalPlayer.Character
            if not char then error("Karakter tidak ditemukan") end
            local hrp = char:WaitForChild("HumanoidRootPart", 3)
            ReplicatedStorage
                :WaitForChild("Remotes", 5)
                :WaitForChild("Healing", 5)
                :WaitForChild("HealEvent", 5)
                :FireServer(hrp, true)   -- true = heal diri
        end)
        WindUI:Notify({
            Title   = "Heal",
            Content = ok and "Heal diri berhasil!" or "Gagal: "..tostring(err),
            Duration = 2, Icon = ok and "heart" or "alert-circle"
        })
    end
})

-- Auto Heal diri sendiri (loop)
local autoHealSelfEnabled = false
local autoHealSelfConn = nil

SurTab:Toggle({
    Title       = "Auto Heal Diri Sendiri",
    Description = "Otomatis kirim HealEvent setiap beberapa detik",
    Value       = false,
    Callback    = function(v)
        autoHealSelfEnabled = v
        if v then
            autoHealSelfConn = task.spawn(function()
...
-- Heal player lain
do
    local healSelectedName = ""

    local function getHealPlayerList()
        local list = {}
        for _, pl in ipairs(Players:GetPlayers()) do
    ...
    local healDropdown = SurTab:Dropdown({
        Title    = "Pilih Target Heal",
        Values   = getHealPlayerList(),
        Value    = "",
        Callback = function(v) healSelectedName = v end
    })

    SurTab:Button({
        Title    = "🔄 Refresh Daftar Player",
        Callback = function()
            pcall(function() healDropdown:Refresh(getHealPlayerList(), false) end)
            healSelectedName = ""
        end
    })

    SurTab:Button({
        Title       = "💊 Heal Player yang Dipilih",
        Description = "FireServer → Healing/HealEvent [HRP target, false]",
        Callback    = function()
            if healSelectedName == "" or healSelectedName:find("Tidak ada") then
            ...
            local ok, err = pcall(function()
                local target = nil
                for _, pl in ipairs(Players:GetPlayers()) do
                    if pl.Name == healSelectedName then target = pl; break end
                end
                if not target then error("Player tidak ditemukan") end
                if not target.Character then error("Karakter target tidak ada") end
                local hrp = target.Character:FindFirstChild("HumanoidRootPart")
                if not hrp then error("HRP target tidak ditemukan") end
                ReplicatedStorage
                    :WaitForChild("Remotes", 5)
                    :WaitForChild("Healing", 5)
                    :WaitForChild("HealEvent", 5)
                    :FireServer(hrp, false)   -- false = heal orang lain
            end)
            WindUI:Notify({
                Title   = "Heal",
                Content = ok and "Heal → "..healSelectedName or "Gagal: "..tostring(err),
                Duration = 2, Icon = ok and "heart" or "alert-circle"
            })
        end
    })
...
SilentTab:ColorPicker({
    Title    = "Warna FOV Circle Silent Aim",
    Color    = Color3.fromRGB(255, 80, 80),
    Callback = function(c)
        if silentFovCircle then
            pcall(function() silentFovCircle.Color = c end)
        end
    end
})
