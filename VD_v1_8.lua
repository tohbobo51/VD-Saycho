...
-- ── Tombol Rejoin / Ganti Server di TopBar ───────────────────
pcall(function()
    Window:CreateTopbarButton("ServerHop", "refresh-cw", function()
        local TeleportService = game:GetService("TeleportService")
        local placeId = game.PlaceId

        -- Minta konfirmasi lewat dialog sederhana (pakai prompt notifikasi)
        WindUI:Dialog({
            Title   = "Rejoin / Ganti Server",
            Content = "Pilih aksi:",
            Buttons = {
                {
                    Title    = "🔄 Rejoin (server sama)",
                    Callback = function()
                    ...
                    Callback = function()
                        ...
                        task.spawn(function()
                            ...
                            if ok and servers and #servers > 0 then
                                -- Pilih server random yang bukan server sekarang
                                ...
                                if picked then
                                    ...
                                    -- Tidak ada server lain → rejoin saja
                                    pcall(function()
                                        TeleportService:Teleport(placeId, LocalPlayer)
                                    end)
                                end
    ...
    end, 989)   -- priority 989 = langsung di sebelah kiri Transparency
end)
...
-- Helper: fire HealEvent + StopEmote langsung
local function fireHeal(hrp, isRevive)
    local rem = getHealEvent()
    if not rem then error("Remote HealEvent tidak ditemukan") end
    for i = 1, 3 do
        rem:FireServer(hrp, isRevive)
        task.wait(0.05)
    end
    pcall(function()
...
local function runAutoUnhook()
    task.spawn(function()
        while _G.AutoUnhook do
            -- ✅ FIX v1.6: tidak pakai distance check lagi
            -- Ghost teleport mengatasi server-side proximity validation
            local hooks = getAllHooks()
            for _, hookObj in ipairs(hooks) do
                if not _G.AutoUnhook then break end

                local refPart = nil
                if hookObj:IsA("BasePart") then
                    refPart = hookObj
                else
                    refPart = hookObj:FindFirstChild("HookPoint")
                        or hookObj:FindFirstChildWhichIsA("BasePart")
                end

                if refPart then
                    -- Cek apakah ada player yang tergantung di hook ini
                    local hasVictim = false
                    for _, pl in ipairs(Players:GetPlayers()) do
                        if pl ~= LocalPlayer and pl.Character then
                            local plHRP = pl.Character:FindFirstChild("HumanoidRootPart")
                            if plHRP and (plHRP.Position - refPart.Position).Magnitude < 8 then
                    ...
                    if hasVictim then
                        tryFireUnhook(hookObj)  -- ghost teleport + fire + balik
                        WindUI:Notify({
                            Title    = "Auto Unhook",
                        ...
                        task.wait(0.5)  -- cooldown antar unhook
                    end
                end
            end
            task.wait(0.8)
    ...
    GriefTab:Button({
        ...
        Callback    = function()
            ...
            local ok, err = pcall(function()
                ...
                -- 2. Cari HookPoint terdekat dari posisi player itu
                local map = workspace:FindFirstChild("Map")
                local hookFolder = map and (map:FindFirstChild("Hook") or map:FindFirstChild("Hooks"))
                if not hookFolder then error("Folder Hook tidak ada") end
                ...
                if not closestHookPart then error("Tidak ada HookPoint ditemukan") end
