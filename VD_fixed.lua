...
-- ============================================================
-- QUICK PANEL  — 3 tombol TERPISAH, masing-masing bisa di-drag
-- ============================================================
local quickPanelGuis = {}   -- { moon=ScreenGui, aim=ScreenGui, god=ScreenGui }
local quickPanelVisible = false

-- Helper: buat 1 tombol lingkaran mandiri (ScreenGui sendiri)
local function makeSingleQBtn(name, icon, colOn, colStrokeOn, initPos, tapFn)
    local sg = Instance.new("ScreenGui")
    sg.Name = "VD_QB_" .. name
...
local function buildQuickPanel()
    ...
    for _, sg in pairs(quickPanelGuis) do
        if sg and sg.Parent then sg:Destroy() end
    end
    quickPanelGuis = {}

    -- 🌙 Moonwalk — kiri atas
    local sgMoon, setMoon = makeSingleQBtn(
        "Moon", "🌙",
        Color3.fromRGB(40, 20, 80), Color3.fromRGB(168, 85, 247),
        UDim2.new(0, 12, 0.40, 0),
        function(v) if v then startMoonwalk() else stopMoonwalk() end end
    )

    -- 🎯 Aimbot — sedikit di bawah Moonwalk
    local sgAim, setAim = makeSingleQBtn(
        "Aim", "🎯",
    ...
    -- 🛡 GodMode — di bawah Aimbot
    local sgGod, setGod = makeSingleQBtn(
        "God", "🛡",
        Color3.fromRGB(50, 30, 10), Color3.fromRGB(232, 137, 12),
        UDim2.new(0, 12, 0.60, 0),
        function(v) if v then startGodMode() else stopGodMode() end end
    )

    quickPanelGuis = { sgMoon, sgAim, sgGod }
