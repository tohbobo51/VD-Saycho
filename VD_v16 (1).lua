--[[
  ...
  Version : 16.0
  Author  : Saycho
  New v16 : Custom Crosshair (5 shape: Cross/Dot/Circle/Square/Arrow,
            5 preset warna + HSV Color Picker popup, live update)
]]
...
local States = {
    ...
    CrosshairShape = "Cross",   -- "Cross" | "Dot" | "Circle" | "Square" | "Arrow"
    -- Utility
...
-- ─── CUSTOM CROSSHAIR (shape + color, live update) ──────────
local crosshairGui = nil
local crosshairParts = {}   -- semua frame crosshair (bisa recolor live)

local function buildCrosshairShape(container, shape, color)
    -- Bersihkan isi container dulu
    for _, c in ipairs(container:GetChildren()) do c:Destroy() end
    crosshairParts = {}
    local function mkFrame(w, h, ox, oy, radius)
        ...
        f.BorderSizePixel  = 0
        if radius then Instance.new("UICorner", f).CornerRadius = UDim.new(radius, 0) end
        table.insert(crosshairParts, f)
        return f
    end
    if shape == "Cross" then
        mkFrame(2, 14, 0, 0);  mkFrame(14, 2, 0, 0)     -- vertikal + horizontal
        mkFrame(3, 3, 0, 0, 1)                           -- dot tengah
    elseif shape == "Dot" then
        mkFrame(6, 6, 0, 0, 1)                           -- satu titik besar
    elseif shape == "Circle" then
        -- 4 busur kecil
        mkFrame(12, 2, 0, -8); mkFrame(12, 2, 0, 8)
        mkFrame(2, 12, -8, 0); mkFrame(2, 12, 8, 0)
        mkFrame(3, 3, 0, 0, 1)                           -- dot tengah
    elseif shape == "Square" then
        -- 4 sudut bracket
        mkFrame(6, 2, -7, -7); mkFrame(2, 6, -7, -7)
        mkFrame(6, 2,  7, -7); mkFrame(2, 6,  7, -7)
        mkFrame(6, 2, -7,  7); mkFrame(2, 6, -7,  7)
        mkFrame(6, 2,  7,  7); mkFrame(2, 6,  7,  7)
    elseif shape == "Arrow" then
        mkFrame(2, 12, 0, 0)                             -- tangkai vertikal
        mkFrame(8, 2, 0, -8)                             -- ujung horizontal
        mkFrame(2, 5, -3, -5, 0)                         -- sayap kiri
        mkFrame(2, 5,  3, -5, 0)                         -- sayap kanan
    end
end

local function refreshCrosshairColor(color)
    States.CrosshairColor = color
    for _, f in ipairs(crosshairParts) do
        pcall(function() f.BackgroundColor3 = color end)
    end
end
...
local function setCrosshair(on)
    ...
    container.BackgroundTransparency = 1
    buildCrosshairShape(container, States.CrosshairShape, States.CrosshairColor)
end

-- HSV Mini Color Picker popup (untuk crosshair & ESP)
local function OpenColorPicker(defaultColor, onChanged, anchorBtn)
    local existing = ScreenGui:FindFirstChild("VD_ColorPop")
    if existing then existing:Destroy(); return end  -- toggle off

    local H, S, V = Color3.toHSV(defaultColor)
    local PW, PH = 218, 170

    local pop = Instance.new("Frame", ScreenGui)
    pop.Name = "VD_ColorPop"
    pop.Size = UDim2.new(0, PW, 0, PH)
    pop.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
    pop.BorderSizePixel  = 0; pop.ZIndex = 300
    Instance.new("UICorner", pop).CornerRadius = UDim.new(0, 10)
    Instance.new("UIStroke", pop).Color = Color3.fromRGB(50, 50, 50)

    -- Posisi popup di atas/kiri anchorBtn
    if anchorBtn then
    ...
    svKnob.Position=UDim2.new(S,0,1-V,0); svKnob.BackgroundColor3=Color3.new(1,1,1); svKnob.BorderSizePixel=0; svKnob.ZIndex=10
    ...
    hueStrip.Size     = UDim2.new(0, 16, 0, svH)
    hueStrip.Position = UDim2.new(0, svW+14, 0, 30)
...
do
    ...
    xhRow.Size = UDim2.new(1, 0, 0, 46); xhRow.BackgroundColor3 = Color3.fromRGB(28,28,28)
    xhRow.BorderSizePixel=0; xhRow.LayoutOrder=51
    ...
    xhSub.Text="Bantu akurasi Twist of Fate (seperti FPS)"; xhSub.Position=UDim2.new(0,12,0,24); xhSub.Size=UDim2.new(1,-70,0,14)
    ...
    local xhKnob=Instance.new("Frame",xhTrack); xhKnob.Size=UDim2.new(0,17,0,17); xhKnob.Position=UDim2.new(0,3,0.5,-8)
    xhKnob.BackgroundColor3=C.TEXT_A; xhKnob.BorderSizePixel=0; Instance.new("UICorner",xhKnob).CornerRadius=UDim.new(1,0)
    xhTrack.Activated:Connect(function()
        States.Crosshair = not States.Crosshair
        setCrosshair(States.Crosshair)
        tw(xhTrack,{BackgroundColor3 = States.Crosshair and C.ON or C.OFF})
    ...
    shapeRow.Size=UDim2.new(1,0,0,38); shapeRow.BackgroundColor3=Color3.fromRGB(24,24,24)
    shapeRow.BorderSizePixel=0; shapeRow.LayoutOrder=52
    Instance.new("UICorner",shapeRow).CornerRadius=UDim.new(0,8)

    local shapeLbl = Instance.new("TextLabel", shapeRow)
    shapeLbl.Text="Shape"; shapeLbl.Position=UDim2.new(0,12,0,0); shapeLbl.Size=UDim2.new(0,50,1,0)
    shapeLbl.TextColor3=Color3.fromRGB(160,160,160); shapeLbl.Font=Enum.Font.GothamBold; shapeLbl.TextSize=11; shapeLbl.BackgroundTransparency=1; shapeLbl.TextXAlignment=Enum.TextXAlignment.Left

    local shapes = {"Cross","Dot","Circle","Square","Arrow"}
    local shapeSymbols = {Cross="✛", Dot="●", Circle="◯", Square="⬜", Arrow="▲"}
    local shapeX = 60
    for _, sh in ipairs(shapes) do
        local sb = Instance.new("TextButton", shapeRow)
        sb.Size=UDim2.new(0,36,0,26); sb.Position=UDim2.new(0,shapeX,0.5,-13)
        sb.Text = shapeSymbols[sh]; sb.TextSize=14
        ...
        sb.BorderSizePixel=0; Instance.new("UICorner",sb).CornerRadius=UDim.new(0,6)
        sb.Activated:Connect(function()
            States.CrosshairShape = sh
            -- Reset semua warna tombol shape
            for _, c2 in ipairs(shapeRow:GetChildren()) do
                if c2:IsA("TextButton") then
                    tw(c2,{BackgroundColor3=Color3.fromRGB(38,38,38)}); c2.TextColor3=Color3.new(1,1,1)
                end
            end
            tw(sb,{BackgroundColor3=C.ACCENT}); sb.TextColor3=Color3.new(0,0,0)
            -- Rebuild crosshair kalau sedang aktif
    ...
    colorRow.Size=UDim2.new(1,0,0,38); colorRow.BackgroundColor3=Color3.fromRGB(24,24,24)
    colorRow.BorderSizePixel=0; colorRow.LayoutOrder=53
    Instance.new("UICorner",colorRow).CornerRadius=UDim.new(0,8)

    local colorLbl = Instance.new("TextLabel", colorRow)
    colorLbl.Text="Warna Crosshair"; colorLbl.Position=UDim2.new(0,12,0,0); colorLbl.Size=UDim2.new(0,130,1,0)
    colorLbl.TextColor3=Color3.fromRGB(160,160,160); colorLbl.Font=Enum.Font.GothamBold; colorLbl.TextSize=11; colorLbl.BackgroundTransparency=1; colorLbl.TextXAlignment=Enum.TextXAlignment.Left

    -- Swatch colors cepat
    local presets = {
        Color3.fromRGB(255,140,0),   -- orange (default)
        Color3.fromRGB(255,255,255), -- putih
        Color3.fromRGB(0,255,100),   -- hijau
        Color3.fromRGB(80,180,255),  -- biru
        Color3.fromRGB(255,80,80),   -- merah
    }
    local swatchX = 130
    for _, col in ipairs(presets) do
        local sw = Instance.new("TextButton", colorRow)
        sw.Size=UDim2.new(0,22,0,22); sw.Position=UDim2.new(0,swatchX,0.5,-11)
    ...
    -- Tombol custom (buka color picker)
    local customBtn = Instance.new("TextButton", colorRow)
    customBtn.Size=UDim2.new(0,22,0,22); customBtn.Position=UDim2.new(0,swatchX,0.5,-11)
    customBtn.BackgroundColor3=States.CrosshairColor; customBtn.Text="🎨"; customBtn.TextSize=12
    customBtn.BorderSizePixel=0; customBtn.TextColor3=Color3.new(1,1,1)
    Instance.new("UICorner",customBtn).CornerRadius=UDim.new(1,0)
    customBtn.Activated:Connect(function()
        OpenColorPicker(States.CrosshairColor, function(col)
            States.CrosshairColor = col
            refreshCrosshairColor(col)
            customBtn.BackgroundColor3 = col
        end, customBtn)
    end)
end
...
print("[VD v16.0] Loaded — Saycho | Custom Crosshair: 5 shape + preset warna + HSV picker")
