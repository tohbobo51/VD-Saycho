...
local function getHealReset()
    if remHealReset then return remHealReset end
    local ok, r = pcall(function()
        return ReplicatedStorage
            :WaitForChild("Remotes", 5)
            :WaitForChild("Healing", 5)
...
SurTab:Button({
    ...
    Callback    = function()
        local ok, err = pcall(function()
            ...
            -- Spam 10x agar pasti kena server-side
            for i = 1, 10 do
...
local function tryFireUnhook(hookObj)
    ...
    -- Fire dengan arg yang benar sesuai RemoteSpy
    local ok, err = pcall(function()
...
local function runAutoUnhook()
    task.spawn(function()
        while _G.AutoUnhook do
            local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                -- Pakai getAllHooks() — scan Map rekursif
                local hooks = getAllHooks()
                for _, hookObj in ipairs(hooks) do
                    if not _G.AutoUnhook then break end

                    -- Cari posisi referensi hook (pakai HookPoint kalau ada)
                    local refPart = nil
                    if hookObj:IsA("BasePart") then
                        refPart = hookObj
                    else
                        -- ✅ FIX v1.5: prioritaskan HookPoint sebagai referensi posisi
                    ...
                    if refPart then
                        ...
                        if dist <= _G.UnhookRange then
                            ...
                            if hasVictim then
                                -- ✅ FIX: kirim hookObj (Model) sehingga getHookPoint bisa ekstrak HookPoint
                                tryFireUnhook(hookObj)
                                WindUI:Notify({
                                    Title    = "Auto Unhook",
                                    Content  = "Unhook dikirim → " .. refPart.Name,
                                    Duration = 1.5,
                                    Icon     = "anchor"
                                })
                            end
