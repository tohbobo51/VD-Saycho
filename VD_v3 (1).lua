...
local function getMapFolders()
    ...
    if mainMap then
        ...
        -- Scan semua subfolder (Rooftop, Nature, dll) sampai kedalaman apapun
        for _, desc in ipairs(mainMap:GetDescendants()) do
            if desc:IsA("Folder") or desc:IsA("Model") then
                table.insert(folders, desc)
            end
        end
    end
    return folders
...
-- Helper: ambil semua Palletwrong dari seluruh Map (tidak peduli nesting)
local function getAllPallets()
    local list = {}
    local mainMap = workspace:FindFirstChild("Map")
    if not mainMap then return list end
    for _, obj in ipairs(mainMap:GetDescendants()) do
...
-- Helper: ambil semua PalletPointSlide dari seluruh Map
local function getAllPalletPoints()
    local list = {}
    local mainMap = workspace:FindFirstChild("Map")
    if not mainMap then return list end
    for _, obj in ipairs(mainMap:GetDescendants()) do
...
-- Helper: ambil semua Hook dari seluruh Map
-- ✅ FIX v1.5: RemoteSpy menunjukkan arg = workspace.Map.Hook.HookPoint
-- Artinya Hook adalah folder/model, HookPoint adalah BasePart di dalamnya
-- Kita cari semua HookPoint BasePart di bawah model/folder bernama Hook
local function getAllHooks()
    ...
    -- Scan seluruh Map
    for _, obj in ipairs(mainMap:GetDescendants()) do
        -- Prioritas 1: BasePart bernama "HookPoint" (persis seperti RemoteSpy)
        if obj:IsA("BasePart") and obj.Name == "HookPoint" then
            table.insert(list, obj)
        -- Prioritas 2: Model/folder bernama "Hook" yang punya HookPoint di dalamnya
        elseif (obj.Name == "Hook" or obj.Name == "HookBase") and not obj:IsA("BasePart") then
            local hp = obj:FindFirstChild("HookPoint")
            if hp and hp:IsA("BasePart") then
                -- Sudah akan ditangkap oleh kondisi pertama, skip
            else
                -- Kalau tidak ada HookPoint, tambahkan modelnya saja sebagai fallback
                local anyPart = obj:FindFirstChildWhichIsA("BasePart")
                if anyPart then table.insert(list, obj) end
            end
        end
    end
    return list
...
local function updateESP(dt)
    ...
    -- ── Pallet ESP (scan deep, pakai getAllPallets) ──────────────────
    -- Harus di luar loop biasa karena Palletwrong bisa nested 3+ level
    local allPallets = getAllPallets()
    for _, obj in ipairs(allPallets) do
        if espPallet then
            createESP(obj, COLOR_PALLET)
        else
            removeESP(obj)
...
-- ─── Heal ─────────────────────────────────────────────────────
-- CATATAN (dari RemoteSpy):
--   HealEvent(HRP, false) = heal orang LAIN (sender != target, server reject kalau diri sendiri)
--   HealEvent(HRP, true)  = revive orang LAIN saat knocked
--   Reset(LocalPlayer)    = ✅ reset status DIRI SENDIRI langsung (tidak butuh orang lain)
-- ─────────────────────────────────────────────────────────────
SurTab:Section({ Title = "Heal / Revive", Icon = "heart" })

-- Cache remote emote handler
local remEmoteHandler = nil
local function getEmoteHandler()
    if remEmoteHandler and remEmoteHandler.Parent then return remEmoteHandler end
    local ok, r = pcall(function()
        return ReplicatedStorage:WaitForChild("Remotes",5):WaitForChild("EmoteHandler",5)
    end)
    if ok and r then remEmoteHandler = r end
    return remEmoteHandler
end

-- Cache HealEvent & HealReset remotes
local remHealEvent = nil
local remHealReset = nil

local function getHealEvent()
    if remHealEvent and remHealEvent.Parent then return remHealEvent end
    local ok, r = pcall(function()
    ...
    if ok and r then remHealEvent = r end
    return remHealEvent
end

local function getHealReset()
    if remHealReset and remHealReset.Parent then return remHealReset end
    local ok, r = pcall(function()
        return ReplicatedStorage
            :WaitForChild("Remotes", 5)
            :WaitForChild("Healing", 5)
            :WaitForChild("Reset", 5)
    end)
    if ok and r then remHealReset = r end
    return remHealReset
end

-- Helper: Ghost TP ke target → fire HealEvent → balik (bypass distance validation)
local function fireHeal(targetHRP, isRevive)
    local rem = getHealEvent()
    if not rem then error("Remote HealEvent tidak ditemukan") end
    ...
    -- Ghost TP ke target untuk bypass validasi jarak server
    local ok, err = pcall(function()
        myHRP.CFrame = targetHRP.CFrame * CFrame.new(0, 0, 2)
        task.wait() -- 1 frame agar server update posisi kita

        for i = 1, 3 do
            rem:FireServer(targetHRP, isRevive)
            task.wait(0.05)
        end

        myHRP.CFrame = originalCF -- Kembalikan ke posisi asal
    end)

    if not ok then pcall(function() myHRP.CFrame = originalCF end) end

    pcall(function()
        local eh = getEmoteHandler()
        if eh then eh:FireServer("StopEmote") end
    end)
end

-- ── Reset Status diri sendiri (WORKS: tidak butuh orang lain) ─
SurTab:Button({
    Title       = "🔄 Reset Status Diri Sendiri",
    Description = "Healing/Reset → bangunkan diri saat knocked, tidak butuh orang lain",
    Callback    = function()
        local ok, err = pcall(function()
            local rem = getHealReset()
            if not rem then error("Remote Healing/Reset tidak ditemukan") end
        ...
        WindUI:Notify({
            Title   = "Reset",
            Content = ok and "✅ Kamu berdiri kembali!" or "❌ Gagal: "..tostring(err),
            Duration = 2, Icon = ok and "refresh-cw" or "alert-circle"
        })
    end
})

-- Auto Reset saat knocked
local autoResetEnabled = false
SurTab:Toggle({
    Title       = "Auto Reset Status  (saat knocked, loop tiap 0.5s)",
    Description = "Kirim Healing/Reset terus-menerus. Matikan setelah berdiri.",
    Value       = false,
    Callback    = function(v)
        autoResetEnabled = v
        if v then
            task.spawn(function()
                while autoResetEnabled do
                    pcall(function()
                        local rem = getHealReset()
                        if rem then rem:FireServer(LocalPlayer) end
                        local eh = getEmoteHandler()
                        if eh then eh:FireServer("StopEmote") end
                    end)
                    task.wait(0.5)
            ...
            WindUI:Notify({ Title="Auto Reset", Content="Aktif! Matikan setelah berdiri.", Duration=2, Icon="refresh-cw" })
        end
    end
})

-- ── Heal / Revive Player Lain ─────────────────────────────────
SurTab:Section({ Title = "Heal / Revive Player Lain", Icon = "users" })
...
do
    ...
    SurTab:Button({
        Title       = "💊 Heal Luka Player yang Dipilih",
        Description = "HealEvent(HRP, false) → sembuhkan luka biasa",
        Callback    = function()
            ...
            local ok, err = pcall(function()
                ...
                if not (target and target.Character) then error("Target tidak valid") end
                local hrp = target.Character:FindFirstChild("HumanoidRootPart")
                if not hrp then error("Target tidak punya HRP") end
                fireHeal(hrp, false)
            end)
            WindUI:Notify({
                Title   = "Heal",
                Content = ok and "✅ Heal → "..healTargetName or "❌ Gagal: "..tostring(err),
    ...
    SurTab:Button({
        Title       = "💙 Revive Player yang Dipilih  (saat knocked)",
        Description = "HealEvent(HRP, true) → bangunkan player yang di-knock",
        Callback    = function()
            if healTargetName == "" or healTargetName:find("Tidak ada") then
            ...
            WindUI:Notify({
                Title   = "Revive",
                Content = ok and "✅ Revive → "..healTargetName or "❌ Gagal: "..tostring(err),
                Duration = 2, Icon = ok and "heart" or "alert-circle"
            })
        end
    })

    local autoHealAllEnabled = false
    SurTab:Toggle({
        Title       = "Auto Heal Semua Player  (loop tiap 1s)",
        Description = "Heal semua survivor lain secara otomatis",
        Value       = false,
        Callback    = function(v)
            autoHealAllEnabled = v
            if v then
                task.spawn(function()
                    while autoHealAllEnabled do
                        pcall(function()
                            local rem = getHealEvent()
                            local eh  = getEmoteHandler()
                            ...
                            for _, pl in ipairs(Players:GetPlayers()) do
                                if pl ~= LocalPlayer and pl.Character then
                                    local hrp = pl.Character:FindFirstChild("HumanoidRootPart")
                                    if hrp then
                                        rem:FireServer(hrp, false)
                                        task.wait(0.1)
                                        rem:FireServer(hrp, true)
                                    end
                                end
                            end
                            if eh then eh:FireServer("StopEmote") end
...
-- ── State & lingkaran visual ─────────────────────────────────────
_G.AutoUnhook   = false
_G.UnhookRange  = 15

-- Lingkaran aura (cylinder tipis di bawah karakter)
local UnhookRing = Instance.new("Part")
UnhookRing.Name        = "VD_UnhookRing"
UnhookRing.Shape       = Enum.PartType.Cylinder
UnhookRing.Material    = Enum.Material.Neon
UnhookRing.Color       = Color3.fromRGB(200, 150, 255)
UnhookRing.Transparency = 1      -- mulai tersembunyi
UnhookRing.CanCollide  = false
UnhookRing.Anchored    = true
UnhookRing.CastShadow  = false
UnhookRing.Size        = Vector3.new(0.12, _G.UnhookRange * 2, _G.UnhookRange * 2)
UnhookRing.Parent      = workspace

-- Update posisi & ukuran ring setiap frame
RunService.RenderStepped:Connect(function()
    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if hrp and _G.AutoUnhook then
        UnhookRing.Transparency = 0.65
        UnhookRing.Size = Vector3.new(0.12, _G.UnhookRange * 2, _G.UnhookRange * 2)
        -- Tengah badan = HumanoidRootPart.Position (sudah di tengah)
        -- Cylinder horizontal → rotate 90° Z, tidak perlu offset Y
        UnhookRing.CFrame = CFrame.new(hrp.Position) * CFrame.Angles(0, 0, math.rad(90))
    else
        UnhookRing.Transparency = 1
    end
end)

-- ============================================================
-- KILL AURA — state, ring, logic
-- ============================================================
_G.KillAura      = false
_G.KillAuraRange = 12    -- studs default
local killAuraConn = nil

-- Ring visual Kill Aura (merah, terpisah dari unhook)
local KillAuraRing = Instance.new("Part")
KillAuraRing.Name         = "VD_KillAuraRing"
KillAuraRing.Shape        = Enum.PartType.Cylinder
KillAuraRing.Material     = Enum.Material.Neon
KillAuraRing.Color        = Color3.fromRGB(255, 60, 60)
KillAuraRing.Transparency = 1
KillAuraRing.CanCollide   = false
KillAuraRing.Anchored     = true
KillAuraRing.CastShadow   = false
KillAuraRing.Size         = Vector3.new(0.12, _G.KillAuraRange * 2, _G.KillAuraRange * 2)
KillAuraRing.Parent       = workspace

-- Update posisi ring setiap frame — TENGAH badan
RunService.RenderStepped:Connect(function()
    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if hrp and _G.KillAura then
        KillAuraRing.Transparency = 0.55
        KillAuraRing.Size  = Vector3.new(0.12, _G.KillAuraRange * 2, _G.KillAuraRange * 2)
        KillAuraRing.CFrame = CFrame.new(hrp.Position) * CFrame.Angles(0, 0, math.rad(90))
    else
        KillAuraRing.Transparency = 1
    end
end)
            ...
            if pl ~= LocalPlayer and pl.Character then
                ...
                if tHRP then
                    ...
                    if dist <= _G.KillAuraRange then
                        -- FireServer BasicAttack (target = true agar cek server)
                        pcall(function()
                            ReplicatedStorage.Remotes.Attacks.BasicAttack:FireServer(true)
                        end)
                        -- Fallback remote path lain
                        pcall(function()
                            ReplicatedStorage.Remotes.Killer.BasicAttack:FireServer(pl.Character)
                        end)
                    end
                end
            end
...
-- ── Fungsi unhook utama ──────────────────────────────────────────
-- Remote unhook: dicache sekali saja (FindFirstChild, tidak block)
-- ✅ FIX v1.5: Path dari RemoteSpy = Remotes/Carry/UnHookEvent
-- Arg = workspace.Map.Hook.HookPoint (BasePart bernama "HookPoint" di dalam model Hook)
local _unhookRemoteCache = nil
local function getUnhookRemote()
    if _unhookRemoteCache and _unhookRemoteCache.Parent then return _unhookRemoteCache end
    local rem = ReplicatedStorage:FindFirstChild("Remotes")
    if not rem then return nil end

    -- PATH UTAMA (dari RemoteSpy): Carry/UnHookEvent
    local ok1, r1 = pcall(function()
        return rem:WaitForChild("Carry", 3):WaitForChild("UnHookEvent", 3)
    end)
    if ok1 and r1 then _unhookRemoteCache = r1; return r1 end

    -- Fallback: scan semua descendant nama mengandung "unhook"
    for _, v in ipairs(rem:GetDescendants()) do
        if v:IsA("RemoteEvent") and v.Name:lower():find("unhook") then
            _unhookRemoteCache = v; return v
        end
    end

    warn("[VD Unhook] Remote Carry/UnHookEvent tidak ditemukan!")
    return nil
end

-- ✅ FIX v1.5: arg yang benar = HookPoint (BasePart) di dalam model Hook
-- RemoteSpy: workspace.Map.Hook.HookPoint
local function getHookPoint(hookModel)
    -- hookModel bisa berupa Model "Hook" atau langsung BasePart
    if hookModel:IsA("BasePart") then
        -- Kalau sudah BasePart, cek namanya
        if hookModel.Name == "HookPoint" then return hookModel end
        -- Coba ambil dari parent
        if hookModel.Parent and hookModel.Parent:FindFirstChild("HookPoint") then
            return hookModel.Parent:FindFirstChild("HookPoint")
        end
        return hookModel
    end
    -- hookModel adalah Model → cari child bernama "HookPoint"
    local hp = hookModel:FindFirstChild("HookPoint")
    if hp and hp:IsA("BasePart") then return hp end
    -- Fallback ke BasePart apapun
    return hookModel:FindFirstChildWhichIsA("BasePart")
end

local function tryFireUnhook(hookObj)
    local remote = getUnhookRemote()
    if not remote then return end

    local hookPoint = getHookPoint(hookObj)
    ...
    -- ✅ FIX v1.6: Ghost Teleport — pindah ke hook, fire, balik
    -- Server cek posisi player harus dekat hook → kita ikuti aturan itu
    local originalCF = myHRP.CFrame
    local hookCF     = hookPoint.CFrame + Vector3.new(0, 0, 2)  -- sedikit di depan hook

    local ok, err = pcall(function()
        myHRP.CFrame = hookCF          -- ghost teleport ke hook
        task.wait()                    -- 1 frame agar server terima posisi baru
        remote:FireServer(hookPoint)   -- fire unhook
        task.wait()                    -- 1 frame buffer
        myHRP.CFrame = originalCF      -- balik ke posisi semula
    end)

    if not ok then
        warn("[VD Unhook] FireServer gagal: " .. tostring(err))
        -- Coba tetap balik posisi kalau error
        pcall(function() myHRP.CFrame = originalCF end)
    end
end

local function runAutoUnhook()
    task.spawn(function()
        while _G.AutoUnhook do
            local myChar = LocalPlayer.Character
            local myHRP  = myChar and myChar:FindFirstChild("HumanoidRootPart")

            if myHRP then
                local hooks = getAllHooks()
                for _, hookObj in ipairs(hooks) do
                    if not _G.AutoUnhook then break end

                    local refPart = getHookPoint(hookObj)

                    if refPart then
                        -- CEK RADIUS AURA: Hanya proses hook yang ada di dalam lingkaran ungu
                        local distToHook = (myHRP.Position - refPart.Position).Magnitude
                        if distToHook <= _G.UnhookRange then

                            -- Cek apakah benar-benar ada player yang DIAM tergantung di hook
                            local hasVictim = false
                            for _, pl in ipairs(Players:GetPlayers()) do
                                if pl ~= LocalPlayer and pl.Character then
                                    local plHRP = pl.Character:FindFirstChild("HumanoidRootPart")
                                    -- Jarak diperkecil < 4 agar tidak trigger orang yang sekadar lewat
                                    if plHRP and (plHRP.Position - refPart.Position).Magnitude < 4 then
                                        hasVictim = true; break
                                    end
                                end
                            end

                            if hasVictim then
                                tryFireUnhook(hookObj)
                                WindUI:Notify({
                                    Title    = "Auto Unhook",
                                    Content  = "✅ Unhook → " .. refPart.Name,
                                    Duration = 1.5,
                                    Icon     = "anchor"
                                })
                                task.wait(0.5)
...
-- ── UI GriefTab ──────────────────────────────────────────────────
GriefTab:Section({ Title = "Auto Unhook (Aura)", Icon = "anchor" })

GriefTab:Toggle({
    Title       = "Auto Ghost Unhook",
    Description = "Otomatis lepas survivor dari hook dalam jangkauan. Lingkaran ungu = radius aktif.",
    Value       = false,
    Callback    = function(v)
        _G.AutoUnhook = v
        if v then
            ...
            WindUI:Notify({
                Title    = "Auto Unhook",
                Content  = "Aktif! Jangkauan: " .. _G.UnhookRange .. " studs",
                Duration = 2,
                Icon     = "anchor"
            })
        else
...
GriefTab:Slider({
    Title       = "Unhook Aura Range  (studs)",
    Description = "Radius jangkauan untuk detect & unhook. Lingkaran ikut berubah.",
    Value       = { Min = 5, Max = 100, Default = 15 },
    Step        = 1,
    Callback    = function(v)
        pcall(function()
            _G.UnhookRange = v
            UnhookRing.Size = Vector3.new(0.12, v * 2, v * 2)
        end)
    end
})

GriefTab:Colorpicker({
    Title   = "Warna Lingkaran Aura",
    Desc    = "Ubah warna lingkaran visual aura unhook",
    Default = Color3.fromRGB(200, 150, 255),
    Transparency = 0,
    Callback = function(v)
        pcall(function()
            UnhookRing.Color = v
...
-- ── Manual Unhook (pilih PLAYER yang tergantung di hook) ──────
GriefTab:Section({ Title = "Manual Unhook", Icon = "anchor" })

do
    local unhookTargetName = ""

    -- Bangun list player yang sedang tergantung di hook
    -- (dekat hook dalam 8 studs)
    local function buildUnhookPlayerList()
        local list = {}
        local map = workspace:FindFirstChild("Map")
        local hookFolder = map and (map:FindFirstChild("Hook") or map:FindFirstChild("Hooks"))
        if not hookFolder then
            -- Fallback: semua player selain kita
            for _, pl in ipairs(Players:GetPlayers()) do
                if pl ~= LocalPlayer then table.insert(list, pl.Name) end
            end
            return #list > 0 and list or {"(Tidak ada player)"}
        end

        -- Kumpulkan semua HookPoint
        local hookParts = {}
        for _, h in pairs(hookFolder:GetChildren()) do
            local part = h:IsA("BasePart") and h
                or h:FindFirstChild("HookPoint")
                or h:FindFirstChildWhichIsA("BasePart")
            if part then table.insert(hookParts, part) end
        end

        -- Cari player yang posisinya dekat salah satu hook
        for _, pl in ipairs(Players:GetPlayers()) do
            if pl ~= LocalPlayer and pl.Character then
                local plHRP = pl.Character:FindFirstChild("HumanoidRootPart")
                if plHRP then
                    for _, hp in ipairs(hookParts) do
                        if (plHRP.Position - hp.Position).Magnitude < 10 then
                            table.insert(list, pl.Name .. " [DI HOOK]")
                            break
                        end
                    end
                end
        ...
        -- Kalau tidak ada yang terdeteksi di hook, tampilkan semua player
        if #list == 0 then
            for _, pl in ipairs(Players:GetPlayers()) do
                if pl ~= LocalPlayer then table.insert(list, pl.Name) end
            end
    ...
    local unhookDrop = GriefTab:Dropdown({
        Title    = "Pilih Player yang Mau Di-Unhook",
        Values   = buildUnhookPlayerList(),
        Value    = "",
        Callback = function(v) unhookTargetName = v end
    })

    GriefTab:Button({
        Title    = "🔄 Refresh Daftar Player",
        Callback = function()
            pcall(function() unhookDrop:Refresh(buildUnhookPlayerList(), false) end)
            unhookTargetName = ""
        end
    })

    GriefTab:Button({
        Title       = "⚡ Unhook Player  (Instant Ghost TP + Balik + Stop Emote)",
        Description = "TP invisible ke player → cari hook terdekat → fire UnHook → balik → stop emote",
        Callback    = function()
            if unhookTargetName == "" or unhookTargetName:find("Tidak ada") then
                WindUI:Notify({ Title="Unhook", Content="Pilih player dulu!", Duration=2, Icon="alert-circle" })
                return
            end

            local ok, err = pcall(function()
                -- 1. Cari player target
                local targetName = unhookTargetName:match("^(.-)%s*%[") or unhookTargetName
                local target = nil
                for _, pl in ipairs(Players:GetPlayers()) do
                    if pl.Name == targetName then target = pl; break end
                end
                if not target or not target.Character then error("Player tidak valid") end
                local tHRP = target.Character:FindFirstChild("HumanoidRootPart")
                if not tHRP then error("HRP player tidak ditemukan") end

                -- 2. Cari HookPoint terdekat dari posisi player target (pakai getAllHooks - rekursif)
                local allHooks = getAllHooks()
                if #allHooks == 0 then error("Tidak ada hook di map") end

                local closestHookObj  = nil
                local closestHookPart = nil
                local closestDist     = math.huge

                for _, h in ipairs(allHooks) do
                    local part = getHookPoint(h)
                    if part then
                        local d = (tHRP.Position - part.Position).Magnitude
                        if d < closestDist then
                            closestDist    = d
                            closestHookPart = part
                            closestHookObj  = h
                        end
                    end
                end
                if not closestHookPart then error("HookPoint terdekat tidak ditemukan") end

                -- 3. Dapatkan remote
                local remote = getUnhookRemote()
                if not remote then error("Remote UnHookEvent tidak ditemukan") end

                -- 4. Simpan posisi asal kita
                local myChar = LocalPlayer.Character
                local myHRP  = myChar and myChar:FindFirstChild("HumanoidRootPart")
                if not myHRP then error("HRP kita tidak ada") end
                local originalCF = myHRP.CFrame

                -- 5. Ghost TP secepat kilat (tidak visible = 1 frame saja)
                --    TP ke sisi hook, fire, langsung balik, stop emote
                myHRP.CFrame = closestHookPart.CFrame + Vector3.new(0, 0, 2)
                -- Tidak task.wait() agar tidak terlihat sama sekali oleh render
                remote:FireServer(closestHookPart)
                myHRP.CFrame = originalCF

                -- 6. Stop emote langsung supaya tidak ada animasi sisa
                pcall(function()
                    local eh = ReplicatedStorage
                        :FindFirstChild("Remotes")
                        and ReplicatedStorage.Remotes:FindFirstChild("EmoteHandler")
                    if eh then eh:FireServer("StopEmote") end
                end)
            end)
            ...
            WindUI:Notify({
                Title   = "Unhook",
                Content = ok and "✅ "..unhookTargetName:match("^(.-)%s*%[?")..
                          " berhasil di-unhook!" or "❌ Gagal: "..tostring(err),
                Duration = 2,
                Icon    = ok and "anchor" or "alert-circle"
            })
        end
    })
end

-- ── TP Cepat ke Hook + Stop Emote ────────────────────────────
GriefTab:Section({ Title = "TP Cepat ke Hook", Icon = "map-pin" })

GriefTab:Button({
    Title       = "⚡ TP ke Hook Terdekat + Stop Emote",
    Description = "Teleport instan ke hook terdekat lalu stop emote otomatis",
    Callback    = function()
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if not hrp then
            WindUI:Notify({ Title="TP Hook", Content="Karakter tidak ditemukan!", Duration=2, Icon="alert-circle" })
            return
        end
        local map = workspace:FindFirstChild("Map")
        if not map then
            WindUI:Notify({ Title="TP Hook", Content="Map tidak ditemukan!", Duration=2, Icon="alert-circle" })
            return
        end
        local hookFolder = map:FindFirstChild("Hook") or map:FindFirstChild("Hooks")
        if not hookFolder then
            WindUI:Notify({ Title="TP Hook", Content="Folder Hook tidak ada di Map!", Duration=2, Icon="alert-circle" })
            return
        end

        local closest, closestDist = nil, math.huge
        for _, hook in pairs(hookFolder:GetChildren()) do
            local part = hook:IsA("BasePart") and hook
                or hook:FindFirstChild("HookPoint")
        ...
        if closest then
            -- TP langsung tanpa animasi
            hrp.CFrame = CFrame.new(closest.Position + Vector3.new(0, 3, 2))
            -- Stop emote supaya tidak stuck
            task.wait(0.05)
            pcall(function()
                local eh = ReplicatedStorage
                    :FindFirstChild("Remotes")
            ...
            WindUI:Notify({
                Title   = "TP Hook",
                Content = "⚡ TP ke hook (" .. math.floor(closestDist) .. " studs) + Emote dihentikan!",
                Duration = 2, Icon = "map-pin"
            })
        else
            WindUI:Notify({ Title="TP Hook", Content="Tidak ada hook di map.", Duration=2, Icon="alert-circle" })
        end
    end
})
...
GriefTab:Button({
    Title       = "🛑 Stop Emote",
    Description = "Paksa hentikan emote yang sedang berjalan",
    Callback    = function()
        local ok, err = pcall(function()
            local eh = ReplicatedStorage
                :WaitForChild("Remotes", 5)
                :WaitForChild("EmoteHandler", 5)
            eh:FireServer("StopEmote")
        end)
        WindUI:Notify({
            Title   = "Emote",
            Content = ok and "✅ Emote dihentikan!" or "❌ Gagal: "..tostring(err),
...
GriefTab:Section({ Title = "Hook ESP", Icon = "eye" })

local hookEspEnabled = false
local hookEspObjects = {}

local function clearHookESP()
    for _, obj in pairs(hookEspObjects) do
        pcall(function()
...
local function buildHookESP()
    ...
    for _, hook in pairs(hookFolder:GetChildren()) do
        ...
        if part then
            ...
            table.insert(hookEspObjects, { hl = hl, bb = bb })
        end
    end
end

GriefTab:Toggle({
    Title       = "Hook ESP  (lihat semua hook di map)",
    Description = "Highlight merah di setiap hook yang ada di peta",
    Value       = false,
    Callback    = function(v)
        hookEspEnabled = v
        if v then
...
GriefTab:Button({
    Title       = "Refresh Hook ESP",
    Description = "Paksa scan ulang hook (gunakan saat map berganti)",
    Callback    = function()
        if hookEspEnabled then
            buildHookESP()
            WindUI:Notify({ Title = "Hook ESP", Content = "Scan ulang selesai.", Duration = 1.5, Icon = "refresh-cw" })
        else
            WindUI:Notify({ Title = "Hook ESP", Content = "Aktifkan Hook ESP dulu!", Duration = 1.5, Icon = "alert-circle" })
        end
    end
})

GriefTab:Section({ Title = "Teleport Grief", Icon = "map-pin" })

GriefTab:Button({
    Title       = "TP ke Hook Terdekat",
    Description = "Teleport langsung ke hook terdekat dari posisimu",
    Callback    = function()
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        local map = workspace:FindFirstChild("Map")
        if not hrp or not map then
            WindUI:Notify({ Title = "TP Hook", Content = "Karakter atau Map tidak ditemukan!", Duration = 2, Icon = "alert-circle" })
            return
        end
        local hookFolder = map:FindFirstChild("Hook") or map:FindFirstChild("Hooks")
        if not hookFolder then
        ...
        if closest then
            hrp.CFrame = CFrame.new(closest.Position + Vector3.new(0, 3, 2))
            WindUI:Notify({ Title = "TP Hook", Content = "Teleport ke hook (" .. math.floor(closestDist) .. " studs)!", Duration = 2, Icon = "map-pin" })
        else
            WindUI:Notify({ Title = "TP Hook", Content = "Tidak ada hook ditemukan.", Duration = 2, Icon = "alert-circle" })
        end
    end
})

GriefTab:Section({ Title = "Map Sabotage", Icon = "zap" })

GriefTab:Button({
    Title       = "Break All Generators",
    Description = "Kirim remote untuk merusak semua generator di map",
    Callback    = function()
        local map  = workspace:FindFirstChild("Map")
        local gens = map and (map:FindFirstChild("Generator") or map:FindFirstChild("Generators"))
        if not map then
            WindUI:Notify({ Title = "Grief", Content = "Map tidak ditemukan!", Duration = 2, Icon = "alert-circle" })
            return
        end
        local count = 0
        for _, gen in pairs((gens and gens:GetChildren()) or map:GetChildren()) do
            if gen:IsA("Model") and gen.Name:lower():find("generator") then
                pcall(function()
                    ReplicatedStorage.Remotes.Generator.BreakGenEvent:FireServer(gen)
                end)
                count = count + 1
            end
        end
        WindUI:Notify({ Title = "Grief", Content = count .. " Generator di-break!", Duration = 3, Icon = "zap" })
    end
})

GriefTab:Button({
    Title       = "Destroy All Pallets",
    Description = "Kirim remote untuk menghancurkan semua pallet",
    Callback    = function()
        local count = 0
        for _, v in pairs(workspace:GetDescendants()) do
            if v.Name == "Pallet" or v.Name == "PalletStatic" or v.Name == "Palletwrong" then
                pcall(function()
                    ReplicatedStorage.Remotes.Pallet.Jason["Destroy-Global"]:FireServer(v)
                end)
                count = count + 1
            end
        end
        WindUI:Notify({ Title = "Grief", Content = count .. " Pallet dihancurkan!", Duration = 3, Icon = "zap" })
    end
})

GriefTab:Button({
    Title       = "Drop All Pallets  (jatuhkan semua pallet)",
    Description = "Kirim PalletDropEvent ke setiap PalletPointSlide di map",
    Callback    = function()
        -- Pakai FindFirstChild (bukan WaitForChild — tidak block)
        local remPallet = ReplicatedStorage:FindFirstChild("Remotes")
            and ReplicatedStorage.Remotes:FindFirstChild("Pallet")
        local dropRemote = remPallet and remPallet:FindFirstChild("PalletDropEvent")

        if not dropRemote then
            WindUI:Notify({ Title = "Drop Pallet", Content = "Remote PalletDropEvent tidak ditemukan!", Duration = 3, Icon = "alert-circle" })
            return
        end

        -- Pakai getAllPalletPoints() — scan Map secara rekursif
        local points = getAllPalletPoints()
        local count = 0
        for _, obj in ipairs(points) do
            pcall(function()
                dropRemote:FireServer(obj)
                count = count + 1
            end)
            task.wait(0.05)
        end
        WindUI:Notify({
            Title    = "Grief",
            Content  = count .. " Pallet dijatuhkan! (ditemukan " .. #points .. " point)",
            Duration = 3,
            Icon     = "zap"
        })
    end
...
-- ── KILL AURA UI ─────────────────────────────────────────────────
GriefTab:Section({ Title = "Kill Aura (Killer)", Icon = "sword" })

GriefTab:Toggle({
    Title       = "Kill Aura",
    Description = "Otomatis BasicAttack semua player dalam radius. Lingkaran merah = jangkauan.",
    Value       = false,
    Callback    = function(v)
        _G.KillAura = v
        if v then
            startKillAura()
            WindUI:Notify({ Title = "Kill Aura", Content = "Aktif! Range: " .. _G.KillAuraRange .. " studs", Duration = 2, Icon = "sword" })
        else
            stopKillAura()
            WindUI:Notify({ Title = "Kill Aura", Content = "Dimatikan.", Duration = 2, Icon = "sword" })
        end
    end
})
...
GriefTab:Slider({
    ...
    Description = "Radius serangan. Lingkaran merah ikut berubah.",
    Value       = { Min = 3, Max = 60, Default = 12 },
    Step        = 1,
    Callback    = function(v)
        _G.KillAuraRange = v
        KillAuraRing.Size = Vector3.new(0.12, v * 2, v * 2)
    end
})

GriefTab:Colorpicker({
    Title   = "Warna Ring Kill Aura",
    Desc    = "Ubah warna lingkaran merah kill aura",
    Default = Color3.fromRGB(255, 60, 60),
    Transparency = 0,
    Callback = function(v)
        pcall(function() KillAuraRing.Color = v end)
    end
})

-- ── Self Features (Khusus Diri Sendiri) ──────────────────────
-- Fungsi: Self Heal / Revive diri sendiri
-- Spam HealEvent + StopEmote agar server menerima
local function selfHealRevive()
    local char = LocalPlayer.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    local rem  = getHealEvent()

    if hrp and rem then
        local myOriginalCF = hrp.CFrame
        -- Ghost TP ke diri sendiri (trick: geser 0,0,0 = tetap posisi)
        -- Kirim sinyal berulang kali agar server menerima salah satunya
        for i = 1, 10 do
            pcall(function() rem:FireServer(hrp, true) end) -- true = revive/up
            task.wait(0.01)
        end
        -- Paksa stop animasi knocked agar bisa bergerak kembali
        pcall(function()
            local eh = getEmoteHandler()
            if eh then eh:FireServer("StopEmote") end
        end)
        WindUI:Notify({
            Title   = "Self Action",
            Content = "✅ Mencoba Revive Diri Sendiri...",
            Duration = 2, Icon = "heart"
        })
    else
        WindUI:Notify({
            Title   = "Self Action",
            Content = "❌ Gagal: Remote atau karakter tidak ditemukan",
            Duration = 2, Icon = "alert-circle"
        })
    end
end

-- Fungsi: Lepas dari Hook sendiri
local function selfUnhook()
    local myChar = LocalPlayer.Character
    local myHRP  = myChar and myChar:FindFirstChild("HumanoidRootPart")
    if not myHRP then
        WindUI:Notify({ Title="Self Unhook", Content="❌ Karakter tidak ditemukan", Duration=2, Icon="alert-circle" })
        return
    end

    local hooks = getAllHooks()
    local targetedHook = nil

    -- Cari Hook yang jaraknya sangat dekat dengan badan kita (< 5 studs)
    for _, h in ipairs(hooks) do
        local p = getHookPoint(h)
        if p and (myHRP.Position - p.Position).Magnitude < 5 then
            targetedHook = h
            break
        end
    end

    if targetedHook then
        local hookPt = getHookPoint(targetedHook)
        local ok, err = pcall(function()
            local rem = ReplicatedStorage
                :WaitForChild("Remotes", 5)
                :WaitForChild("Carry", 5)
                :WaitForChild("UnHookEvent", 5)
            rem:FireServer(hookPt)
        end)
        WindUI:Notify({
            Title   = "Self Unhook",
            Content = ok and "✅ Berhasil mengirim remote Unhook!" or "❌ Gagal: "..tostring(err),
            Duration = 2, Icon = ok and "anchor" or "alert-circle"
        })
    else
        WindUI:Notify({
            Title   = "Self Unhook",
            Content = "❌ Kamu tidak terdeteksi sedang di Hook (jarak > 5 studs)",
            Duration = 2, Icon = "alert-circle"
        })
    end
end

GriefTab:Section({ Title = "Self Features (Khusus Diri Sendiri)", Icon = "user" })

GriefTab:Button({
    Title       = "💊 Sembuhkan / Bangunkan Diri",
    Description = "Klik jika kamu sedang Down atau darah tipis",
    Callback    = function() selfHealRevive() end
})

GriefTab:Button({
    Title       = "🪝 Lepas dari Hook (Self Unhook)",
    Description = "Gunakan saat kamu sedang digantung di hook",
    Callback    = function() selfUnhook() end
})
