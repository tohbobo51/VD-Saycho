--[[
  ...
  Version : 16.0
  Author  : Saycho
  New v16 : Custom Crosshair (5 shape + 5 preset warna + HSV Color Picker)
]]
...
-- ============================================================
-- PRINT DIAGNOSTIK — muncul di console Delta
-- Kalau script crash, liat print terakhir yang muncul
-- ============================================================
local function dbg(msg) print("[VD v17] " .. msg) end

dbg("START — Services OK")

-- ⚠️ FORWARD DECLARE: startMoonwalk/stopMoonwalk
-- Wajib ada di sini karena MoonBtn.Activated memanggilnya
-- sebelum fungsinya didefinisikan (ini bug utama v16)
local startMoonwalk, stopMoonwalk
...
local R_HealEvent      = safeRemote("Healing", "HealEvent")          -- NEW dari remotespy
local R_TwistOfFate    = safeRemote("Items", "Twist of Fate", "Fire") -- NEW dari remotespy
...
dbg("Remotes loaded — RepairEvent=" .. tostring(R_RepairEvent ~= nil)
    .. " BasicAttack=" .. tostring(R_BasicAttack ~= nil)
    .. " FastVault=" .. tostring(R_FastVault ~= nil))
...
dbg("ScreenGui OK — parent=" .. tostring(ScreenGui and ScreenGui.Parent and ScreenGui.Parent.Name))
...
local States = {
    ...
    CrosshairColor = Color3.fromRGB(255, 140, 0),
...
MoonBtn.Activated:Connect(function()
...
SubLabel.Text = "Violence District  v16.0"; SubLabel.Position = UDim2.new(0, 82, 0, 16)
...
local crosshairParts = {}  -- track semua frame crosshair untuk recolor live

local function buildCrosshairShape(container, shape, color)
    for _, c in ipairs(container:GetChildren()) do c:Destroy() end
    crosshairParts = {}
    local function mk(w, h, ox, oy, r)
        local f = Instance.new("Frame", container)
        f.AnchorPoint = Vector2.new(0.5, 0.5)
        f.Position = UDim2.new(0.5, ox, 0.5, oy)
        f.Size = UDim2.new(0, w, 0, h)
        f.BackgroundColor3 = color; f.BorderSizePixel = 0
        if r then Instance.new("UICorner", f).CornerRadius = UDim.new(r, 0) end
        table.insert(crosshairParts, f)
    end
    if shape == "Cross" then
        mk(2, 14, 0, 0); mk(14, 2, 0, 0); mk(4, 4, 0, 0, 1)
    elseif shape == "Dot" then
        mk(7, 7, 0, 0, 1)
    elseif shape == "Circle" then
        mk(12, 2, 0, -9); mk(12, 2, 0, 9)
        mk(2, 12, -9, 0); mk(2, 12, 9, 0)
        mk(4, 4, 0, 0, 1)
    elseif shape == "Square" then
        mk(7, 2, -6, -6); mk(2, 7, -6, -6)
        mk(7, 2,  6, -6); mk(2, 7,  6, -6)
        mk(7, 2, -6,  6); mk(2, 7, -6,  6)
        mk(7, 2,  6,  6); mk(2, 7,  6,  6)
    elseif shape == "Arrow" then
        mk(2, 12, 0, 2); mk(9, 2, 0, -8)
        mk(2, 6, -3, -5); mk(2, 6, 3, -5)
    end
end

local function refreshCrosshairColor(color)
    States.CrosshairColor = color
    for _, f in ipairs(crosshairParts) do pcall(function() f.BackgroundColor3 = color end) end
end

local function setCrosshair(on)
    if crosshairGui then crosshairGui:Destroy(); crosshairGui = nil; crosshairParts = {} end
    if not on then return end
    local sg = Instance.new("ScreenGui")
    sg.Name = "VD_Crosshair"; sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true; sg.DisplayOrder = 999; sg.Parent = PlayerGui
    crosshairGui = sg
    local ctn = Instance.new("Frame", sg)
    ctn.AnchorPoint = Vector2.new(0.5, 0.5)
    ctn.Position = UDim2.new(0.5, 0, 0.5, 0)
    ctn.Size = UDim2.new(0, 32, 0, 32)
    ctn.BackgroundTransparency = 1
    buildCrosshairShape(ctn, States.CrosshairShape, States.CrosshairColor)
end

-- HSV Color Picker popup
local function OpenColorPicker(defaultColor, onChanged, anchorBtn)
    local old = ScreenGui:FindFirstChild("VD_ColorPop")
    if old then old:Destroy(); return end
    local H, S, V = Color3.toHSV(defaultColor)
    local PW, PH = 218, 170
    local pop = Instance.new("Frame", ScreenGui)
    pop.Name = "VD_ColorPop"; pop.Size = UDim2.new(0, PW, 0, PH)
    pop.BackgroundColor3 = Color3.fromRGB(18, 18, 18); pop.BorderSizePixel = 0; pop.ZIndex = 300
    ...
    if anchorBtn then
        ...
        pop.Position = UDim2.new(0, ap.X - PW - 6, 0, ap.Y - 20)
    else
        pop.Position = UDim2.new(0.5, -PW/2, 0.5, -PH/2)
    end
    local bar = Instance.new("TextButton", pop)
    bar.Size = UDim2.new(1, 0, 0, 24); bar.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
    bar.Text = "   🎨 Color Picker"; bar.TextColor3 = Color3.fromRGB(200, 200, 200)
    bar.Font = Enum.Font.GothamBold; bar.TextSize = 11
    bar.TextXAlignment = Enum.TextXAlignment.Left; bar.BorderSizePixel = 0
    Instance.new("UICorner", bar).CornerRadius = UDim.new(0, 10)
    MakeDraggable(pop, bar)
    local cx = Instance.new("TextButton", bar)
    cx.Size = UDim2.new(0, 18, 0, 18); cx.Position = UDim2.new(1, -22, 0.5, -9)
    cx.Text = "✕"; cx.TextSize = 10; cx.TextColor3 = Color3.new(1, 1, 1)
    cx.BackgroundColor3 = Color3.fromRGB(180, 40, 40); cx.BorderSizePixel = 0
    Instance.new("UICorner", cx).CornerRadius = UDim.new(1, 0)
    cx.Activated:Connect(function() pop:Destroy() end)
    local svW, svH = PW - 46, PH - 52
    local svBase = Instance.new("Frame", pop)
    svBase.Size = UDim2.new(0, svW, 0, svH); svBase.Position = UDim2.new(0, 8, 0, 30)
    svBase.BackgroundColor3 = Color3.fromHSV(H, 1, 1); svBase.BorderSizePixel = 0
    ...
    Instance.new("UIGradient", wOvl).Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0,0), NumberSequenceKeypoint.new(1,1)})
    ...
    svKnob.Size = UDim2.new(0,10,0,10); svKnob.AnchorPoint = Vector2.new(0.5,0.5)
    svKnob.Position = UDim2.new(S,0,1-V,0); svKnob.BackgroundColor3 = Color3.new(1,1,1)
    svKnob.BorderSizePixel = 0; svKnob.ZIndex = 10
    Instance.new("UICorner", svKnob).CornerRadius = UDim.new(1, 0)
    Instance.new("UIStroke", svKnob).Color = Color3.new(0, 0, 0)
    local hueStrip = Instance.new("Frame", pop)
    hueStrip.Size = UDim2.new(0, 16, 0, svH); hueStrip.Position = UDim2.new(0, svW+14, 0, 30)
    hueStrip.BorderSizePixel = 0
    Instance.new("UICorner", hueStrip).CornerRadius = UDim.new(0, 5)
    local hg = Instance.new("UIGradient", hueStrip); hg.Rotation = 90
    ...
    hKnob.Position = UDim2.new(0.5,0,H,0); hKnob.BackgroundColor3 = Color3.new(1,1,1); hKnob.BorderSizePixel = 0
    Instance.new("UIStroke", hKnob).Color = Color3.new(0, 0, 0)
    local preview = Instance.new("Frame", pop)
    preview.Size = UDim2.new(1,-16,0,12); preview.Position = UDim2.new(0,8,1,-18)
    ...
    local function refresh()
        ...
        if anchorBtn then pcall(function() anchorBtn.BackgroundColor3 = col end) end
        pcall(onChanged, col)
    end
    local svDrag, hueDrag = false, false
    bOvl.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then svDrag=true end end)
    hueStrip.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then hueDrag=true end end)
    UIS.InputEnded:Connect(function(i)
    ...
    UIS.InputChanged:Connect(function(i)
        ...
        if svDrag then
            ...
            S=math.clamp((i.Position.X-ap.X)/as.X,0,1); V=1-math.clamp((i.Position.Y-ap.Y)/as.Y,0,1); refresh()
        elseif hueDrag then
            local ap=hueStrip.AbsolutePosition; local as=hueStrip.AbsoluteSize
            H=math.clamp((i.Position.Y-ap.Y)/as.Y,0,0.999); refresh()
        end
    end)
    refresh()
end
...
-- ✅ FIX v17: Assign ke forward-declared local (bukan redeclare local baru)
...
dbg("Moonwalk OK — forward declaration resolved")
...
SectionLabel(VisPage, "CROSSHAIR", 5)

do
    -- ON/OFF toggle row
    local xhRow = Instance.new("Frame", VisPage)
    xhRow.Size=UDim2.new(1,0,0,46); xhRow.BackgroundColor3=C.CARD; xhRow.BorderSizePixel=0; xhRow.LayoutOrder=51
    Instance.new("UICorner",xhRow).CornerRadius=UDim.new(0,8)
    local xhLbl=Instance.new("TextLabel",xhRow)
    xhLbl.Text="🎯 Custom Crosshair"; xhLbl.Position=UDim2.new(0,12,0,6); xhLbl.Size=UDim2.new(0,180,0,16)
    xhLbl.TextColor3=Color3.new(1,1,1); xhLbl.Font=Enum.Font.GothamBold; xhLbl.TextSize=13; xhLbl.BackgroundTransparency=1; xhLbl.TextXAlignment=Enum.TextXAlignment.Left
    local xhSub=Instance.new("TextLabel",xhRow)
    xhSub.Text="Bantu akurasi Twist of Fate (FPS style)"; xhSub.Position=UDim2.new(0,12,0,24); xhSub.Size=UDim2.new(1,-70,0,14)
    xhSub.TextColor3=Color3.fromRGB(110,110,110); xhSub.Font=Enum.Font.Gotham; xhSub.TextSize=10; xhSub.BackgroundTransparency=1; xhSub.TextXAlignment=Enum.TextXAlignment.Left
    local xhTrack=Instance.new("TextButton",xhRow)
    xhTrack.Size=UDim2.new(0,44,0,23); xhTrack.Position=UDim2.new(1,-54,0.5,-11)
    xhTrack.BackgroundColor3=C.OFF; xhTrack.Text=""; xhTrack.BorderSizePixel=0
    Instance.new("UICorner",xhTrack).CornerRadius=UDim.new(1,0)
    local xhKnob=Instance.new("Frame",xhTrack)
    xhKnob.Size=UDim2.new(0,17,0,17); xhKnob.Position=UDim2.new(0,3,0.5,-8)
    xhKnob.BackgroundColor3=C.TEXT_A; xhKnob.BorderSizePixel=0
    Instance.new("UICorner",xhKnob).CornerRadius=UDim.new(1,0)
    xhTrack.Activated:Connect(function()
        States.Crosshair = not States.Crosshair
        setCrosshair(States.Crosshair)
        tw(xhTrack,{BackgroundColor3 = States.Crosshair and C.ACCENT or C.OFF})  -- FIX: C.ACCENT bukan C.ON
        tw(xhKnob, {Position = States.Crosshair and UDim2.new(1,-20,0.5,-8) or UDim2.new(0,3,0.5,-8)})
    end)

    -- Shape selector
    local shapeRow=Instance.new("Frame",VisPage)
    shapeRow.Size=UDim2.new(1,0,0,40); shapeRow.BackgroundColor3=Color3.fromRGB(24,24,24); shapeRow.BorderSizePixel=0; shapeRow.LayoutOrder=52
    Instance.new("UICorner",shapeRow).CornerRadius=UDim.new(0,8)
    local shapeLbl=Instance.new("TextLabel",shapeRow)
    shapeLbl.Text="Bentuk"; shapeLbl.Position=UDim2.new(0,12,0,0); shapeLbl.Size=UDim2.new(0,55,1,0)
    shapeLbl.TextColor3=C.TEXT_B; shapeLbl.Font=Enum.Font.GothamBold; shapeLbl.TextSize=11; shapeLbl.BackgroundTransparency=1; shapeLbl.TextXAlignment=Enum.TextXAlignment.Left
    local shapes = {"Cross","Dot","Circle","Square","Arrow"}
    local symbols = {Cross="✛",Dot="●",Circle="◯",Square="⬜",Arrow="▲"}
    local sx = 66; local shapeBtns = {}
    for _, sh in ipairs(shapes) do
        local sb=Instance.new("TextButton",shapeRow)
        sb.Size=UDim2.new(0,36,0,28); sb.Position=UDim2.new(0,sx,0.5,-14)
        sb.Text=symbols[sh]; sb.TextSize=14
        sb.BackgroundColor3 = sh==States.CrosshairShape and C.ACCENT or Color3.fromRGB(38,38,38)
        sb.TextColor3 = sh==States.CrosshairShape and Color3.new(0,0,0) or Color3.new(1,1,1)
        sb.BorderSizePixel=0; Instance.new("UICorner",sb).CornerRadius=UDim.new(0,7)
        table.insert(shapeBtns, {btn=sb, sh=sh})
        sb.Activated:Connect(function()
            States.CrosshairShape = sh
            for _, e in ipairs(shapeBtns) do
                tw(e.btn,{BackgroundColor3=e.sh==sh and C.ACCENT or Color3.fromRGB(38,38,38)})
                e.btn.TextColor3 = e.sh==sh and Color3.new(0,0,0) or Color3.new(1,1,1)
            end
            if States.Crosshair then setCrosshair(true) end
        end)
        sx = sx + 40
    end

    -- Color row
    local colorRow=Instance.new("Frame",VisPage)
    colorRow.Size=UDim2.new(1,0,0,40); colorRow.BackgroundColor3=Color3.fromRGB(24,24,24); colorRow.BorderSizePixel=0; colorRow.LayoutOrder=53
    Instance.new("UICorner",colorRow).CornerRadius=UDim.new(0,8)
    local colorLbl=Instance.new("TextLabel",colorRow)
    colorLbl.Text="Warna"; colorLbl.Position=UDim2.new(0,12,0,0); colorLbl.Size=UDim2.new(0,55,1,0)
    colorLbl.TextColor3=C.TEXT_B; colorLbl.Font=Enum.Font.GothamBold; colorLbl.TextSize=11; colorLbl.BackgroundTransparency=1; colorLbl.TextXAlignment=Enum.TextXAlignment.Left
    local presets = {
        Color3.fromRGB(255,140,0), Color3.fromRGB(255,255,255),
        Color3.fromRGB(0,255,100), Color3.fromRGB(80,180,255), Color3.fromRGB(255,80,80),
    }
    local cx2 = 66
    for _, col in ipairs(presets) do
        local sw=Instance.new("TextButton",colorRow)
        sw.Size=UDim2.new(0,24,0,24); sw.Position=UDim2.new(0,cx2,0.5,-12)
        sw.BackgroundColor3=col; sw.Text=""; sw.BorderSizePixel=0
        Instance.new("UICorner",sw).CornerRadius=UDim.new(1,0)
        sw.Activated:Connect(function() refreshCrosshairColor(col) end)
        cx2=cx2+28
    end
    -- 🎨 Custom picker button
    local pickerBtn=Instance.new("TextButton",colorRow)
    pickerBtn.Size=UDim2.new(0,24,0,24); pickerBtn.Position=UDim2.new(0,cx2,0.5,-12)
    pickerBtn.BackgroundColor3=States.CrosshairColor; pickerBtn.Text="🎨"; pickerBtn.TextSize=12
    pickerBtn.BorderSizePixel=0; pickerBtn.TextColor3=Color3.new(1,1,1)
    Instance.new("UICorner",pickerBtn).CornerRadius=UDim.new(1,0)
    pickerBtn.Activated:Connect(function()
        OpenColorPicker(States.CrosshairColor, function(col)
            refreshCrosshairColor(col)
            pickerBtn.BackgroundColor3 = col
        end, pickerBtn)
    end)
end
...
print("[VD v17.0] Loaded — Saycho | FIX: startMoonwalk forward declaration | Diagnostik prints aktif")
