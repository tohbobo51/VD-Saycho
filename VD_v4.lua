...
local RemoteTab = Window:Tab({ Title = "Remotes",    Icon = "send" })
local SpyTab    = Window:Tab({ Title = "Remote Spy", Icon = "radio" })
...
-- ============================================================
-- REMOTES TAB  — tombol-tombol remote event khusus + Color Picker
-- ============================================================
RemoteTab:Section({ Title = "Pallet", Icon = "layers" })

RemoteTab:Button({
    Title       = "Destroy Global Pallet",
    Description = "FireServer → Pallet/Jason/Destroy-Global",
    Callback    = function()
        pcall(function()
            ...
            ReplicatedStorage
                ...
                :WaitForChild("Pallet")
                :WaitForChild("Jason")
                :WaitForChild("Destroy-Global")
                :FireServer(unpack(args))
        end)
        WindUI:Notify({ Title="Remotes", Content="Destroy-Global fired!", Duration=2, Icon="check" })
    end
})

RemoteTab:Section({ Title = "Generator", Icon = "zap" })

-- Dropdown pilih generator + color picker highlight
local genColorEnabled = false
local genHighlightColor = Color3.fromRGB(255, 80, 0)
local genList = {}

local function getGeneratorList()
    local list = {}
    local map = workspace:FindFirstChild("Map")
    if not map then return {"(Map belum dimuat)"} end
    for _, v in ipairs(map:GetChildren()) do
        if v.Name:find("Generator") then
            table.insert(list, v.Name)
        end
    end
    return #list > 0 and list or {"(Tidak ada generator)"}
end
...
RemoteTab:Toggle({
    ...
    Description = "Warnai semua generator di map",
    Value       = false,
    Callback    = function(v)
        genColorEnabled = v
        local map = workspace:FindFirstChild("Map")
            ...
            if obj.Name:find("Generator") then
                for _, part in ipairs(obj:GetDescendants()) do
                    if part:IsA("BasePart") then
                        part.Color = v and genHighlightColor or part.Color
                    end
                end
            end
        end
...
RemoteTab:ColorPicker({
    Title    = "Warna Highlight Generator",
...
genDropdown = RemoteTab:Dropdown({
    Title    = "Pilih Generator",
    Values   = getGeneratorList(),
    Value    = "",
    Callback = function(v) selectedGen = v end
})

RemoteTab:Button({
    Title    = "🔄 Refresh Generator List",
    Callback = function()
        pcall(function() genDropdown:Refresh(getGeneratorList(), false) end)
        selectedGen = ""
    end
})

RemoteTab:Button({
    Title       = "Break Generator (FireServer)",
    Description = "Kirim BreakGenEvent ke generator yang dipilih",
    Callback    = function()
        if selectedGen == "" or selectedGen:find("Tidak ada") or selectedGen:find("belum") then
            WindUI:Notify({ Title="Remotes", Content="Pilih generator dulu!", Duration=2, Icon="alert-circle" })
            return
        end
        pcall(function()
            local target = workspace:WaitForChild("Map"):FindFirstChild(selectedGen)
            if not target then return end
            -- cari GeneratorPoint di dalam generator tsb
            local point = target:FindFirstChildWhichIsA("BasePart") or target
            local args = { point }
            ReplicatedStorage
                :WaitForChild("Remotes")
                :WaitForChild("Generator")
                :WaitForChild("BreakGenEvent")
                :FireServer(unpack(args))
        end)
        WindUI:Notify({ Title="Remotes", Content="BreakGenEvent → "..selectedGen, Duration=2, Icon="zap" })
    end
})

RemoteTab:Section({ Title = "Carry / Hook", Icon = "anchor" })

RemoteTab:Button({
    Title       = "Hook Event (HookPoint)",
    Description = "FireServer → Carry/HookEvent",
    Callback    = function()
        pcall(function()
            local args = {
            ...
            ReplicatedStorage
                :WaitForChild("Remotes")
                :WaitForChild("Carry")
                :WaitForChild("HookEvent")
                :FireServer(unpack(args))
        end)
        WindUI:Notify({ Title="Remotes", Content="HookEvent fired!", Duration=2, Icon="anchor" })
    end
})

RemoteTab:Section({ Title = "Killer / Attacks", Icon = "swords" })

RemoteTab:Button({
    Title       = "Update Weapon (false)",
    Description = "FireServer → Killers/Veil/updatewep  [false]",
    Callback    = function()
        pcall(function()
            ReplicatedStorage
                :WaitForChild("Remotes")
                ...
                :WaitForChild("Veil")
                :WaitForChild("updatewep")
                :FireServer(false)
        end)
        WindUI:Notify({ Title="Remotes", Content="updatewep fired!", Duration=2, Icon="swords" })
    end
})

RemoteTab:Button({
    Title       = "Basic Attack (true)",
    Description = "FireServer → Attacks/BasicAttack  [true]",
    Callback    = function()
        pcall(function()
            ReplicatedStorage
                :WaitForChild("Remotes")
                :WaitForChild("Attacks")
                :WaitForChild("BasicAttack")
                :FireServer(true)
        end)
        WindUI:Notify({ Title="Remotes", Content="BasicAttack fired!", Duration=2, Icon="zap" })
    end
})

RemoteTab:Section({ Title = "Color Picker Kustom", Icon = "palette" })

local customColor = Color3.fromRGB(255, 255, 255)
RemoteTab:ColorPicker({
    Title    = "Warna Aksen (global)",
    Color    = Color3.fromRGB(168, 85, 247),
    Callback = function(c) customColor = c end
})

-- ============================================================
-- REMOTE SPY TAB  — log semua remote event yang di-fire
-- ============================================================
local spyEnabled    = false
local spyLog        = {}        -- { time, path, args }
local spyConns      = {}
local spyMaxLog     = 200       -- max baris sebelum di-trim
local spyFilterStr  = ""
local spyLabel      = nil       -- referensi label output di GUI

-- ScrollingFrame untuk log output (Drawing-based pakai ScreenGui)
local spyGui = Instance.new("ScreenGui")
spyGui.Name           = "VD_RemoteSpy"
spyGui.ResetOnSpawn   = false
spyGui.DisplayOrder   = 997
spyGui.Enabled        = false
spyGui.Parent         = LocalPlayer:FindFirstChild("PlayerGui") or game:GetService("CoreGui")

local spyFrame = Instance.new("Frame", spyGui)
spyFrame.Size             = UDim2.new(0, 480, 0, 300)
spyFrame.Position         = UDim2.new(0.5, -240, 0, 60)
spyFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 15)
spyFrame.BorderSizePixel  = 0
Instance.new("UICorner", spyFrame).CornerRadius = UDim.new(0, 8)
local spyStroke = Instance.new("UIStroke", spyFrame)
spyStroke.Color = Color3.fromRGB(168, 85, 247); spyStroke.Thickness = 1.5

-- Titlebar + drag
local spyTitle = Instance.new("TextLabel", spyFrame)
spyTitle.Size            = UDim2.new(1, -8, 0, 28)
spyTitle.Position        = UDim2.new(0, 8, 0, 0)
spyTitle.BackgroundTransparency = 1
spyTitle.Text            = "📡  Remote Spy  —  drag untuk pindahkan"
spyTitle.TextColor3      = Color3.fromRGB(168, 85, 247)
spyTitle.Font            = Enum.Font.GothamBold
spyTitle.TextSize        = 12
spyTitle.TextXAlignment  = Enum.TextXAlignment.Left

local dragSpy, dsSpy, spSpy = false, nil, nil
spyTitle.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1
        or i.UserInputType == Enum.UserInputType.Touch then
        dragSpy=true; dsSpy=i.Position; spSpy=spyFrame.Position
    end
end)
game:GetService("UserInputService").InputChanged:Connect(function(i)
    if dragSpy and (i.UserInputType==Enum.UserInputType.MouseMovement
        or i.UserInputType==Enum.UserInputType.Touch) then
        local d = i.Position - dsSpy
        spyFrame.Position = UDim2.new(spSpy.X.Scale, spSpy.X.Offset+d.X, spSpy.Y.Scale, spSpy.Y.Offset+d.Y)
    end
end)
game:GetService("UserInputService").InputEnded:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1
        or i.UserInputType==Enum.UserInputType.Touch then dragSpy=false end
end)

-- Scrolling list
local spyScroll = Instance.new("ScrollingFrame", spyFrame)
spyScroll.Size                  = UDim2.new(1, -8, 1, -60)
spyScroll.Position              = UDim2.new(0, 4, 0, 30)
spyScroll.BackgroundColor3      = Color3.fromRGB(8, 8, 12)
spyScroll.BorderSizePixel       = 0
spyScroll.ScrollBarThickness    = 4
spyScroll.ScrollBarImageColor3  = Color3.fromRGB(168, 85, 247)
spyScroll.CanvasSize            = UDim2.new(0, 0, 0, 0)
spyScroll.AutomaticCanvasSize   = Enum.AutomaticSize.Y
Instance.new("UICorner", spyScroll).CornerRadius = UDim.new(0, 4)
Instance.new("UIListLayout", spyScroll).SortOrder = Enum.SortOrder.LayoutOrder

-- Tombol Clear di bawah
local spyClear = Instance.new("TextButton", spyFrame)
spyClear.Size            = UDim2.new(0.48, 0, 0, 24)
spyClear.Position        = UDim2.new(0.01, 0, 1, -28)
spyClear.BackgroundColor3 = Color3.fromRGB(40,10,10)
spyClear.Text            = "🗑 Clear Log"
spyClear.TextColor3      = Color3.fromRGB(255,100,100)
spyClear.Font            = Enum.Font.GothamBold
spyClear.TextSize        = 11
spyClear.BorderSizePixel = 0
Instance.new("UICorner", spyClear).CornerRadius = UDim.new(0, 4)
...
-- Tombol Copy di bawah
local spyCopy = Instance.new("TextButton", spyFrame)
spyCopy.Size            = UDim2.new(0.48, 0, 0, 24)
spyCopy.Position        = UDim2.new(0.51, 0, 1, -28)
spyCopy.BackgroundColor3 = Color3.fromRGB(10,30,50)
spyCopy.Text            = "📋 Copy All"
spyCopy.TextColor3      = Color3.fromRGB(100,200,255)
spyCopy.Font            = Enum.Font.GothamBold
spyCopy.TextSize        = 11
spyCopy.BorderSizePixel = 0
Instance.new("UICorner", spyCopy).CornerRadius = UDim.new(0, 4)
spyCopy.MouseButton1Click:Connect(function()
    local out = {}
    for _, entry in ipairs(spyLog) do
        table.insert(out, string.format("[%s] %s | args: %s", entry.t, entry.path, entry.args))
    end
    pcall(function() setclipboard(table.concat(out, "\n")) end)
    WindUI:Notify({ Title="Remote Spy", Content="Log disalin ke clipboard!", Duration=2, Icon="clipboard" })
end)
...
-- Tambah satu baris ke log GUI
local spyRowIndex = 0
local function appendSpyRow(timeStr, path, argsStr)
    -- Trim kalau terlalu panjang
    if spyRowIndex > spyMaxLog then
        local kids = spyScroll:GetChildren()
        for _, k in ipairs(kids) do
            if k:IsA("TextLabel") then k:Destroy(); break end
        end
        table.remove(spyLog, 1)
    end
    spyRowIndex = spyRowIndex + 1

    local row = Instance.new("TextLabel", spyScroll)
    row.LayoutOrder      = spyRowIndex
    row.Size             = UDim2.new(1, -4, 0, 0)
    row.AutomaticSize    = Enum.AutomaticSize.Y
    row.BackgroundColor3 = spyRowIndex % 2 == 0
        and Color3.fromRGB(14, 14, 20)
    ...
    row.Text             = string.format(
        '<font color="#888888">[%s]</font> <font color="#c084fc">%s</font>\n<font color="#6ee7b7">args:</font> %s',
        timeStr, path, argsStr
    )

    -- auto-scroll ke bawah
    task.defer(function()
        spyScroll.CanvasPosition = Vector2.new(0, math.huge)
    end)
end

-- Fungsi hook semua remote
local function hookRemotes()
    ...
    if not hookfunction and not hookmetamethod then
        WindUI:Notify({ Title="Remote Spy", Content="Executor tidak support hook — coba Synapse/Solara.", Duration=4, Icon="alert-triangle" })
        return
    end

    -- Hook FireServer pada semua RemoteEvent
    local function watchRemote(re)
        -- Pastikan belum di-hook
        local ok, conn = pcall(function()
            return re.OnClientEvent:Connect(function() end) -- dummy, cek akses
        end)
        if conn then conn:Disconnect() end

        -- Hook FireServer
        local ok2, hookConn = pcall(function()
            return re.FireServer:Connect and nil  -- RemoteEvent punya FireServer method
        end)

        -- Pakai hookfunction kalau ada (Synapse style)
        pcall(function()
            local orig = hookfunction(re.FireServer, function(self, ...)
                if spyEnabled then
                    local args = {...}
                    local path = re:GetFullName()
                    local t    = os.date("%H:%M:%S")
                    local aStr = fmtArgs(args)
                    -- filter
                    if spyFilterStr == "" or path:lower():find(spyFilterStr:lower(), 1, true) then
                        table.insert(spyLog, { t=t, path=path, args=aStr })
                        appendSpyRow(t, path, aStr)
                    end
                end
                return orig(self, ...)
            end)
            table.insert(spyConns, { Disconnect = function()
                pcall(function() hookfunction(re.FireServer, function() end) end)
            end })
        end)
    end

    -- Scan semua RemoteEvent di ReplicatedStorage secara rekursif
    local function scanDescendants(parent)
        ...
        -- Listen untuk yang ditambah belakangan
        local c = parent.DescendantAdded:Connect(function(v)
            if (v:IsA("RemoteEvent") or v:IsA("RemoteFunction")) and spyEnabled then
                task.wait(0.1)
                pcall(watchRemote, v)
            end
        end)
...
-- ─── SpyTab UI ───────────────────────────────────────────────
SpyTab:Section({ Title = "Remote Spy", Icon = "radio" })

SpyTab:Toggle({
    Title       = "Enable Remote Spy",
    Description = "Log semua FireServer yang terjadi di game",
    Value       = false,
    Callback    = function(v)
        spyEnabled = v
        spyGui.Enabled = v
        if v then
            hookRemotes()
            WindUI:Notify({ Title="Remote Spy", Content="Remote Spy aktif!", Duration=2, Icon="radio" })
        else
            for _, c in ipairs(spyConns) do pcall(function() c:Disconnect() end) end
            spyConns = {}
            WindUI:Notify({ Title="Remote Spy", Content="Remote Spy dimatikan.", Duration=2, Icon="radio" })
        end
    end
})

SpyTab:Input({
    Title       = "Filter Path  (kosongkan = semua)",
    Description = "Contoh: Attacks  atau  Generator",
    Value       = "",
    Callback    = function(v) spyFilterStr = v end
})
...
SpyTab:Slider({
    ...
    Step     = 10,
    Callback = function(v) spyMaxLog = v end
})
...
SpyTab:Button({
    ...
    Callback = function()
        ...
        pcall(function() setclipboard(table.concat(out, "\n")) end)
        WindUI:Notify({ Title="Remote Spy", Content="Log disalin!", Duration=2, Icon="clipboard" })
    end
})
Info:Label({
