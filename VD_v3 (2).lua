            ...
            if targetPart then
                ...
                if isPlayer then
                    ...
                    if data.progressLabel and obj.Name == "Generator" then
                        local pct = 0
                        pcall(function()
                            -- BENAR: progress disimpan sebagai Attribute "RepairProgress" (0–1 float)
                            ...
                            if rp then
                                ...
                                -- Fallback: cek PointLight hijau = selesai
...
-- ============================================================
-- FLING TAB — isi konten (sebelumnya kosong)
-- ============================================================
FlingTab:Section({ Title = "Fling Tools", Icon = "zap" })

FlingTab:Button({
    Title = "⚡ Fling Killer Terdekat",
    Desc  = "Fling killer yang paling dekat 1x",
    Callback = function()
        pcall(function()
            local root = GetRoot(); if not root then return end
            ...
            if best then NEX_FlingNearest() end
        end)
    end,
})
...
FlingTab:Button({
    ...
    Desc  = "Fling semua orang di server",
    Callback = function()
        pcall(NEX_FlingAll)
    end,
})
...
local flingConn = nil
FlingTab:Toggle({
    Title = "🔄 Auto Fling Killer (Loop)",
    Desc  = "Terus-terusan fling killer setiap 0.5 detik",
    Value = false,
    Callback = function(v)
        flingLoop = v
...
FlingTab:Divider()
FlingTab:Section({ Title = "Generator & Pallet (Killer)", Icon = "tool" })

-- Break All Generator
FlingTab:Button({
    Title = "🔨 Break All Generator",
    Desc  = "Fire BreakGenEvent ke semua GeneratorPoint di map",
    Callback = function()
        local ok, err = pcall(function()
            local be = ReplicatedStorage
                :WaitForChild("Remotes", 5)
                :WaitForChild("Generator", 5)
                :WaitForChild("BreakGenEvent", 5)
            local map = workspace:FindFirstChild("Map")
                or workspace:FindFirstChild("Map1")
            if not map then error("Map tidak ditemukan") end
            local count = 0
            for _, obj in ipairs(map:GetDescendants()) do
                if obj:IsA("BasePart") and obj.Name:find("GeneratorPoint") then
        ...
        WindUI:Notify({
            ...
            Content  = ok and "✅ Semua generator di-break!" or "❌ "..tostring(err),
            Duration = 3, Icon = ok and "check" or "alert-circle",
        })
    end,
})

local autoBreakGen = false
local autoBreakConn = nil
FlingTab:Toggle({
    Title = "🔄 Auto Break Generator (Loop tiap 1s)",
    Desc  = "Terus break semua generator selama aktif",
    Value = false,
    Callback = function(v)
        autoBreakGen = v
        if autoBreakConn then autoBreakConn:Disconnect(); autoBreakConn = nil end
        if v then
            task.spawn(function()
                while autoBreakGen do
                    pcall(function()
                        local be = ReplicatedStorage
                            :FindFirstChild("Remotes")
                            :FindFirstChild("Generator")
                            :FindFirstChild("BreakGenEvent")
                        if not be then return end
...
FlingTab:Button({
    ...
    Desc  = "Fire PalletDropEvent ke semua PalletPoint di map",
    Callback = function()
        local ok, err = pcall(function()
            local pe = ReplicatedStorage
                :WaitForChild("Remotes", 5)
                :WaitForChild("Pallet", 5)
                :WaitForChild("PalletDropEvent", 5)
            local maps = {
            ...
            for _, map in ipairs(maps) do
                if map then
                    for _, obj in ipairs(map:GetDescendants()) do
                        if obj:IsA("BasePart") and obj.Name:lower():find("palletpoint") then
                            pcall(function() pe:FireServer(obj) end)
                            count = count + 1
                            task.wait(0.05)
                        end
                    end
        ...
        WindUI:Notify({
            Title    = "Drop Pallet",
            Content  = ok and "✅ Semua pallet dijatuhkan!" or "❌ "..tostring(err),
            Duration = 3, Icon = ok and "check" or "alert-circle",
        })
    end,
})

local autoPallet = false
FlingTab:Toggle({
    Title = "🔄 Auto Drop Pallet saat Killer Dekat",
    Desc  = "Drop semua pallet kalau killer dalam 20 studs",
    Value = false,
    Callback = function(v)
        autoPallet = v
        if v then
                        ...
                        for _, pl in ipairs(Players:GetPlayers()) do
                            if pl ~= LocalPlayer and IsKiller(pl) and pl.Character then
                                ...
                                if kHRP and (kHRP.Position - myHRP.Position).Magnitude <= 20 then
                                    local pe = ReplicatedStorage:FindFirstChild("Remotes")
                                        :FindFirstChild("Pallet"):FindFirstChild("PalletDropEvent")
                                    if not pe then return end
                                    local maps = {workspace:FindFirstChild("Map"), workspace:FindFirstChild("Map1")}
                                    for _, map in ipairs(maps) do
                                        if map then
                                            for _, obj in ipairs(map:GetDescendants()) do
                                                if obj:IsA("BasePart") and obj.Name:lower():find("palletpoint") then
