...
EspTab:ColorPicker({
    ...
    Callback = function(v)
        COLOR_SURVIVOR = v
        -- Update semua survivor ESP yang sudah ada
        for _, pl in ipairs(Players:GetPlayers()) do
            if pl.Character and pl ~= LocalPlayer then
                local isMurderer = pl.Character:FindFirstChild("Weapon") ~= nil
                if not isMurderer and espObjects[pl.Character] then
                    local data = espObjects[pl.Character]
                    if data.highlight then
...
EspTab:ColorPicker({
    ...
    Callback = function(v)
        ...
        for _, pl in ipairs(Players:GetPlayers()) do
            if pl.Character and pl ~= LocalPlayer then
                local isMurderer = pl.Character:FindFirstChild("Weapon") ~= nil
                if isMurderer and espObjects[pl.Character] then
                    local data = espObjects[pl.Character]
                    if data.highlight then
                        data.highlight.FillColor = v
                        data.highlight.OutlineColor = v
...
-- Heal diri sendiri (instant - spoof SkillCheck selesai berkali-kali)
SurTab:Button({
    Title       = "💊 Heal Diri Sendiri (Instant)",
    Description = "Fire SkillCheck success berkali-kali agar langsung full",
    Callback    = function()
        local ok, err = pcall(function()
            local char = LocalPlayer.Character
            ...
            -- Spoof: fire sebagai healer terhadap diri sendiri, repeat agar full
            for i = 1, 20 do
...
-- Auto Heal diri sendiri (instant tiap interval)
local autoHealSelfEnabled = false
SurTab:Toggle({
    Title       = "Auto Heal Diri  (loop tiap 1s)",
    Value       = false,
    Callback    = function(v)
...
do
    ...
    SurTab:Button({
        ...
        Description = "SkillCheck success → heal langsung full tanpa proses",
        Callback    = function()
            ...
            local ok, err = pcall(function()
                ...
                -- Fire berkali-kali agar heal langsung full
                for i = 1, 20 do
...
-- Rusak Semua Generator Instan
GriefTab:Button({
    Title = "Break All Generators ⛔",
    Callback = function()
        local gens = workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("Generator")
        if gens then
            for _, gen in pairs(gens:GetChildren()) do
                pcall(function()
                    game:GetService("ReplicatedStorage").Remotes.Generator.BreakGenEvent:FireServer(gen)
                end)
            end
            WindUI:Notify({ Title = "Griefing", Content = "Semua Generator Rusak!", Duration = 3, Icon = "skull" })
        else
            WindUI:Notify({ Title = "Griefing", Content = "Generator tidak ditemukan!", Duration = 3, Icon = "alert-circle" })
        end
    end
})

-- Hancurkan Semua Pallet Instan
GriefTab:Button({
        ...
        for _, v in pairs(workspace:GetDescendants()) do
            if v.Name == "Pallet" or v.Name == "PalletStatic" then
                ...
                pcall(function()
                    ...
                    game:GetService("ReplicatedStorage").Remotes.Pallet.Jason["Destroy-Global"]:FireServer(unpack(args))
                end)
            end
        end
        WindUI:Notify({ Title = "Griefing", Content = found and "Semua Pallet Hancur!" or "Pallet tidak ditemukan!", Duration = 3, Icon = found and "skull" or "alert-circle" })
    end
})
