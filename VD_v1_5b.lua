...
-- HealEvent       → FireServer(HumanoidRootPart, false=heal / true=revive)
-- SkillCheckResult→ FireServer("neutral", 0, Character)   ← BUKAN "success",1 !
-- Reset           → FireServer(Player)                    ← untuk self reset
...
local function getResetRemote()
    if remReset then return remReset end
    local ok, r = pcall(function()
        return ReplicatedStorage:WaitForChild("Remotes",5)
            :WaitForChild("Healing",5):WaitForChild("Reset",5)
    end)
    if ok then remReset = r end
    return remReset
end

-- ══ 1. INSTAN REVIVE diri sendiri ════════════════════════════
-- Cara 1: Reset(LocalPlayer) — paling bersih untuk self
-- Cara 2: HealEvent(myHRP, true) spam
-- Cara 3: SkillCheck("neutral", 0, myChar) — argumen BENAR dari spy
SurTab:Button({
    Title       = "💉 Instan Revive Diri Sendiri",
    Description = "Reset + HealEvent + SkillCheck(neutral,0) → langsung bangun",
    Callback    = function()
        local ok, err = pcall(function()
            local char = LocalPlayer.Character
            ...
            -- Cara 1: Reset
            local rr = getResetRemote()
            if rr then
                for i = 1, 5 do
                    pcall(function() rr:FireServer(LocalPlayer) end)
                    task.wait(0.05)
                end
            end

            -- Cara 2: HealEvent(hrp, true) = revive
            if hrp then
            ...
            -- Cara 3: SkillCheck("neutral", 0, char) — argumen benar dari spy
            ...
            task.spawn(function()
                while autoReviveSelfEnabled do
                    pcall(function()
                        ...
                        -- Reset
                        local rr = getResetRemote()
                        if rr then pcall(function() rr:FireServer(LocalPlayer) end) end
...
do
    ...
    SurTab:Button({
        ...
        Description = "20 coroutine HealEvent + SkillCheck(neutral,0) → langsung hidup",
        Callback    = function()
            ...
            local ok, err = pcall(function()
                ...
                -- 20 coroutine sekaligus = 20 healer simultan
                for i = 1, 20 do
                    task.spawn(function()
                        for j = 1, 5 do
                            pcall(function() rh:FireServer(tHRP, true) end) -- true = revive
                            task.wait(0.05)
                        end
                    end)
                end

                -- SkillCheck dengan argumen BENAR: "neutral", 0
                task.delay(0.15, function()
                    local sk = getSkillCheck()
