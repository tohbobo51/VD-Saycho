...
-- ============================================================
-- SURVIVOR REMOTES  — Fast Vault, Instant Vault, Stop Emote,
--                     Heal Player
-- ============================================================
SurTab:Section({ Title = "Vault / Movement", Icon = "wind" })

SurTab:Button({
    Title       = "Fast Vault (diri sendiri)",
    Description = "FireServer → Window/fastvault [LocalPlayer]",
    Callback    = function()
        local ok, err = pcall(function()
            local remote = ReplicatedStorage
                :WaitForChild("Remotes", 5)
                :WaitForChild("Window", 5)
                :WaitForChild("fastvault", 5)
            remote:FireServer(LocalPlayer)
        end)
        WindUI:Notify({
            Title    = "Vault",
            Content  = ok and "Fast Vault fired!" or "Gagal: " .. tostring(err),
            Duration = 2,
            Icon     = ok and "wind" or "alert-circle"
        })
    end
})

SurTab:Button({
    Title       = "Instant Vault (VaultEvent + Complete)",
    Description = "Kirim VaultEvent lalu VaultCompleteEvent seketika",
    Callback    = function()
        local ok, err = pcall(function()
            local remotes = ReplicatedStorage:WaitForChild("Remotes", 5)
            local winRemotes = remotes:WaitForChild("Window", 5)
            local vaultEvent    = winRemotes:WaitForChild("VaultEvent", 5)
            local vaultComplete = winRemotes:WaitForChild("VaultCompleteEvent", 5)

            -- Cari VaultTrigger di Map
            local map     = workspace:WaitForChild("Map", 5)
            local trigger = nil
            for _, v in ipairs(map:GetDescendants()) do
                if v.Name == "VaultTrigger" then trigger = v; break end
            end
            if not trigger then error("VaultTrigger tidak ditemukan di Map") end

            vaultEvent:FireServer(trigger, true)
            task.wait(0.05)     -- jeda kecil supaya server proses start dulu
            vaultComplete:FireServer(trigger, false)
        end)
        WindUI:Notify({
            Title    = "Vault",
            Content  = ok and "Instant Vault selesai!" or "Gagal: " .. tostring(err),
            Duration = 2,
            Icon     = ok and "check" or "alert-circle"
        })
    end
})
...
SurTab:Button({
    Title       = "Stop Emote",
    Description = "FireServer → EmoteHandler [StopEmote]",
    Callback    = function()
        local ok, err = pcall(function()
            ReplicatedStorage
                :WaitForChild("Remotes", 5)
        ...
        WindUI:Notify({
            ...
            Duration = 2,
            Icon     = ok and "smile" or "alert-circle"
        })
    end
})

SurTab:Section({ Title = "Heal Player", Icon = "heart" })
...
do
    ...
    local healDropdown = SurTab:Dropdown({
        ...
        Value    = "",      -- kosong = tidak trigger callback saat init
    ...
    SurTab:Button({
        ...
        Callback    = function()
            if healSelectedName == "" or healSelectedName == "(Tidak ada player lain)" then
            ...
            local ok, err = pcall(function()
                ...
                local hrp = target.Character
                    and target.Character:WaitForChild("HumanoidRootPart", 3)
                if not hrp then error("HumanoidRootPart tidak ditemukan") end

                ReplicatedStorage
                    :WaitForChild("Remotes", 5)
                    :WaitForChild("Healing", 5)
                    :WaitForChild("HealEvent", 5)
                    :FireServer(hrp, false)
            end)
            WindUI:Notify({
                Title    = "Heal",
                Content  = ok and "Heal → " .. healSelectedName or "Gagal: " .. tostring(err),
                Duration = 2,
                Icon     = ok and "heart" or "alert-circle"
            })
        end
    })
end
