...
--   HealEvent(HRP, false) = sembuhkan luka biasa (kena pukul killer)
--   HealEvent(HRP, true)  = revive saat lagi knocked/down
--   Reset(LocalPlayer)    = reset status kamu (berdiri sendiri saat down/knock)
-- ─────────────────────────────────────────────────────────────
...
-- Helper: fire HealEvent lalu langsung StopEmote
local function fireHeal(hrp, isRevive)
    local rem = getHealEvent()
    if not rem then error("Remote HealEvent tidak ditemukan") end
    -- Fire 3x cukup (server biasanya proses sekali, 10x tidak perlu)
    for i = 1, 3 do
        rem:FireServer(hrp, isRevive)
        task.wait(0.05)
    end
    -- ✅ FIX: langsung StopEmote supaya karakter tidak stuck animasi
    pcall(function()
...
-- ── Heal Diri (luka biasa, kena pukul killer) ───────────────
SurTab:Button({
    Title       = "💊 Heal Luka Diri Sendiri",
    Description = "Sembuhkan luka biasa kena pukul killer (HealEvent false)",
    Callback    = function()
        local ok, err = pcall(function()
            local char = LocalPlayer.Character
            if not char then error("Karakter tidak ada") end
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if not hrp then error("HumanoidRootPart tidak ada") end
            fireHeal(hrp, false)
        end)
        WindUI:Notify({
            Title   = "Heal",
            Content = ok and "✅ Heal luka berhasil!" or "❌ Gagal: "..tostring(err),
            Duration = 2, Icon = ok and "heart" or "alert-circle"
        })
    end
})

-- ── Revive Diri (saat knocked / down) ───────────────────────
SurTab:Button({
    Title       = "💙 Revive Diri Sendiri (Saat Down/Knock)",
    Description = "Bangunkan diri saat di-knock killer (HealEvent true)",
    Callback    = function()
        local ok, err = pcall(function()
            local char = LocalPlayer.Character
            if not char then error("Karakter tidak ada") end
        ...
        WindUI:Notify({
            Title   = "Revive",
            Content = ok and "✅ Revive diri berhasil!" or "❌ Gagal: "..tostring(err),
            Duration = 2, Icon = ok and "heart" or "alert-circle"
        })
    end
})

-- ── Reset Status (bangun sendiri via Reset remote) ───────────
SurTab:Button({
    Title       = "🔄 Reset Status (Bangun Sendiri)",
    Description = "Reset kondisi knocked/injured kamu sendiri via Healing/Reset",
    Callback    = function()
        local ok, err = pcall(function()
            local rem = getHealReset()
            if not rem then error("Remote Reset tidak ditemukan") end
            rem:FireServer(LocalPlayer)
        end)
        WindUI:Notify({
            Title   = "Reset",
            Content = ok and "✅ Status di-reset!" or "❌ Gagal: "..tostring(err),
...
SurTab:Toggle({
    Title       = "Auto Heal Luka Diri (loop tiap 1s)",
    Description = "Otomatis sembuhkan luka biasa setiap detik",
    Value       = false,
    Callback    = function(v)
                ...
                while autoHealSelfEnabled do
                    pcall(function()
                        ...
                        if hrp and rem then
                            ...
                            -- ✅ FIX: stop emote langsung supaya tidak stuck gerak
                            local eh = getEmoteHandler()
                            if eh then pcall(function() eh:FireServer("StopEmote") end) end
                        end
                    end)
                    task.wait(1)
                end
...
do
    ...
    -- Heal luka biasa orang lain
    SurTab:Button({
        Title       = "💊 Heal Luka Player yang Dipilih",
        Description = "Sembuhkan luka biasa player (HealEvent false)",
        Callback    = function()
            ...
            local ok, err = pcall(function()
                ...
                fireHeal(hrp, false)  -- ✅ FIX: pakai fireHeal (auto StopEmote)
            end)
            WindUI:Notify({
                Title   = "Heal",
    ...
    -- Revive orang lain (saat knocked)
    SurTab:Button({
        Title       = "💙 Revive Player yang Dipilih (Saat Down)",
        Description = "Bangunkan player yang lagi di-knock (HealEvent true)",
        Callback    = function()
            if healTargetName == "" or healTargetName:find("Tidak ada") then
            ...
            local ok, err = pcall(function()
                ...
                local hrp = target.Character:FindFirstChild("HumanoidRootPart")
                if not hrp then error("Target tidak punya HRP") end
                fireHeal(hrp, true)   -- ✅ FIX: pakai fireHeal (auto StopEmote)
            end)
            WindUI:Notify({
                Title   = "Revive",
...
local function tryFireUnhook(hookObj)
    ...
    -- ✅ METODE B: Fire langsung tanpa teleport
    -- Tidak ada pergerakan karakter, tidak kelihatan orang lain
    -- Server VD tidak validasi posisi player di UnHookEvent
    pcall(function()
...
local function runAutoUnhook()
    task.spawn(function()
        while _G.AutoUnhook do
            -- ✅ METODE B: fire langsung tanpa cek jarak, no teleport
            ...
            for _, hookObj in ipairs(hooks) do
                ...
                if refPart then
                    ...
                    if hasVictim then
                        tryFireUnhook(hookObj)  -- metode B: fire langsung, no teleport
                        WindUI:Notify({
                            Title    = "Auto Unhook",
