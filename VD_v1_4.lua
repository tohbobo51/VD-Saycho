...
local function getAllHooks()
    ...
    if not mainMap then return list end
    for _, obj in ipairs(mainMap:GetDescendants()) do
        if obj.Name == "Hook" or obj.Name == "HookPoint" or obj.Name == "HookBase" then
            table.insert(list, obj)
        end
    end
    return list
...
-- Remote SkillCheckResultEvent untuk heal instan (sama trik seperti auto generator)
...
-- Heal diri sendiri INSTAN
-- Teknik: fire SkillCheckResultEvent dengan character sendiri sebagai target
-- Server akan proses seolah orang lain yang heal kita → bisa full & instan
SurTab:Button({
...
-- Auto Heal diri sendiri INSTAN (loop)
local autoHealSelfEnabled = false
SurTab:Toggle({
    Title       = "Auto Heal Diri (Instant, loop tiap 1s)",
    Value       = false,
    Callback    = function(v)
...
local function getUnhookRemote()
    local rem = ReplicatedStorage:FindFirstChild("Remotes"); if not rem then return nil end
    -- Coba path yang paling umum di Violence District
    local paths = {
        function() return rem:FindFirstChild("Carry") and rem.Carry:FindFirstChild("HookEvent") end,
        function() return rem:FindFirstChild("Hook") and rem.Hook:FindFirstChild("Unhook") end,
        function() return rem:FindFirstChild("Hook") and rem.Hook:FindFirstChild("HookEvent") end,
        function() return rem:FindFirstChild("Unhook") end,
        function() return rem:FindFirstChild("Carry") and rem.Carry:FindFirstChild("Unhook") end,
        function()
            -- Scan semua child Remotes untuk nama yang mengandung "hook"/"unhook"
            for _, v in ipairs(rem:GetDescendants()) do
                local n = v.Name:lower()
                if (n:find("unhook") or n:find("hookevent") or n:find("rescue")) and v:IsA("RemoteEvent") then
                    return v
                end
            end
        end,
...
local function tryFireUnhook(hookPart)
    local remote = getUnhookRemote()
    if not remote then
        warn("[VD Unhook] Remote tidak ditemukan! Cek RemoteSpy untuk path yang benar.")
        return
    end
    -- Kirim hookPart langsung, dan juga parent-nya (beberapa game pakai parent model)
    pcall(function() remote:FireServer(hookPart) end)
    pcall(function() remote:FireServer(hookPart.Parent) end)
end
...
local function runAutoUnhook()
    task.spawn(function()
        while _G.AutoUnhook do
            local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                -- Pakai getAllHooks() — scan Map rekursif, tidak limit 1 level
                local hooks = getAllHooks()
                for _, hookObj in ipairs(hooks) do
                    if not _G.AutoUnhook then break end

                    -- hookObj bisa BasePart langsung atau Model yang punya BasePart
                    local hookPart = hookObj:IsA("BasePart") and hookObj
                        or hookObj:FindFirstChildWhichIsA("BasePart")

                    if hookPart then
                        local dist = (hrp.Position - hookPart.Position).Magnitude
                        if dist <= _G.UnhookRange then
                            -- Cek victim: ada player yang HRP dekat hook
                            -- Toleransi diperbesar ke 8 studs (beberapa game offset sedikit)
                            local hasVictim = false
                            for _, pl in ipairs(Players:GetPlayers()) do
                                if pl ~= LocalPlayer and pl.Character then
                                    local plHRP = pl.Character:FindFirstChild("HumanoidRootPart")
                                    if plHRP and (plHRP.Position - hookPart.Position).Magnitude < 8 then
                            ...
                            if hasVictim then
                                tryFireUnhook(hookPart)
                                WindUI:Notify({
                                    Title    = "Auto Unhook",
                                    Content  = "Unhook dikirim ke " .. hookPart.Name,
                                    Duration = 1.5,
                                    Icon     = "anchor"
                                })
                            end
            ...
            task.wait(0.8) -- lebih cepat dari sebelumnya (1s → 0.8s)
        end
    end)
end
