...
VerLabel.Text = "v12.0"; VerLabel.Position = UDim2.new(1, -70, 0, 16)
...
-- ✅ FIX: Activated instead of MouseButton1Click (works on Delta/mobile)
...
-- ─── REMOTE-BASED LOGIC (Auto Gen, etc.) ────────────────────
-- Auto Generator: pakai RepairEvent + SkillCheckResultEvent (dari V2, lebih efektif)
task.spawn(function()
    while task.wait(0.15) do
        if States.AutoGenerator and R_RepairEvent and R_SkillCheck then
            pcall(function()
                local map = WS:FindFirstChild("Map") or WS:FindFirstChild("Map1"); if not map then return end
                for _, obj in ipairs(map:GetDescendants()) do
                    if obj:IsA("Model") and obj.Name == "Generator" then
                        for _, point in ipairs(obj:GetChildren()) do
                            if point.Name:find("GeneratorPoint") then
                                pcall(function()
                                    R_RepairEvent:FireServer(point, true)
                                    R_SkillCheck:FireServer("success", 1, obj, point)
                                end)
                            end
                        end
                    end
        ...
        -- Auto Leave Generator (jauhin gen kalau killer deket)
        ...
        -- Auto Attack (Killer)
        if States.AutoAttack and R_BasicAttack then
            pcall(function() R_BasicAttack:FireServer(false) end)
        end
        -- Auto Lunge
        if States.AutoLunge and R_Lunge then
            pcall(function() R_Lunge:FireServer(false) end)
        end
        -- Power Skip
        if States.PowerSkip and R_PowerDone then
            pcall(function() R_PowerDone:FireServer() end)
        end
        -- Heal Reset
        if States.AutoHealReset and R_HealReset then
            pcall(function() R_HealReset:FireServer(LocalPlayer) end)
        end
    end
end)
...
CreateToggle(SurPage, "Auto Generator  (+ SkillCheck)", false, function(s) States.AutoGenerator=s end, 2)
...
WaterText.Text=" LYNXX VD v12.0  |  Saycho"; WaterText.Size=UDim2.new(1,0,1,0)
...
print("[VD v12.0] Loaded — Saycho | Fixed MouseButton1Click → Activated")
