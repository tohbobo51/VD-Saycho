...
-- Helper: fire HealEvent langsung tanpa Ghost TP (tidak perlu cek jarak)
-- Spam 10x untuk pastikan server menerima — cara ini terbukti bekerja di v1.5
...
local function tryFireUnhook(hookObj)
    ...
    -- Fire langsung tanpa Ghost TP — server tidak cek jarak untuk UnHookEvent
    -- Spam 3x untuk meningkatkan keberhasilan
    for i = 1, 3 do
        local ok, err = pcall(function()
            remote:FireServer(hookPoint)
        end)
        if not ok then
...
-- Gunakan Healing/Reset (yang memang dibuat untuk self) + backup spam HealEvent
local function selfHealRevive()
    local char = LocalPlayer.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")

    -- Cara 1: Healing/Reset remote — ini yang paling bisa untuk self
    local remReset = getHealReset()
    if remReset then
        for i = 1, 5 do
            pcall(function() remReset:FireServer(LocalPlayer) end)
            task.wait(0.05)
        end
    end

    -- Cara 2: Backup — spam HealEvent(hrp, true) untuk boost recovery
    if hrp then
        local remHeal = getHealEvent()
        if remHeal then
            for i = 1, 10 do
                pcall(function() remHeal:FireServer(hrp, true) end)
                task.wait(0.05)
            end
        end
    end

    -- Stop animasi knocked agar bisa bergerak
    pcall(function()
        local eh = getEmoteHandler()
        if eh then eh:FireServer("StopEmote") end
    end)
    ...
    WindUI:Notify({
        Title   = "Self Heal/Revive",
        Content = "✅ Reset + Revive dikirim!",
...
local function selfUnhook()
    ...
    if targetedHook then
        ...
        local ok, err = pcall(function()
            local rem = getUnhookRemote()
            if not rem then error("Remote UnHookEvent tidak ditemukan") end
            -- Fire langsung 3x tanpa Ghost TP
            for i = 1, 3 do
        ...
        WindUI:Notify({
            Title   = "Self Unhook",
            Content = ok and "✅ Unhook berhasil dikirim!" or "❌ Gagal: "..tostring(err),
