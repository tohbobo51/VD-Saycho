--[[
  ...
  Version : 15.1 + Veil Killer
  Author  : Saycho
  New v15 : 1 tombol Moonwalk (floating, GUI = show/hide saja)
            Tombol Jump on-screen (muncul saat Infinite Jump ON)
            Draggable FPS Counter (Lynxx|fps|ms)
]]
...
-- ── VEIL KILLER REMOTES — nil dulu, diload async setelah GUI ────
local R_VeilVFX       = nil
...
SubLabel.Text = "Violence District  v15.0"; SubLabel.Position = UDim2.new(0, 82, 0, 16)
...
local function startVeilKillAura()
    ...
    veilAuraConn = RunService.Heartbeat:Connect(function(dt)
        ...
        pcall(function()
            ...
            if target then pcall(function() R_BasicAttack:FireServer(true) end) end
        end)
    end)
end
...
local function startVeilVFXSpam()
    ...
    veilVFXConn = RunService.Heartbeat:Connect(function(dt)
        ...
        _vfxT = _vfxT + dt
        if _vfxT < 0.5 then return end
        _vfxT = 0
        pcall(function()
            if R_VeilVFX    then R_VeilVFX:FireServer(1, true, false) end
            if R_VeilUpdateWep then R_VeilUpdateWep:FireServer(false) end
        end)
    end)
end
...
-- ── VEIL KILLER SECTION ──────────────────────────────────────────
SectionLabel(KilPage, "🗡  VEIL  (aktif saat main Veil)", 5)
...
do
    ...
    lbl.Size=UDim2.new(1,-14,1,0); lbl.Position=UDim2.new(0,14,0,0)
    lbl.Text="⚠  Remote dicek otomatis — aman jika Veil tidak terload"
...
-- ── VEIL REMOTE ASYNC LOADER ────────────────────────────────────
-- Pakai WaitForChild + timeout agar remote pasti ketemu setelah load
-- Kalau bukan Veil: semua nil, pcall di logic sudah handle
task.spawn(function()
    local ok, err = pcall(function()
        -- Tunggu Remotes folder (max 15 detik)
        local Remotes = RS:WaitForChild("Remotes", 15)
        if not Remotes then return end

        -- Cek dulu apakah Killers/Veil ada (pakai FindFirstChild, no wait)
        -- Kalau tidak ada = bukan main Veil, keluar normal
        local Killers = Remotes:WaitForChild("Killers", 10)
        if not Killers then return end

        local Veil = Killers:WaitForChild("Veil", 10)
        if not Veil then
            warn("[VD] Folder Veil tidak ada — normal jika bukan main Veil")
            return
        end

        -- Sekarang load remote satu per satu pakai WaitForChild
        R_VeilVFX       = Veil:WaitForChild("vfx",        8)
        R_VeilUpdateWep = Veil:WaitForChild("updatewep",   8)
        R_VeilSpear     = Veil:WaitForChild("Spearthrow",  8)

        print("[VD] ✅ Veil remotes loaded!")
        print("  vfx:", R_VeilVFX ~= nil)
        print("  updatewep:", R_VeilUpdateWep ~= nil)
        print("  Spearthrow:", R_VeilSpear ~= nil)
    end)
    if not ok then
        warn("[VD] Veil loader error: " .. tostring(err))
    end
end)

print("[VD v15.1] Loaded + Veil Killer (Silent Aim, Kill Aura, VFX Spam) — Saycho | 1 MoonBtn + JumpBtn (muncul saat InfJump ON) + Draggable FPS")
